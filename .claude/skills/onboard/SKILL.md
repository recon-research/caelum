...
5. **Pick up the Resume point** (branch · issue · next action · verify command) and run the [`textbooks/AGENT_GUIDE.md`](../../../textbooks/AGENT_GUIDE.md) build loop on the slice. Delegate broad exploration to read-only subagents (`mech-sweeper` for mechanical breadth); keep conclusions, not file dumps. **Emit the session model/effort recommendation** for this slice — one line, keyed to [`.claude/skills/README.md` › Session model & effort](../README.md) (`this slice is mechanical — Sonnet/medium is plenty` / `this is a D-NN-adjacent design slice — strongest model, max effort`), so the human sees it exactly when they'd change the setting (#151).
