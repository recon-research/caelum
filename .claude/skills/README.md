...
| [understand_intent](understand_intent/SKILL.md) | "help me figure out" / "what are my options" / a knowledge-gap ask — landscape-first elicitation *before* solutioning or the decision issue: teach, then ask; emits the intent brief (job restatement · dimensions · criteria · consequence grid · what-matters-most · dissolve check). Grounded in `research/notes/intent-elicitation.md` (#290). |
...
| [fleet_size](fleet_size/SKILL.md) | "how many sessions can we run?" — certify-safe fleet width from the ready set's Surfaces disjointness; N + lane partition + limiting factor (D-330, #332). |
...
| **inherit** (session model) | default agents + [`adversarial-reviewer`](../agents/adversarial-reviewer.md) + [`repro-verifier`](../agents/repro-verifier.md) (no `model:` line) | judgment — architecture, review lenses, root-causing, gates | — |
...
**Fan-out width — the second axis (#312).** The table routes *which model*; width (N workers / N sources) scales with a **declared complexity class**, never ad-hoc mood: each fan-out skill carries its own class→N rubric, and its report/note/run-ticket must carry the forced `effort: <class> → N` line (worked rubrics: `adversarial_review` step 2, `research_topic` step 3, `harvest_downstream` step 4). Grounding — cite as research notes, never a Book §: `research/notes/agent-project-systems.md` (token budget explained ~80% of outcome variance in Anthropic's multi-agent system, whose prompts embed per-class effort-scaling rules; the same note's Ruflo entry is the over-scaling anti-pattern) and `research/notes/parallel-builder-fleet.md` (vague briefs duplicate work — width past the rubric's cap buys overlap, not coverage).
...
- **Gated skills carry eval goldens — a SKILL.md diff merges only on a fresh green run.** The six highest-stakes skills (the `GATED_SKILLS` tuple in `scripts/skill_evals.py`) each ship 2–3 scenario goldens in `<name>/evals/goldens.json`, asserted mechanically against a fresh-subagent dry run; the preflight/CI `skill evals` stage refuses a stale or red stamp. Contract, protocol, and failure triage: [`EVALS.md`](EVALS.md) (#302). Re-run the suite when the session model changes (`onboard` Mode B watches for it). Skill text *evolves* on evidence — failure diagnoses and mined successes — through the beat-the-incumbent loop (`EVALS.md` › The evolution loop, #303).
- **A misfire is a defect — file it, then fix it (#48).** If a skill misfires (fires when it shouldn't, is skipped when it should, or its procedure steers wrong), file `skill-defect: <name> — <symptom>` (label `debt`) at the moment it happens, with the required `diagnosis:` block (excerpt + root cause — shape in `track_followups` bullet 5, #303) — chat observations evaporate at compaction, and these tickets are [`retrospective`](retrospective/SKILL.md)'s skill-layer input. Then tighten the `description` or procedure via PR. **Measure description edits:** a frontmatter/description change claiming an efficiency or routing win states its before/after in the PR body — listing size via `/context`, invocation counts from `.claude/metrics/skill_usage.jsonl` once the ledger has data (the #6 measure-first discipline).
...
- **Mind the always-resident listing.** Each skill's `name` + `description` stays in context permanently so Claude can auto-route to it (the `SKILL.md` body loads only when invoked). Claude Code budgets ~1% of the window for the whole listing and caps each entry at 1,536 chars ([skills docs](https://code.claude.com/docs/en/skills.md)) — keep descriptions tight and lead with the trigger phrase. This template's 34 skills total ~3.8k tokens (~1.9% of a 200k window): past the default budget, so raising `skillListingBudgetFraction` in settings (or trimming descriptions) is live guidance here, not future-proofing. Measure yours with `/context`. (The skill count in this sentence is audit-asserted against the catalog — `_audit_routing.py`, #261.)
