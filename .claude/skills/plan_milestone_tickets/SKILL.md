...
4. **Write each ticket** to the slice template's shape (`.github/ISSUE_TEMPLATE/slice.md`) — a **context-payload ticket** (#304): Goal (one sentence — what exists after the merge) · Context (milestone, plan/`D-NN`, **verified** library grounding, `Blocked-by: #NN` edges the ready query reads — #308) · `Surfaces:` line (the predicted file/dir footprint — the fleet's disjointness gate reads it, #332; predict honestly, don't pad) · **Dev Notes** (the payload: the architecture constraints that bind this slice + the source paths to read first, cited — kills per-slice re-discovery; the before/after receipt delta is the metric) · Acceptance criteria (verifiable, numbered; **EARS shape recommended where it fits** — `WHEN <trigger> THE SYSTEM SHALL <behavior>` — so `definition_of_done` evidence maps per-SHALL; a recommended shape, never a straitjacket — template tickets vary in kind) · Out of scope (deferred items filed as `followup` now). **Write-regions:** planning text (Goal/Context/Dev Notes/ACs) is immutable to the executing session — it appends progress/findings as comments or a Record section, never edits the plan (provenance stays legible). Label `slice` (+ any area labels the project uses), assign the milestone.
