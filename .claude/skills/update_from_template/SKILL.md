...
3. **Machinery — overwrite from upstream** (the README boundary table is authoritative; re-check it each sync, it can grow):
   ...
   - `scripts/` and `.github/` — take upstream's structure, then re-apply the project's filled stage bodies / real CI commands on top (structure is upstream's; commands are the project's). A synced issue template that auto-labels (`labels: [...]` in a form) needs its label(s) to exist — `gh label create <name>` for any the repo lacks. GitHub silently drops unknown labels at submission: the ticket lands unlabeled and label-filtered listings (e.g. `triage_inbox`'s `--label inbox`) never see it, so the lane looks wired while being invisible (#127, hit live downstream).
