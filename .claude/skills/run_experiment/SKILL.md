...
6. **Record the outcome** against the *pre-registered* bar: supported / refuted / inconclusive — plus Failure modes & limitations (where it breaks, what wasn't tested). **Negative results are recorded with the same care** — they prevent re-litigating the idea next quarter. A refutation is also what licenses *deleting* the mechanism later, so it carries a debt forward: **the PR that deletes the code under test owes this experiment a last-reproducing-commit note** in its `## Reproducibility` section — figure regeneration survives the deletion, re-execution does not, and the break is often silent rather than loud (`research/README.md` rule 7 is the single home of that rule and its worked instance, #713).
7. **Wire it in:** `experiments[]` entry in `research/MANIFEST.json` (status: `experimenting` → the note's status too); run `python3 tools/_audit_research.py` (from `research/` — the script's cwd contract); file follow-up tickets; if the outcome grounds a decision, update/file the `decision` issue citing `EXP-NN`. Ship the PR.

**Worked example — judging LLM output:** reuse the EXP-01 harness shape (`research/experiments/EXP-NN_<slug>/EXPERIMENT.md`; upstream's instance is EXP-01_delegated-drafting) rather than rebuilding it. **Spelled as a shape, deliberately** — this is an *illustrative example*, not a grounding citation, so the tree does **not** travel with this skill (README boundary table › *Cited research artifact*), and any resolvable spelling dangles in every downstream copy. #204 demoted the original hard link to a code span, which held until #625 taught `audit_ops_config` to resolve backticked citations too — a concrete path here now hard-fails a downstream's first preflight (#646, the #196 class): blinding assignment fixed at pre-registration, order-swapped judge passes by fresh provenance-blind agents (position-bias check for free), generated specimens stored under `results/` as frozen *data* (never merged into `textbooks/`, no citation authority), prompt files committed verbatim and exempt from the tracker-hygiene grep (`research/experiments/*/prompts/*`), a stdlib conformance checker as the mechanical gate, and a stdlib SVG plot script (no matplotlib dependency). Cost accounting: prefer the Agent tool's per-run `subagent_tokens` (orchestration-free) alongside ledger deltas — and pass workers file paths, not pasted text (`CLAUDE.md` › Token discipline). **Budget fleet *rate*, not just total cost** — concurrency spends against the account's rolling token window, not only the priced total: both experiment-scale sessions to date priced tokens correctly and still exhausted the account's 5-hour cap mid-run (#179), because ~10 concurrent max-effort agents drain the window in minutes. Estimate per-agent tokens × concurrency against the *remaining* window and stage the fleet in **waves sized to survive it**, so a wave completes rather than straddling a window boundary — a generator dying mid-gate-loop is a salvage hazard (dead banking passes; a forced account swap mid-judging). Prefer `Workflow`'s `agent()` effort/model overrides for mechanical stages: a plain Agent-tool subagent inherits the session effort, so at max a blind judge thinks at max for zero protocol benefit. (General per-task advisor: #151; this is its experiment-protocol slice.)
