...
2. **Pick the lenses** — distinct, non-overlapping, matched to the change's real failure axes (mine them from `textbooks/reference/ANTI_PATTERNS.md` and the project's invariants). Default 3; 1–2 for moderate risk, 3–5 for critical / security. Neutral defaults to adapt:
   ...
   - **Lens C — test / oracle integrity + invariants:** do the tests actually *prove* the claim (could a wrong impl still pass?); **changed tests are guilty until proven** — diff the test files specifically: a loosened assertion, widened tolerance, new skip, or real-call→mock swap must trace its justification to the spec, not to making the new code pass; coverage gaps; conformance to the library (verify `Book NN §X` citations) and the project invariants; CI-vs-local reality.
