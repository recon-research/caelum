...
2. **Hunt, by tag** (the vocabulary shared with `adversarial_review`'s lens), capturing `file:line` for each:
   ...
   - **derivable** (#307) — always-loaded instruction content (root `CLAUDE.md`, `PROJECT_CONVENTIONS.md`) the repo itself can answer: directory layouts, dependency lists, command restatements, architecture overviews. Keep pitfalls, rationale, and deltas-from-default; route subsystem-scoped survivors to a path-scoped rule (`.claude/rules/` — `docs/AUTOMATION.md` §7). The criterion is the published /doctor trim heuristic (`research/notes/agent-project-systems.md`).
