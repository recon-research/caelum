...
- **Status:** ◐ **active** (D-18). ★ families COMPLETE: media, tree, input, Splitter, drag-drop; breadcrumb #332 shipped. **D-596 icons COMPLETE**; **D-595 routerLink OPEN** (#333 → `caelum/breadcrumb-router`; #165/#150 copy it); **#652 FIXED** — `caelum/grid-tanstack` split (D-652). **M3 exit ticketed** (07-23): #662–#666 in CI-core-first order + niche deferrals #667. **#662 display cluster SHIPPED** (skeleton·avatar·timeline·tag; 4 barrel-riding entry points; 3-lens review fixed 2 HIGH — tag contrast, avatar `[hidden]`). Next: #663 rating. Naming per COMPARISON: Calendar → **`cae-datepicker`**, OverlayPanel → **`cae-popover`**. Per-ticket detail → the tracker.
