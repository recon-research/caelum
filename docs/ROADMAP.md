...
- **Slices** — two ordered tracks (**D-849**):
  - **Parity (5 slices):** ☑ `cae-drawer` #709 (PR #859 — the modal semantics Material omits, D-826; carry-overs #855–#858) · ☑ `cae-alert` #710 (PR #867 — owns its live region, `role=alert`/`status` by severity; carry-overs #863–#866) · ☑ `cae-panel`/`cae-fieldset` #711 (PR #872 — two components: a `<legend>` natively names its group; carry-overs #870/#871) · ☐ `cae-menu` submenus #150 (the model is self-referential; a flat menu renders only the top level) · ☐ `cae-autocomplete` #120 — **only the `p-chips` form/CVA half closes the row**; the rest stays an `idea`.
...
- **Status:** ◐ **in progress** (opened 2026-07-28, at `0454a3d`; **3 of 5 parity slices done** at `1bf7619`, ledger **67/67**). Scope + order = **D-849**; first version `0.1.0` = **D-850**. Carried in from M4, none of it gating: #811/#812/#813/#832/#836/#840/#841, plus retro guards #844/#846.
