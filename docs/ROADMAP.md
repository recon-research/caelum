...
- **Status:** ◐ **active** (D-18). ★ families COMPLETE: media, tree, input, Splitter, drag-drop; breadcrumb #332 shipped. Working the **standing menu** — parity-extras/debt on shipped components. **D-596 icons COMPLETE** (#644/#645). **D-595 routerLink OPEN**: #333 shipped `caelum/breadcrumb-router`, the first optional-peer entry point — #165/#150 copy it; it exposed #652 (grid's optional peer is unbuildable when absent). Remaining for M3 exit: ~9 unticketed components (Calendar/Rating/Skeleton · OverlayPanel/ConfirmPopup · PanelMenu · Avatar/Timeline/Tag). Breadth follow-ups + per-ticket detail → the tracker.
