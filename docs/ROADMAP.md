...
- **Slices** — two ordered tracks (**D-849**):
  - **Parity (5 slices):** ☑ `cae-drawer` #709 (PR #859 — the modal semantics Material omits, D-826; carry-overs #855–#858) · ☑ `cae-alert` #710 (PR #867 — owns its live region, `role=alert`/`status` by severity; carry-overs #863–#866) · ☑ `cae-panel`/`cae-fieldset` #711 (PR #872 — two components: a `<legend>` natively names its group; carry-overs #870/#871) · ☑ `cae-menu` submenus **#150** (PR #876 — `cae-split-button`/`cae-menubar` inherit it; carry-over #875) · ☑ `cae-autocomplete` **#120** (PR #889 — `[multiple]` chips + `[freeText]` close the `p-chips` form/CVA half, **D-549**). #150/#120 stay OPEN for their non-parity bullets.
...
- **Status:** ◐ **in progress** (opened 2026-07-28, at `0454a3d`). **Parity track CLOSED 5/5** at `ec6654d`; **clause (1) met** — `audit_comparison.py` grades 88 mapped / **14** planned / 0 untracked, all 14 policy. **#514** at `72a348f` + **#851** at `345a120` — `0.1.0` with a gated release path, dry run green. **#852 HELD by owner direction 2026-07-30** (npm setup waits; the ticket grind continues). Ledger **68/68**; `autocomplete`'s row had its **independent 5-lens review** — verdict was **do not sign until #897 is fixed** (it found a shipped bug in #889: #897/#898/#899/#900/#901). ☑ **#897** fixed at `4c9a199` (PR #908: clear-on-pick + key-split Enter/comma; real-event regression tests, mutations 7/7) — the row is now signable, in its own PR (#860). Order = **D-849**; `0.1.0` = **D-850**. Carried from M4, non-gating: #811/#812/#813/#832/#836/#840/#841 + guards #844/#846.
