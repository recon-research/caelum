...
- **Slices** — two ordered tracks (**D-849**):
  ...
  - **Release (3 slices, ordered):** ☐ #514 rename → `@recon-research/caelum` (**D-501**) · ☐ #851 release engineering (CHANGELOG, the 0.x stability contract in published docs, tag-triggered `npm publish --provenance`, package-surface verification) · ☐ #852 publish `0.1.0` (**D-850**).
...
- **Status:** ◐ **in progress** (opened 2026-07-28, at `0454a3d`). **Parity track CLOSED 5/5** at `ec6654d`; **clause (1) met** — `audit_comparison.py` grades 88 mapped / **14** planned / 0 untracked, all 14 policy. Only the **release track** (#514 → #851 → #852) and its clause (2) remain. Ledger **68/68**, but `autocomplete`'s row predates #889 and wants re-signing — **pending independent review, not self-signed** (M4 retro). Order = **D-849**; `0.1.0` = **D-850**. Carried from M4, non-gating: #811/#812/#813/#832/#836/#840/#841 + guards #844/#846.
