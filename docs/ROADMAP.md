...
Each milestone has a **goal**, **slices**, a verifiable **exit criterion**, a **leverage** note, and **status**. Slices become issues once `origin` is live; until then they live in [`PROJECT_BACKLOG.md`](../PROJECT_BACKLOG.md).
...
- **Slices:** ☐ Angular 22 workspace (lib `caelum` + app `forge`) · ☐ Material + CDK + Angular Aria install + provenance scan green · ☐ theming token bridge (color/surface/spacing/radius/type/focus, light+dark) · ☐ ESLint `no-restricted-imports` + dependency-provenance CI gate · ☐ Direct components batch 1, usage-ranked (Input/Select/Checkbox/Radio/Button/Card/Tabs/**Dialog**/**Stepper**/**Tree**/Tooltip/Menu/Snackbar)
