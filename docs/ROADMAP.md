...
- **Slices** — two ordered tracks (**D-849**):
  - **Parity (5 slices):** ☑ `cae-drawer` #709 (PR #859 — supplies the modal semantics Material omits, D-826; review carry-overs #855–#858) · ☑ `cae-alert` #710 (PR #867 — Build-S, owns its live region: `role=alert`/`status` by severity, dismiss button *outside* it, severity name rendered visually-hidden so AT receives it at all; carry-overs #863–#866) · ☐ `cae-panel` / `cae-fieldset` #711 · ☐ `cae-menu` submenu rendering #150 (the model is already self-referential; a flat menu renders only the top level) · ☐ `cae-autocomplete` free-text + `[multiple]` #120 — **only the `p-chips` form/CVA half of that ticket closes the row**; the rest of #120 stays an `idea`.
...
- **Status:** ◐ **in progress** (opened 2026-07-28, at `0454a3d`; **2 of 5 parity slices done** at `2a91731`, ledger **66/66**). Scope + order = **D-849**; first version `0.1.0` = **D-850**. Carried in from M4, none of it gating: #811/#812/#813/#832/#836/#840/#841, plus retro guards #844/#846.
