...
- **Slices** — two ordered tracks (**D-849**):
  - **Parity (5 slices):** ☑ `cae-drawer` #709 (PR #859 — the modal semantics Material omits, D-826; carry-overs #855–#858 **all closed**, PR #939) · ☑ `cae-alert` #710 (PR #867 — owns its live region, `role=alert`/`status` by severity; carry-overs #863–#866) · ☑ `cae-panel`/`cae-fieldset` #711 (PR #872 — two components: a `<legend>` natively names its group; carry-overs #870/#871) · ☑ `cae-menu` submenus **#150** (PR #876 — `cae-split-button`/`cae-menubar` inherit it; carry-over #875) · ☑ `cae-autocomplete` **#120** (PR #889 — `[multiple]` chips + `[freeText]` close the `p-chips` form/CVA half, **D-549**). #150/#120 stay OPEN for their non-parity bullets.
