...
- **Status:** ◐ **in progress** (kicked off 2026-07-05). Grid track: **#169 ☑** provenance sign-off + **D-07 TanStack ratified** (PASS: `@tanstack/table-core@8.21.3`, MIT, US, dependency-free) · **#170 ☑** the vendor-free neutral grid interface + `cae-data-grid` + client default + Forge demo (PR #180; 4-lens-reviewed; no 3p yet — the swap to TanStack behind the identical port is the isolation proof; Forge initial 881.51 kB, warn budget 875→890 per #142; 407 tests). **Next = #171** (the **exit criterion**): admit `@tanstack/table-core` (headless core only) behind the identical `CaeGridAdapter` port as `grid.adapter.ts`, pin the ESLint fence, at-scale Forge screen, 4-lens with a leakage lens. Provisional on the grid-API decision #168 (#170 landed the recommended default). Editor (D-09 Lexical) sign-off + build come later in M2; **charts (D-14) deferred** — team not using charts yet.
