...
- **Status:** ◐ **active** (M3 exited 2026-07-24) — density parity (R4) shipped (#411); **a11y Layer 1 axe complete** (#690 + #691, PR #695: 60 components axe-clean, 0 real violations). **Hardening batch 07-24** (PRs #697–#701): #563/#419 splitter focus-ring tokens + stale jsdom comments · #642 receipt-alarm denominator (17/40→17/17) · #532 breadcrumb 2.4.8 nameless-current dev-warn · #669 tag colour-only dev-warn · #571 carousel focus re-sync · #491 single CDK node type (tree + tree-select). Remaining: theming polish · keyboard/SR + real-browser audit (#240) · visual-regression suite (#240) · capability ledger green · `p-*`→`cae-*` adoption/migration guide.
