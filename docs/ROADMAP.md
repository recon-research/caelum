...
- **Status:** ☑ **done** (2026-07-24) — exit criterion met: the long-tail parity surface is complete to the frequency cutoff — all ★ families (media, tree, input, Splitter, drag-drop) + breadcrumb + the M3-exit set #662–#666 shipped; the four niche widgets (Knob/OrgChart/MegaMenu/Dock) are explicitly deferred on-demand per **D-18** (⚠ **#706** — 13 further `p-*` have no equivalent and no ticket; this cutoff claim needs a triage pass), all four mapped in `COMPARISON.md` as *Conditional Build* (#667 = the standing list). At close: **1671 tests**. Open parity-extras (#669/#675/#677/#680/#683/#686) + **D-595 routerLink** #333 ride into M4 / on-demand. Per-slice detail → the tracker.

### M4 — Parity hardening & adoption
- **Goal:** Make "looks done" = "is done", and make the library adoptable.
- **Slices:** ☑ **density parity (R4)** — *pulled forward*, shipped #411 (PR #415; runtime `[data-density]` compact arm, Material `density:-2` + `--cae-space-*` re-scale under one selector, target-size floor held; API shape = D-19, ratified #412) · ☐ theming polish (visual polish across density × scheme) · ◐ full a11y audit — **Layer 1 axe ☑** (#690 harness + #691 sweep, PR #695: 60 components axe-clean, 0 real violations); keyboard/SR + real-browser (grid virtual rows, tooltip panel, contrast) → #240 · ☐ visual-regression suite (light+dark, incl. density arms — #240) · ☐ capability ledger green · ☐ adoption / `p-*`→`cae-*` migration guide + usage-mapping doc
...
- **Status:** ◐ **active** (M3 exited 2026-07-24) — density parity R4 (#411); **a11y Layer 1 axe complete** (#690+#691, PR #695: 60 components axe-clean, 0 real violations). **Hardening batch 07-24** (PRs #697–#705): #563/#419 · #642 · #532 · #669 · #571 · #491 · #509 — per-slice detail in the PRs. Remaining: **`p-*`→`cae-*` migration guide** (owner-chosen next; research done, gated on #706/#707) · theming polish · keyboard/SR + real-browser audit (#240) · visual-regression suite (#240) · capability ledger green.
