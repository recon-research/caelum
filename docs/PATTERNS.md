...
- **Accessible name = a projected `<caption>`** (MatTable natively projects `caption`/`colgroup`), *or* an `ariaLabel` — **not both** (an `aria-label` overrides the visible `<caption>` as the accessible name, the "label in name" caveat of the #70 naming seam); prefer the visible caption. Generic `<T extends Record<string, unknown>>` + a `cellText(row, key)` helper (nullish → `''`, not the string `"null"`) keeps template indexing type-honest.
- **Deferred, not budgeted** (#85): MatTable + MatSort + MatPaginator are heavy, so the Forge demo `@defer (on idle)`s it into its own lazy chunk. Gotcha this exposed: a `@defer`'d component still pulls its *shared* Material deps (MatPaginator needs MatSelect/MatFormField/MatButton for the page-size picker) into the eager bundle when they're shared across enough chunks — this nudged Forge's initial total over the 850 kB warn (→ provisional raise to 900 kB, `decision` #142; the 1 mb error + the shipped-lib per-entry-point budgets are the real guards).
