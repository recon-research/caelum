...
- `menuTriggerFor` uses a **two-branch template**: the trigger is applied **only when a menu is bound**, because `MatMenuTrigger` binds `aria-expanded` unconditionally — an always-present trigger would announce every plain button as a collapsed disclosure. Note the asymmetry behind the duplication: a **null menu nulls `aria-haspopup`** (`menu ? "menu" : null`) but `aria-expanded` is bound to `menuOpen` with no such guard. That asymmetry is real at the **Material** layer, but it never forced the duplication — this bullet asserted that it did until #993 measured otherwise (next bullet). **Nothing in the library two-branches any more.** Since #998 `cae-menubar` and `cae-split-button` each render one button whose `CaeMenuTrigger` carries `caeMenuTriggerDisabled`, nulls both attributes itself and refuses to open, under **D-859** (a dead trigger advertises no popup); since #992 `cae-button` renders one button behind its own `CaeButtonMenuTrigger`. It needed a *local* composer rather than reusing `caeMenuTriggerFor`, because `caelum/button` importing `caelum/menu` would pull the menu entry point into the graph of every app that uses a plain button — a bundler resolves the graph before it tree-shakes. Note which gate does **not** catch that: `check-lib-size.mjs` weighs each entry point's own FESM, and a cross-entry-point import stays an `import` there rather than being inlined, so `button`'s row would barely move while every consumer's bundle grew (§8).
- **Host-binding precedence — measured, not argued (#993).** The two-branch above rested on "there is no one-button way to drop both", which had only ever been checked at the *Material* layer; whether a **Caelum** directive composing `MatMenuTrigger` could out-bind it was never compiled. It can. Four arms, one throwaway spike, each an independent precedence question:
  - **A directive's own `host` binding beats its `hostDirectives`' binding** for the same attribute. `CaeMenuTrigger` nulling both `aria-haspopup` and `aria-expanded` puts a dead trigger on **one** element, and the live path is untouched — closed still reads `"false"`, opening still flips it to `"true"`, and the menu-family suites (124 specs) pass unchanged with the binding applied. This is the sound route: a host directive occupies a contiguous index block and the composing directive's own index sits *after* `hostDirectivesEnd` (`setDirectiveInput`), so "last write wins" resolves in the composer's favour structurally.
  - **A template `[attr.*]` binding LOSES to a directive's host binding** — host bindings run after the element's own template bindings. So a component cannot null the attribute from its own template, which is exactly `cae-button`'s position: it stamps `[matMenuTriggerFor]` directly rather than through a Caelum directive, so it needs a composing directive of its own, not a binding.
  - **A sibling directive's host binding wins only by registry order — do not use it.** It measured as working, then stopped working when `SpikeSibling` was moved earlier in the component's `imports`: absent → `"false"`, same code. Sibling matching walks `tView.directiveRegistry` in order (`findDirectiveMatches`), so a formatter, an import sorter, or a lint autofix would silently restore the wrong a11y posture with nothing failing. The cheapest-looking route is the one that rots.
  - **The discriminator cannot be the panel.** `cae-menu` stamps `<mat-menu>` unconditionally, so `getMenuPanel()` always resolves and a dead trigger's `menu` is **never** null — the ticket's proposed `trigger.menu ? … : null` would never fire at any real site. It needs an explicit deadness input. So this is one element, not one line.
  - **Control arm:** with the own host binding removed, a dead trigger really does carry `aria-expanded="false"`. The measurement is not vacuous — the attribute was there to be overridden.
- **What the collapse actually cost, and the one state it introduced (#998).** Shipping the above is not a pure deletion, and the new state is the part worth carrying forward. **A surviving element can be marked dead while its panel is open.** The two arms were separate elements, so going dead *destroyed* the open trigger and Material's teardown closed the overlay with it — free, and invisible until you remove it. One element survives its own bindings changing, so without an explicit close the panel stays open above a trigger that now reports **no `aria-expanded` at all**: a worse a11y state than the one the change set out to fix. The composing directive owes an effect that closes on going dead. Two smaller consequences: the deadness discriminator is now a real **input**, so guard `open()`/`toggle()` on it rather than leaning on Material refusing an `aria-disabled` host — that refusal reads an attribute the *consumer* supplies, so it holds only while every call site remembers `disabledInteractive` (D-859's rejected option 3 is exactly that shape); and any DI or query that used the trigger's **absence** as a deadness test now resolves on every item, so re-key it to the deadness input.
- **Deleting focus machinery is per-component, and the test that licenses it asserts survival.** Removing the branch removes the strand *only where the branch was the destruction*. At `cae-split-button` it was — the toggle sits outside any `@for` — so the witness, the `focusin`/`focusout` bookkeeping and the deferred restore all went. At `cae-menubar` it never was: groups track by object identity, a deadness change necessarily arrives as a new group object, and the `@for` re-creates that row regardless, so the identical-looking machinery **stays**. Two components, one commit, opposite dispositions. Pin the deletion with an arm asserting the element **survives** the flip as the same node — *not* with the absence of a restore, which is equally true when the element vanished and nothing put focus back, i.e. the bug the machinery existed to prevent.
- **Two-branching costs a focus strand, and it is on you to pay it (#977).** The arms are separate elements, so flipping the condition **destroys the focused control** and builds its replacement — focus falls to `<body>` (WCAG 2.4.3) even though nothing was ever disabled. The recipe: witness the focused element on `focusin` (it cannot be recovered afterwards — `activeElement` is already `<body>`), then after the swap restore focus **only if** the witness is genuinely `!isConnected` *and* `activeElement === document.body`. Both gates are load-bearing: without the first you steal focus from a user who clicked away, without the second you never restore at all. Mutation-pin them in both directions — each gate has a mutation that kills exactly one arm. The swap also invalidates any **parallel index** into the conditional directive (`QueryList<CaeMenuTrigger>` now holds only the live ones); resolve it from the *same element* via `inject(Dir, { optional: true, self: true })` instead, so the two cannot disagree. Read this as the price of a two-branch you cannot remove — **every seam in the library has now removed one instead** (#977 → #998, #992), and deleting the branch is strictly cheaper than paying it.
- **Three corrections to the recipe above, from the independent review of the D-859 slice (#989).** Each one is a way the arm you write to prove the fix can pass while grading something else.
  - **Name the destruction you are actually testing.** "The `@if` swap destroys the control" holds only where the control is not *also* inside an identity-tracked `@for`. In `cae-menubar` a deadness change necessarily arrives as a **new group object**, which re-creates the row regardless of the branch — so the swap is not independently observable there, and the strand is one the identity contract already had. `cae-split-button`'s toggle sits outside any `@for`, so there the branch swap genuinely *is* the mechanism. A fixture that replaces a tracked collection satisfies `after !== before` and `before.isConnected === false` by recreation alone, which is why the vacuity guard, though real, does not settle *which* mechanism ran.
  - **The two gates are necessary but not sufficient — clear the witness on `focusout`.** A user who deliberately blurs to the page background leaves `activeElement === document.body` *and* a live witness; when a later change destroys that element, both gates pass and you steal focus from someone who had already left. Clear the witness when focus leaves a **still-connected** element (`focusout` with `relatedTarget` outside it); the `isConnected` test is what keeps a removal-driven `focusout`, if a browser fires one, from disarming the restore.
  - **Sync roving to focus at the witness site, not in the restore.** Focus arrives by pointer too, and an `aria-disabled` control accepts it while opening nothing to mask a mismatch — so the key manager's active item silently disagrees with the DOM and the next arrow or Down acts on the wrong item. Do it in the `focusin` handler and the restore needs no bookkeeping of its own (`FocusKeyManager.setActiveItem` emits `change` *before* focusing, so the write is a no-op on the manager-driven path). Related: a re-seed guard written when roving could never rest on a disabled item becomes wrong the moment it can — it must not re-seed away from an item that currently holds focus.
  - **Do not re-key the witness by identity.** It looks like the family's answer to a stale index and is provably dead code: `@for` *moves* the view of a surviving identity and destroys only vanished ones, so the witness is disconnected exactly when `indexOf` can no longer find it. What the index needs is a **clamp** (`Math.min(index, list.length - 1)`) — an unclamped lookup returns `undefined` when the model shrank past the witness and declines the restore, stranding focus. The tell is the asymmetry: removing a middle item restores, removing the last one strands.
- **The composed seam is smaller than the first one you write — measure each guard away (#992).** `cae-button`'s composer started as a copy of `CaeMenuTrigger` and finished at *one* host binding and no class body, because three of the four guards were mutation-tested inert and deleted. The reason is a single structural difference worth naming: there the deadness discriminator is a **separate input** and `trigger.menu` is never null, whereas here the **panel is** the discriminator. So Material's own `menu ? "menu" : null` already emits the right `aria-haspopup`; `aria-controls` (`menuOpen ? menu?.panelId : null`) cannot fire on a trigger that cannot open; and `MatMenuTriggerBase`'s `_menu` setter already calls `_destroyMenu()` when set to null, which is the *only* reason this site needs no hand-written close while the menu family does. Keep the spec arm for each deleted guard: the behaviour is now the 3p's to provide, so the arm is what catches a version bump taking it away — **and that sentence is a promise you have to check, not a disclaimer**. #992's review found it false for one of the three: the `aria-controls` arm asserted only *absence on a menu-less button*, which passes hardest when the attribute never appears at all, so Material dropping the binding entirely would have gone unnoticed. Resolve the panel *through* the idref instead, and the arm becomes a live oracle. And do not read a redundant override as merely harmless — these bindings write through **per-slot** change detection, so a duplicate only writes on ticks where **its own** value changed and never restores what the 3p's later write removed. That hazard runs in **one direction**: a duplicate whose value moves in lockstep with the 3p's is merely redundant; it bites where the two expressions can diverge, i.e. copying a *panel*-discriminated block into the *deadness*-discriminated menu family, whose slot holds still while Material's moves.
- **The attribute the collapse exists to control is the one the tests forget (#992).** Three reachable states — absent (unbound), `"false"` (bound, closed), `"true"` (open) — and the middle one is both the resting state of every menu-bearing button *and* the only one the composer alone produces. Material's slot settles on `false` during the first CD while the panel is still null, then stops writing; the composer is the sole writer from then on. The suite pinned absent and `"true"` and shipped the middle one ungraded — an `aria-expanded` binding emitting only `true`-or-`null` passed 23/23. Assert all three, and assert the closed one on the bound host specifically.
- **A guard deleted as inert still needs the arm that would catch the 3p withdrawing it — and that arm must grade the artifact, not a flag.** Removing `overlayRef.detach()` from Material's own unbind branch (mutating `menu.mjs` directly) left `menuOpen === false` *and* `aria-expanded` absent, because `_setIsMenuOpen(false)` runs on every path out of `_destroyMenu`. The panel is on screen and both assertions are green. Count panels in the overlay container.
- **The `imports`-order hazard is not theoretical — it reproduces on shipped code.** #993 measured the sibling route on a throwaway spike; #992 re-ran it on the real component and the result is worse than "fragile". With the composer listed first in `imports` the sibling route **fails two arms**; with `MatMenuTrigger` listed first it passes the **entire suite, 23/23**. Same code, one array reordered. That is why the composer re-exposes the panel input under a *Caelum* name (`caeButtonMenuTriggerFor`) rather than passing `matMenuTriggerFor` through unaliased: a Material-named binding in the template is an open invitation to "tidy up" by adding `MatMenuTrigger` to `imports`, which converts the sound route into the rotting one with a green suite. Angular closes that door itself once the alias is in place — `trackHostDirectiveDef` **demotes a host directive to an ordinary sibling** if the same directive also matches the element from the template, so the alias is what keeps the demotion unreachable.
- **An internal directive needs no `export`, so keep it unexported rather than policing it.** A directive declared in the *same file* as the component listing it in `imports` compiles clean without `export` (measured: `ng build caelum` exits 0), which makes "it must not reach `public-api.ts`" structural instead of a convention plus a comment plus a test. `caelum/menubar` still carries the opposite claim and a named-export workaround built on it (#1005). Where the class genuinely must be exported — a spec imports it — say *that*, because the stated reason is what the next reader will trust.
- **Compare the arms mechanically, never by whitelist.** Duplicated markup drifts, and a parity spec that names five properties grades only what its author thought to list — it is blind to anything a later edit adds to *one* arm. Two independent reviewers landed on the same surviving mutation in `cae-split-button`'s arm (`[matButton]="variant()"` → bare `matButton`, rendering a filled chevron on an outlined button) because the spec captured the live arm's `className` and then asserted it against *itself*. Compare **sets**: attribute *names* modulo the sanctioned deltas, and the class *set* modulo the classes the 3p derives from each posture (`mat-mdc-button-disabled`, `mat-mdc-button-disabled-interactive`, `mat-mdc-menu-trigger`, `mat-ripple-loader-disabled`). Run the arm on a **non-default** variant and label, or a hardcoded literal in the dead arm matches the default and survives.
- `disabledInteractive` drops the native `disabled` attribute for `aria-disabled="true"`, keeping a *disabled* button focusable/hoverable so its forwarded tooltip still surfaces (the `<p-button pTooltip disabled>` parity case). The action is **not** auto-suppressed — the consumer guards `(click)`/`(ngSubmit)`. A bound `menuTriggerFor` is the **exception**: `MatMenuTrigger._openMenu()` returns early when the trigger host carries `aria-disabled`, which this mode sets, so the menu refuses to open by click, keyboard, or `openMenu()` and **needs no unbinding**. This corrects a claim that stood here and in `button.ts` until #978 — it asserted the opposite, and nothing pinned it in either direction because the only arms were attribute-shaped. Measured against the installed `@angular/material` 22.0.3 (`menu.mjs` `_triggerIsAriaDisabled`, `_icon-button-chunk.mjs` `_getAriaDisabled`), and note the corollary §20 states from the other side: it is choosing **native `disabled` over `aria-disabled` that disarms the 3p's guard**, and then you owe one.
