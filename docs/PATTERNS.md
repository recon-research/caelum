...
Caveat to document on every naming input: setting `ariaLabel` alongside a *visible* projected label overrides it as the accessible name (WCAG 2.5.3 "label in name" mismatch) — use one or the other. Prefer the projected label; reach for the naming seam only when there's none.
