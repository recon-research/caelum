...
- `menuTriggerFor` uses a **two-branch template**: the trigger is applied **only when a menu is bound**, because `MatMenuTrigger` binds `aria-expanded` unconditionally — an always-present trigger would announce every plain button as a collapsed disclosure. Note the asymmetry that forces the duplication: a **null menu nulls `aria-haspopup`** (`menu ? "menu" : null`) but `aria-expanded` is bound to `menuOpen` with no such guard, so there is no one-button way to drop both. `cae-menubar` and `cae-split-button` two-branch the same way under **D-859** (a dead trigger advertises no popup).
