...
- `menuTriggerFor` uses a **two-branch template**: the trigger is applied **only when a menu is bound**, because `MatMenuTrigger` binds `aria-expanded` unconditionally — an always-present trigger would announce every plain button as a collapsed disclosure. Note the asymmetry behind the duplication: a **null menu nulls `aria-haspopup`** (`menu ? "menu" : null`) but `aria-expanded` is bound to `menuOpen` with no such guard. That asymmetry is real at the **Material** layer, but it never forced the duplication — this bullet asserted that it did until #993 measured otherwise (next bullet). **`cae-menubar` and `cae-split-button` no longer two-branch at all**: since #998 each renders one button whose `CaeMenuTrigger` carries `caeMenuTriggerDisabled`, nulls both attributes itself and refuses to open, under **D-859** (a dead trigger advertises no popup). `cae-button` still two-branches, because it stamps `MatMenuTrigger` directly rather than through a Caelum directive, and a *template* binding cannot out-rank a host binding (**#992**).
