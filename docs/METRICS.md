...
**Window:** last 90 days (since 2026-04-05) · **Generated:** 2026-07-04 · **Source:** `gh` (tracker + CI), via `scripts/metrics.py`
...
|---|---|---|---|
| Throughput | 5.1/wk | trend only | Merged PRs per week. A trend line, not a target -- a sudden drop flags a blocker. |
...
*Sample this window: 65 PR(s) merged, 0 `bug`(s) filed, 82 PR CI run(s), 5 decision(s) closed. Small samples are noisy -- treat single-digit windows as directional, not controlled.*
