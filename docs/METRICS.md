...
**Window:** last 90 days (since 2026-04-03) · **Generated:** 2026-07-02 · **Source:** `gh` (tracker + CI), via `scripts/metrics.py`
...
|---|---|---|---|
| Throughput | 2.0/wk | trend only | Merged PRs per week. A trend line, not a target -- a sudden drop flags a blocker. |
...
| Decision latency | 0 d | &le; objection window · alarm &gt; 5 d | Median days a `decision` issue stays open. Measures the human-in-loop bottleneck. |
...
*Sample this window: 26 PR(s) merged, 0 `bug`(s) filed, 33 PR CI run(s), 1 decision(s) closed. Small samples are noisy -- treat single-digit windows as directional, not controlled.*
