...
|---|---|---|---|
| Throughput | 3.5/wk | trend only | Merged PRs per week. A trend line, not a target -- a sudden drop flags a blocker. |
...
*Sample this window: 45 PR(s) merged, 0 `bug`(s) filed, 56 PR CI run(s), 5 decision(s) closed. Small samples are noisy -- treat single-digit windows as directional, not controlled.*
