...
**Window:** last 90 days (since 2026-04-04) · **Generated:** 2026-07-03 · **Source:** `gh` (tracker + CI), via `scripts/metrics.py`
...
|---|---|---|---|
| Throughput | 4.4/wk | trend only | Merged PRs per week. A trend line, not a target -- a sudden drop flags a blocker. |
...
*Sample this window: 57 PR(s) merged, 0 `bug`(s) filed, 71 PR CI run(s), 5 decision(s) closed. Small samples are noisy -- treat single-digit windows as directional, not controlled.*
