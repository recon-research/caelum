...
|---|---|---|---|
| Throughput | 7.2/wk | trend only | Merged PRs per week. A trend line, not a target -- a sudden drop flags a blocker. |
...
*Sample this window: 92 PR(s) merged, 0 `bug`(s) filed, 114 PR CI run(s), 5 decision(s) closed. Small samples are noisy -- treat single-digit windows as directional, not controlled.*
