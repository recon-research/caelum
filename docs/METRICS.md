...
|---|---|---|---|
| Throughput | 0.5/wk | trend only | Merged PRs per week. A trend line, not a target -- a sudden drop flags a blocker. |
...
*Sample this window: 6 PR(s) merged, 0 `bug`(s) filed, 11 PR CI run(s), 0 decision(s) closed. Small samples are noisy -- treat single-digit windows as directional, not controlled.*
