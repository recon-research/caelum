...
|---|---|---|---|
| Throughput | 3.1/wk | trend only | Merged PRs per week. A trend line, not a target -- a sudden drop flags a blocker. |
...
*Sample this window: 40 PR(s) merged, 0 `bug`(s) filed, 50 PR CI run(s), 1 decision(s) closed. Small samples are noisy -- treat single-digit windows as directional, not controlled.*
