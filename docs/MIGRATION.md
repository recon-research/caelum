...
The default is force-selection, which is the flip from `p-autocomplete` (whose `forceSelection` defaults to `false`): a typed value that matches no suggestion is **reverted on blur**. Add **`[freeText]`** to get the upstream behaviour back — blur then commits the trimmed text, so the model may hold a suggestion key *or* an arbitrary string. This sits on two migration paths, because the `p-chips` form case routes here too (§5.9).

One deliberate deviation inside `[freeText]` ([#919](https://github.com/recon-research/caelum/issues/919)): typed text that **exactly names one enabled option's label** commits that option's **key**, exactly as picking the row would have. Upstream resolves a typed label to its key only under `forceSelection` and otherwise stores the raw string, which lets one entity reach the model as both `'us'` and `'United States'` — a mix nothing downstream can render or validate unambiguously, since a chip's text is produced by trying its value as a key first. The match is case-insensitive (matching the filter that offered the row), requires the **whole** label, and stands down when two options share a label or the match is disabled — so a genuine free tag only loses out when it is character-for-character a suggestion you supplied.
...
Enter and comma commit a tag; each renders as a removable chip. Duplicates are rejected (a repeat would throw NG0955 out of the `@for` that renders the chips), and `[options]` may still be supplied to offer suggestions alongside free entry — already-chosen options drop out of the panel, and typing a suggestion's label in full commits its key rather than the text (§5.1), so a tag cannot arrive twice wearing one name. Without `[freeText]` the field becomes multi-select-from-suggestions only. A duplicate arriving from the *form model* is de-duplicated for display but left in the model — the value shape caveats are in [§4.7](#47-value-shapes-that-changed).
