...
**Decision:** the D-10 scoping fork this scan surfaced was filed as [#21](https://github.com/recon-research/caelum/issues/21) and **ratified 2026-07-01 as D-11** (see `docs/ARCHITECTURE.md` Appendix A): scope = **shipped-runtime reachability** (Reading (B)), plus an **allied-nation fallback** — US is preferred and non-US avoided when the need can be met US-origin/self-built, but permissive-licensed deps from allied nations (UK, Germany, …) are acceptable when it cannot, and **Angular's own transitive closure is accepted wholesale**. Under D-11 this scan is **GREEN** (the three non-US flags are all either Angular-brought-over or UK/allied and permissive). The verdict below reads AMBER as recorded at scan time (pre-ratification); D-11 resolves it to green.

> **Now automated (M0-4, #4).** This hand-scan is the seed for the machine-readable [`provenance/allowlist.json`](../../provenance/allowlist.json) and the [`scripts/check_provenance.py`](../../scripts/check_provenance.py) CI/preflight gate — the tree is reconciled against the allowlist on every push, so this document is the *narrative of record*, not the enforcement mechanism. See [`provenance/README.md`](../../provenance/README.md). (The automated tree now also carries `zod` + `@standard-schema/spec`, pulled into `@angular/forms@22.0.4` since this scan — Angular's own closure, both MIT/US·DE, GREEN under D-11.)
