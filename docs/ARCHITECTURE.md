...
Every architectural decision, recorded once. `D-01..D-06` are settled by the brief + this onboarding; `D-07`/`D-09` (grid/editor libs) remain provisional pending the M2 provenance sign-off; every other row (incl. `D-11..D-15`) is decided. **Grounding** cites the intake brief (`brief §N`) for now; upgrade each to a verified `Book NN §X` once `build_library` runs.
