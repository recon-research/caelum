...
- **Preflight (every merge-blocking gate, locally, in CI order)**: `scripts/preflight.sh` / `scripts\preflight.ps1` — run before **every** push; a clean preflight means CI should be green. The scripts mirror `.github/workflows/ci.yml` by construction (change one → change the other); `configure_project` fills their TODO stages from the commands below. The CI posture (Operating posture below) paces *when CI re-runs* these gates — preflight always runs **all** of them locally, in every posture. Flags: `--quick`/`-Quick` (static gates only) · `--skip-smoke`/`-SkipSmoke`.
  > **Provisional until M0 scaffolding** (no Angular workspace exists yet — `configure_project` finalizes these once it does; see backlog CONV-1). Expected commands for an Angular 22 workspace:
- **Build**: `npm run build` (≈ `ng build caelum` for the library; `ng build forge` for the demo)
...
- **Run**: `npm start` (≈ `ng serve forge`); CI/headless smoke via `ng build` + `ng test --watch=false --browsers=ChromeHeadless`
- **Test**: `npm test` (≈ `ng test --watch=false --browsers=ChromeHeadless`) — unit + component; a11y via `axe-core`; visual/interaction via Playwright (M4)
- **Format / lint**: `npm run lint` (ESLint `angular-eslint`, warnings-as-errors) + `npx prettier --check .`; includes the `no-restricted-imports` adapter-isolation rule + the dependency-provenance gate
...
## Tracker & Hygiene
- **Issue tracker**: GitHub Issues (on the public `caelum` repo, once `origin` is live) — the live backlog; **defer = file now** (`track_followups`). *Until `origin` exists, [`PROJECT_BACKLOG.md`](PROJECT_BACKLOG.md) plays the tracker's role.*
