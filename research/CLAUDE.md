...
- **Tier every claim** from the closed vocabulary in [README](README.md) rule 2 (the audit fails on any tag outside it and prints the valid set — don't guess from memory). The tag is the audit's enforcement hook (an untagged **quantitative** claim — a `%`, an `N×` — also warns). Don't launder experimental into proven; something you can't verify is a **re-check question in Watch**, not a low tier.
