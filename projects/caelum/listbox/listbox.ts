 ...
 * controls, #47). Unlike `cae-select-button` / `cae-radio` — where each option is an INDEPENDENT
 * focusable widget, so the description is forwarded onto each — a listbox is a single COMPOSITE widget
 * (`role=listbox`); its description therefore belongs on the container, which carries the list's name
 * and description as focus roves through the options. A label-less variant is #70.
 *
...
export class CaeListbox implements ControlValueAccessor {
  ...
  /**
   * Hide the leading indicator Material draws on the selected option in SINGLE mode (its
   * `hideSingleSelectionIndicator`). No effect in `multiple` mode, which always shows checkboxes.
   */
   ...
   * (see the class docstring and `cae-radio`). Forwarded onto the `role=listbox` container, which
   * carries the list's accessible name and description as focus roves the options; pair with a
  ...
  // A null/undefined (or empty '') resets to the mode-appropriate empty. Values are taken as written;
  // an unknown value simply matches no option (the list shows nothing selected).
  writeValue(value: string | string[] | null | undefined): void {
    if (this.multiple()) {
      this.selectedValues.set(Array.isArray(value) ? value : value != null ? [value] : []);
    } else {
      this.selectedValues.set(value != null && value !== '' ? [String(value)] : []);
    }
  }
