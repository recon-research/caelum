...
export class CaeSwitch implements ControlValueAccessor {
   ...
   * whenever one is shown: setting `ariaLabel` alongside a visible label overrides it as the
   * accessible name, a WCAG 2.5.3 "label in name" mismatch — use one or the other. `ariaLabelledby`
   * wins over `ariaLabel` when both are set (WAI-ARIA precedence).
