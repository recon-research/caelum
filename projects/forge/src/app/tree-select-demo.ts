 ...
 * control end-to-end in both modes: a SINGLE-select "Move to folder" whose value is one node key, and a
 * MULTIPLE-select "Grant access to" whose value is a key array. Each `<code>` readback is driven off the
 * bound `FormControl`'s `valueChanges` (via `toSignal`) — proof the value is the node KEY (not the visible
 * label) and that selection round-trips live. Both start with a written value (proving `writeValue`).
 *
