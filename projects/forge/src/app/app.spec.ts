...
describe('App', () => {
  ...
  it('defers the below-the-fold demo sections off the initial bundle (#85)', async () => {
    ...
    await fixture.whenStable();
    const el = fixture.nativeElement as HTMLElement;
    // Exactly twenty-two @defer blocks — the capacity sliders, modules listbox, timezone autocomplete,
    // skills multi-select, members table, tree-table, carousel, galleria, image, tree-select, input-number, input-otp,
    // password, activity data-grid, orders server-grid, command bar, quick-actions context menu, workspace-sections tab menu,
    ...
    expect((await fixture.getDeferBlocks()).length).toBe(22);
