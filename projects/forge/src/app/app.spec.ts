...
describe('App', () => {
  ...
  it('defers the below-the-fold demo sections off the initial bundle (#85)', async () => {
    ...
    await fixture.whenStable();
    const el = fixture.nativeElement as HTMLElement;
    // Exactly thirty-six @defer blocks — the capacity sliders, modules listbox, timezone autocomplete,
    ...
    // password, input-mask, skeleton, avatar, timeline, cae-tag (the M3 display cluster #662), cae-rating (M3-exit #663), cae-popover + cae-confirm-popup (M3-exit #664), activity data-grid, orders server-grid, command bar, quick-actions context menu, workspace-sections tab menu,
    ...
    // MatToolbar+MatMenu/CDK-Menu/mat-tab-nav-bar/MatTree/CDK-Overlay+A11y×2/MatDialog×2/MatTabs/MatExpansion/MatChips) off
    ...
    expect((await fixture.getDeferBlocks()).length).toBe(36);
    ...
    expect(el.querySelector('.forge-splitter-card')).toBeNull();
    expect(el.querySelector('.forge-scroll-card')).toBeNull();
    expect(el.querySelector('.forge-breadcrumb-card')).toBeNull();
    expect(el.querySelector('.forge-order-list-card')).toBeNull();
    expect(el.querySelector('.forge-pick-list-card')).toBeNull();
    expect(el.querySelector('.forge-file-upload-card')).toBeNull();
    ...
    expect(el.querySelector('.forge-password-card')).toBeNull();
    expect(el.querySelector('.forge-mask-card')).toBeNull();
    expect(el.querySelector('.forge-skeleton-card')).toBeNull();
    expect(el.querySelector('.forge-avatar-card')).toBeNull();
    expect(el.querySelector('.forge-timeline-card')).toBeNull();
    expect(el.querySelector('.forge-tag-card')).toBeNull();
    ...
    expect(el.querySelector('.forge-splitter-card')).not.toBeNull();
    expect(el.querySelector('.forge-scroll-card')).not.toBeNull();
    expect(el.querySelector('.forge-breadcrumb-card')).not.toBeNull();
    expect(el.querySelector('.forge-order-list-card')).not.toBeNull();
    expect(el.querySelector('.forge-pick-list-card')).not.toBeNull();
    expect(el.querySelector('.forge-file-upload-card')).not.toBeNull();
    ...
    expect(el.querySelector('.forge-password-card')).not.toBeNull();
    expect(el.querySelector('.forge-mask-card')).not.toBeNull();
    expect(el.querySelector('.forge-skeleton-card')).not.toBeNull();
    expect(el.querySelector('.forge-avatar-card')).not.toBeNull();
    expect(el.querySelector('.forge-timeline-card')).not.toBeNull();
    expect(el.querySelector('.forge-tag-card')).not.toBeNull();
