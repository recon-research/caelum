...
describe('App', () => {
  ...
  it('defers the below-the-fold demo sections off the initial bundle (#85)', async () => {
    ...
    await fixture.whenStable();
    const el = fixture.nativeElement as HTMLElement;
    // Exactly seventeen @defer blocks — the capacity sliders, modules listbox, timezone autocomplete,
    // skills multi-select, members table, tree-table, carousel, tree-select, activity data-grid, orders
    // server-grid, command bar, quick-actions context menu, workspace-sections tab menu, structure tree,
    // reference tabs, FAQ accordion, and tag row — each carrying a heavy Material module or new CDK family
    // (MatSlider/MatList/MatAutocomplete/MatSelect+MatChips/MatTable+MatSort+MatPaginator/CDK-VirtualScroll×2/
    // MatToolbar+MatMenu/CDK-Menu/mat-tab-nav-bar/MatTree/CDK-Overlay+A11y/MatTabs/MatExpansion/MatChips) off
    ...
    expect((await fixture.getDeferBlocks()).length).toBe(17);
