...
**As of:** 2026-07-25 · main @ `17f78eb` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑** (grid gap → editor #232 / charts #233; niche widgets #667 → all on-demand per D-18) · **M4 ◐ Parity hardening & adoption** (density R4 #411 · a11y Layer 1 axe #690+#691 · real-browser harness #240 · VR suite #732) · template @ pyxis `79fdfbe`. Full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M4 active**. Shipped: density R4 #411 · a11y Layer 1 axe #690+#691 · 07-24 hardening batch (PRs #697–#705) · adoption track (#713, #716) · **real-browser harness #240** (PR #719) · **#405 sweep 3/12** (#718, #724, PRs #728/#730) · **VR suite #732** (PR #737 — `caelum:test-vr`, 4 arms light|dark × comfortable|compact, goldens in `__vr__/`) · **#736** (PR #739 — `loadCaelumTheme()` loaded HALF the bridge, so Material rendered unstyled and axe judged fallback colours). **Onboard reconcile found 3 of 4 remaining M4 exit slices UNTRACKED** — now #732 (◐) · #733 · #734. **1802 tests** (caelum 1674 + forge 71 + browser 38 + VR 4 + scripts 15); gates GREEN.

**Next:** two open tracks. **(a) #405 sweep** — remaining: tree · accordion · confirm · table(×3) · tree-table · grid(×2) · listbox; work-list heuristics = grep component prose for real-browser deferrals, and where a jsdom spec's header admits it asserts *component state* not the DOM. **(b) #732 VR** — extend past `cae-button` to a token-surface-representative set. Then #734 theming polish (blocked on #732) · #733 capability ledger. Recipes → `docs/PATTERNS.md` §9/§9a/§9b/§12. Queued extras: **#709** cae-drawer · #710 cae-alert · #711 cae-panel/fieldset · #648(opt3) · #585.
...
**Resume:** `main` @ `17f78eb`, tree clean, CI green · last shipped: **#736** full theme bridge (PR #739 — my stated mechanism was wrong: axe never reported `incomplete`, it judged the WRONG colours; the inert guard was deleted, not kept); before it **#732** VR suite (PR #737) · **no in-flight slice** · **decision queue: #735** (provisional, measured across dev/container/CI) · **NEXT: either the next #405 box or extending #732's VR arms past `cae-button`** (candidates on the ticket) · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
