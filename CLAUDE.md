...
**As of:** 2026-07-23 · main @ `ebef705` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — ★ families COMPLETE (media, tree, input, Splitter, drag-drop); **the M3 exit is ticketed** (07-23, with the owner): 5 slices **#662–#666** in CI-core-first order, niche deferrals listed in **#667**. Naming per COMPARISON: Calendar → **`cae-datepicker`**, OverlayPanel → **`cae-popover`**. Recently: **D-595 routerLink OPEN** (#333 shipped `caelum/breadcrumb-router`, first optional-peer entry point); **D-596 icons COMPLETE**; **#649** iconContext single-homed; **#652 FIXED** — `caelum/grid-tanstack` split (D-652), exports guard now derived; **#648** remains. **1487 tests**; gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Next:** build the **M3-exit set** in order — **#662** display cluster (skeleton·avatar·timeline·tag) → **#663** rating → **#664** popover + confirm-popup → **#665** panel-menu → **#666** datepicker (full parity; heaviest, lands last). Then the M3 exit-criterion check, with **#667** as the "listed if deferred" evidence. Standing-menu fallbacks if one blocks: #165/#150 routerLink (copy #654), #120 (D-549), #648, #659, #639, #655, #418 (⚠ px-unit decision), #282/#283, #264–#270, #240 (M4).
...
**Resume:** `main` @ `ebef705`, tree clean, CI green · last shipped: **M3-exit ticketing** (#662–#667, planned with the owner) · **no in-flight slice** · **decision queue EMPTY** · **NEXT: start #662** — display cluster, 4 barrel-riding entry points; `cae-tag` **composes MatChip (static)**, severity reuses `--cae-color-{success,warn,error}` (no `info` token exists yet) · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
