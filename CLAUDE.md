...
**As of:** 2026-07-25 · main @ `8efe308` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑** (grid gap → editor #232 / charts #233; niche widgets #667 → all on-demand per D-18) · **M4 ◐ Parity hardening & adoption** (density R4 #411 · a11y Layer 1 axe #690+#691 · real-browser harness #240 · VR suite ☑ #732) · template @ pyxis `79fdfbe`. Full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M4 active**. Shipped: density R4 #411 · a11y Layer 1 axe #690+#691 · 07-24 hardening batch (PRs #697–#705) · adoption track (#713, #716) · **harness #240** (PR #719) · **#405 sweep 3/12** (PRs #722/#726/#728/#730) · **VR suite ☑ #732** (PRs #737/#742; goldens agree across dev box, container and CI runner) · **#734 started** — semantic text contrast #425 (PR #746): measured, `error` was always safe and `warn` failed on *all three* light surfaces (3.65, vs the 4.24 the ticket quoted — `sunken` binds, not `base`); two primitives darkened, pinned by `theming/contrast.browser.spec.ts`. **1834 tests** (caelum 1674 + forge 71 + browser 54 + VR 20 + scripts 15); gates GREEN.

**Next:** **#405 sweep** is the open track — remaining: tree · accordion · confirm · table(×3) · tree-table · grid(×2) · listbox; work-list heuristics = grep component prose for real-browser deferrals, and where a jsdom spec's header admits it asserts *component state* not the DOM. Then **#734** theming polish (◐ started — #425 done; **#510**, **#413** remain) · **#733** capability ledger (M4 exit criterion; carries a derived-vs-curated fork that may need a `D-NN`) · **#745** VR has no status-palette arm (add `cae-tag`). Recipes → `docs/PATTERNS.md` §9/§9a/§9b/§9c/§12. Queued extras: **#709** cae-drawer · #710 cae-alert · #711 cae-panel/fieldset · #741 · #648(opt3) · #585.

**Open decisions:** **#735** — VR golden portability; provisional **Option 1** (pin the render), measured across 3 environments; window ends 2026-07-28T01:05Z. **#744** — semantic status colours: provisional **Option 2** (darken the two failing light arms, no new token API); shipped in PR #746; window ends 2026-07-28T05:22Z. Both ratify by silence → `D-NN`. Recently decided: **D-595** router = optional peerDep · **D-596** per-item icons · **D-623** guarded 3p-private reach · **D-526** tree per-node state · **D-549** tag-entry → #120 · **D-614** no commit-subject gate · **D-652** grid optional-peer split. Prior: **D-428/D-501**. **~130 open issues** = deferral log, not a burndown.

**Resume:** `main` @ `8efe308`, tree clean, CI green · last shipped: **#425 semantic text contrast** (PR #746 — a contrast test passes *hardest* when it measures nothing; the `background-color` guard was genuinely broken until mutation #3, since `transparent` reads back as opaque black) · **no in-flight slice** · **decision queue: #735, #744** (both provisional, both ratify by silence → record as `D-NN`) · **NEXT: the next #405 box, or #734's remainder (#510 residual `--mat-sys-*`, #413 compile-time density)** · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
