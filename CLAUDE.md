...
**As of:** 2026-07-09 · main @ `4227777` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand tracks per D-18) · **M3 ◐ Build-S/M long tail** · ops template synced from pyxis → `dfd6b47` (#259). Recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md); full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Where we are:** the grid is a swappable-engine adapter (client / TanStack / server) behind one neutral `CaeGridAdapter` port. **cae-table's #144 umbrella is closed**; **cae-tree-table #262** landed (PR #271 — hierarchical `role=treegrid` over MatTable). Newest: **cae-carousel #273** (PR #277) — the first ★ media-family slice: a content-agnostic rotating carousel (signal sliding-window, autoplay with WCAG 2.2.2 pause/stop + `prefers-reduced-motion`, APG carousel a11y, roving-tabindex indicators, `caeCarouselItem` projection, new `caelum/carousel` entry point + @defer'd Forge demo); its 2-lens review fixed a nav focus-strand (→ `aria-disabled`), an unreachable-slide `numScroll` gap (→ clamp), and removed an inert `responsiveOptions` false-affordance. **629 tests** (565 caelum + 64 forge); provenance GREEN.

**Next:** M3 active per **D-18**; ★ lead = **TreeSelect** (`cae-tree-select`, the treegrid's form cousin — reuses the tree node model + overlay patterns; carousel now landed). Independent work: carousel follow-ups (#274 galleria, #275 image, #276 responsive/vertical/swipe/RTL/focus-restore), cae-tree-table follow-ups (#263–#270), cae-table follow-ups (#145/#251/#235/#236/#223/#241/#242/#253/#254/#255), grid follow-ups (#193/#175/#177/#190/#174/#178/#179/#216/#228), cae-chip-set (#201/#205/#206), test-infra #240 (M4 browser runner), editor #232 / charts #233 on demand.

**Open decisions:** none — #142 → **D-16** (Forge warn-budget) and #168 → **D-17** (grid public-API shape) ratified by silence 2026-07-07; #185 → **D-18** (M2 scope: grid-only, advance to M3) decided by the human 2026-07-07. **72 open issues** = 0 decisions + 72 followups/debt/ideas.

**Resume:** `main` @ `4227777`, tree clean · **cae-carousel #273 landed** (PR #277; `caelum/carousel` entry point, @defer'd Forge demo, 629 tests; follow-ups #274/#275/#276 filed) · next action = **pick the next M3 slice** — recommended: **TreeSelect** (`cae-tree-select`, reuses the tree node model + CDK overlay patterns) or **Galleria** (#274, ★ media, builds on carousel + the overlay shell), ticket-first. a11y/API-touching slices → 2-lens review · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (629 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS 0-skipped).
