...
**As of:** 2026-07-28 · main @ `ca2170e` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑** (grid gap → editor #232 / charts #233; niche widgets #667 → all on-demand per D-18) · **M4 ◐ — exit clauses 1 ☑ · 2 ☐ · 3 ☑**; clause 2's blocker #809 is fixed, but the re-reviews it commissioned FAILED 3 components (#823/#824/#825) · template @ pyxis `79fdfbe`. Full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).
...
**Open decisions:** **#826** — cae-popover modality: it ships `role="dialog"` + a focus trap + a click-blocking backdrop but **no `aria-modal`** and no background inerting, while Book 09 §3.2 says don't trap focus in a non-modal popover. Recommended **A: add `aria-modal`, keep the trap** (one attribute; `datepicker.ts:241` precedent). **Not taken provisionally** — it changes public a11y behaviour and the right fix for #824's HIGH differs per branch. Prior D-735/744/757/771/791 live in ARCHITECTURE Appendix A. **~137 open issues** = deferral log, not a burndown.

**Resume:** `main` @ `ca2170e`, tree clean, CI green · **no in-flight slice** · last shipped: **#809** (PR #827) — ledger 64/64 → **61/64**, the honest number: `popover`/`rating`/`confirm` sign-offs **revoked** (pointer + quote kept on record; "reviewed and failed" ≠ "never reviewed") · next: **#823** (or #825) · **answer #826** to unblock #824 · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**; a Linux preflight cannot see a Windows-only break (#814).
