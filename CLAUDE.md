...
**As of:** 2026-07-25 · main @ `863b448` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑** (grid gap → editor #232 / charts #233; niche widgets #667 → all on-demand per D-18) · **M4 ◐ Parity hardening & adoption** (density R4 #411 · a11y Layer 1 axe #690+#691 · real-browser harness #240 · VR suite ☑ #732) · template @ pyxis `79fdfbe`. Full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M4 active**. Shipped: density R4 #411 · a11y Layer 1 axe #690+#691 · 07-24 hardening batch (PRs #697–#705) · adoption track (#713, #716) · **harness #240** (PR #719) · **#405 sweep 7/12** (PRs #722/#726/#728/#730/#760/#763) · **VR suite ☑ #732** (PRs #737/#742; goldens agree across dev box, container and CI runner) · **#734 ◐** — #425 semantic text contrast fixed + pinned (PR #746), #745 status palette under VR (PR #748), **#510** last 12 `--mat-sys-*` → `--cae-*` + bridge map pinned (PR #753). **1891 tests** (caelum 1674 + forge 71 + browser 107 + VR 24 + scripts 15); gates GREEN.

**Next:** **#405 sweep** is the open track — remaining: confirm · table(×2) · tree-table · grid #228 (SR half only); work-list heuristics = grep component prose for real-browser deferrals, and where a jsdom spec's header admits it asserts *component state* not the DOM. Then **#734** theming polish (◐ — #425 + #510 done; **#413** remains, and it needs a `decision`) · **#733** capability ledger (M4 exit criterion; carries a derived-vs-curated fork that may need a `D-NN`). Recipes → `docs/PATTERNS.md` §9–§9f/§12. Queued extras: **#709** cae-drawer · #710 cae-alert · #711 cae-panel/fieldset · #741 · #648(opt3) · #585.
...
**Resume:** `main` @ `863b448`, tree clean, CI green · **no in-flight slice** · last shipped: **#405** grid recycling + status mechanism (PR #763) · **decisions: #735, #744, #757** (provisional, ratify by silence 07-28 → `D-NN`) · **NEXT: the next #405 box** (confirm · table · tree-table · grid) — #413 is parked on #757 · new: **#762** (grid #194 born-empty guard defers a CD pass, not a frame — needs real-SR call), #758, #759 · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**, and re-run it **after** doc edits — a stale PASS reddened #753.
