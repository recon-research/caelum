...
**Resume:** `main` @ `a37ca60`, tree clean, CI green · **order-list header slot #341 landed** (PR #368 merged; projected `caeOrderListHeader` → listbox aria-labelledby, mirrors pick-list #358) · next action = **continue closing drag-drop extras** — remaining **order-list #341** (per-item disabled + drag handle, dataKey/trackBy, controlsPosition) / **file-upload #345** (thumbnails, chunked, concurrent, paste/folder, row-template, method), or align siblings RTL #364, or the standing menu · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (1047 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
