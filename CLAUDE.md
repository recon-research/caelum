...
**As of:** 2026-07-11 · main @ `352b013` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail**. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 input family + ★ media cluster + Splitter family + navigation set complete. **Book 11 §3.3 drag-drop cluster COMPLETE** (OrderList ☑ · PickList ☑ · FileUpload ☑). Now closing the cluster's deferred parity extras: **cae-file-upload request-config #345** landed (PR #348) — `[withCredentials]` / `[headers]` / `[params]` threaded into the upload `HttpRequest` (both idiomatic `HttpHeaders`/`HttpParams` and plain-object maps); a consumer-set `Content-Type` is **stripped** so the browser keeps the multipart boundary (2-lens security+API review converged on that footgun → guarded + tested). #345 stays open for the rest (thumbnails/chunked/concurrent/paste-folder/row-template/method). **975 tests** (911 caelum + 64 forge); all gates GREEN.

**Next:** continue **closing drag-drop parity extras** → order-list #341, pick-list #342 (shared: header/label slots, in-list filter, multi-select; describedby + dark-mode-focus items are AT/#240-gated), remaining file-upload #345. Then the standing follow-on menu (all parity-extras/debt on shipped components): breadcrumb #333, scroll-panel #329, splitter #325, image-compare #318, input-family #305/#309/#312/#315, demo heading-order #306, galleria #287/#288/#289 [#286 #240-gated], tree-select #280–#283, carousel #276/#290, tree-table #263–#270, cae-table cluster, grid cluster, chip-set #201/#205/#206, test-infra #240 (M4).
...
**Resume:** `main` @ `352b013`, tree clean, CI green · **file-upload request-config #345 landed** (PR #348 merged; #345 stays open for remaining extras) · next action = **continue closing drag-drop extras** — pick order-list #341 / pick-list #342 / remaining file-upload #345 · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (975 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
