...
**Resume:** `main` @ `1f81fe5`, tree clean, CI green · **no in-flight slice** · last shipped: **#413** (PRs #806/#814) the `caelum.theme()` mixin + its review hardening — the guard now validates `0…-5` and requires `$density-compact` strictly tighter or `null`, since the old `==` proxy missed an *inverted* arm · **M4 exit FAILED its review** → #808–#812, plus **#813** `@use` w/o `@include` · **#804** escalated (no oracle sees it; VR goldens ratified it) · findings: #796, #792, #787, #769, #766, #762, #758 · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**, re-run **after** doc edits; a Linux preflight cannot see a Windows-only break (#814).
