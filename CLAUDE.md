...
**As of:** 2026-07-19 · main @ `04e4a99` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **stepper identity reconcile #608** (PR #626 — filed #624/#627; CONFIRMED #605's repro); **size gate minifies before gzip #603** (PR #621 — 55 rows re-baselined; #586 part 1 absorbed, part 2 refuted); **chip-set focus steal #556** (PR #619). **1442 tests** (1373 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `04e4a99`, tree clean, CI green · last shipped: stepper identity reconcile #608 (PR #626) · **no in-flight slice** · **next = pick ONE:** stepper cluster #606/#607/#602 (same reconciler, decision-free), #627 guard false-positive, chip-set #553 (⚠ item 1 API fork, item 2 browser), splitter #544/#545 (⚠ jsdom-invisible) · ⚠ #611+#605 blocked on **#623**; #586 part 2 refuted · #559/#574 **M4-gated** · decision-blocked: #333 (#595/#596), #282/#283 (#526), #549, #418 · verify: `bash scripts/preflight.sh` **with node on PATH** (nvm bin; else format/build/test/lint silently skip & falsely PASS — #538).
