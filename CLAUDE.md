...
**As of:** 2026-07-17 · main @ `a2e38c3` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **tree-select per-node `disabled`** #282 (PR #528 — shared `CaeTreeNode.disabled`, all 3 modes, provisional on decision **#526**); **template sync** #518 → pyxis `79fdfbe` (PR #520); **Forge demo LIVE** (#495, PR #516) + **D-428/D-501 + CoC** (PR #515). **1322 tests** (1253 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Next:** continue the **standing menu** or start the next M3 component. Candidates: tree-select leaf-only #282 (checkbox×propagation pass) / scale #283, galleria #288 navigator-split/#289, chip-set #201 text-entry+listbox/#448, carousel #276 swipe/loop, breadcrumb #333 extras (per-item icons = cross-cutting **icon-supply** fork; routerLink [`@angular/router` peerDep fork]), splitter px #418, image-compare #422, input-family #305/#312/#315, tree-table #264–#270, hit-target sweep #456, test-infra #240 (M4).
...
**Resume:** `main` @ `a2e38c3`, tree clean, CI green · this session: onboarded (Mode B) + shipped **tree-select per-node `disabled` #282** (PR #528; +`CaeTreeNode.disabled`, provisional on decision **#526**, filed followup #527 + banner-staleness debt #525) · **next = standing menu, pick ONE:** tree-select leaf-only #282 / #283, galleria #288 navigator-split, chip-set #201, carousel #276 · verify: `bash scripts/preflight.sh` / `.\scripts\preflight.ps1` (PASS 2026-07-17, 14 stages).
