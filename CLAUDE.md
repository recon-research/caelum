...
**As of:** 2026-07-09 · main @ `0ff5d1c` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand tracks per D-18) · **M3 ◐ Build-S/M long tail** · ops template synced from pyxis → `4193b6d` (#297; `design_review` gate deferred → #298 pending #240/M4). Recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md); full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Where we are:** the grid is a swappable-engine adapter behind one neutral `CaeGridAdapter` port; **cae-table #144** / **cae-tree-table #262** landed, the ★ media family is complete (**cae-carousel #273** / **cae-galleria #274** / **cae-image #275**), and **cae-tree-select #279** shipped the library's first `cdkConnectedOverlay` (value = node key(s)). Newest: **cae-input-number #301** (PR #307) — the first M3 **input-family** member (`p-inputNumber` parity): a **`number | null`** CVA over `Intl.NumberFormat` (decimal/currency, locale-aware parse, `0`/`null`/negative fidelity, reformat + snap-to-precision on blur), built as a **component** extending `CaeFormFieldControlBase` — the deliberate house-pattern deviation from Book 08 §2.1's directive default, for `<cae-*>` API consistency (`cae-input` reserved the numeric model for it). New `caelum/input-number` entry point + @defer'd Forge demo (defer 19→20). Its 2-lens a11y/API review found **no MAJOR**; applied a runtime reformat effect (stale currency/locale display), blur snap-to-displayed-precision (WYSIWYG), a min>max fraction-digit guard, NaN→empty coerce, an overridable `inputMode`. **718 tests** (654 caelum + 64 forge); provenance GREEN. Siblings **cae-input-mask #302** / **cae-input-otp #303** / **cae-password #304** filed.

**Next:** M3 active per **D-18**. Next input-family slices: **cae-input-mask #302** / **cae-input-otp #303** / **cae-password #304** (mask=directive, otp/password=component per Book 08 §2.1); or Rating / Skeleton / Splitter; or independent **cae-image-compare #293** + `afterClosed`/DestroyRef **#294**. cae-input-number parity extras **#305** (spinner/step, percent, live-format+caret, alt-numeral); demo heading-order **#306**. Follow-ons: galleria (#286 [#240-gated] / #287 / #288 / #289), tree-select (#280–#283), carousel (#276/#290), tree-table (#263–#270), cae-table (#145/#251/#235/#236/#223/#241/#242/#253/#254/#255), grid (#193/#175/#177/#190/#174/#178/#179/#216/#228), cae-chip-set (#201/#205/#206), test-infra #240 (M4 browser runner), editor #232 / charts #233 on demand.

**Open decisions:** none — #142 → **D-16** (Forge warn-budget) and #168 → **D-17** (grid public-API shape) ratified by silence 2026-07-07; #185 → **D-18** (M2 scope: grid-only, advance to M3) decided by the human 2026-07-07. **87 open issues** = 0 decisions + 87 followups/debt/ideas (incl. #298 design_review-adopt [#240/M4-gated], input-family #302–#304, #305 input-number extras, #306 demo heading-order).

**Resume:** `main` @ `0ff5d1c`, tree clean · **cae-input-number #301 landed** (PR #307 — the M3 input family opens; siblings #302 mask / #303 otp / #304 password filed; parity extras #305, demo heading-order #306) · next action = **pick the next M3 slice** — an input-family sibling (#302–#304) or Rating/Skeleton/Splitter, ticket-first; a11y/API-touching → 2-lens review · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (718 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS 0-skipped).
