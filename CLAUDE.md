...
**As of:** 2026-07-13 · main @ `94919c2` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail**. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).
...
**Next:** continue the **standing menu** or start the next M3 component. Candidates: breadcrumb #333 extras (per-item icons = cross-cutting **icon-supply** latent fork; overflow/collapse [no-overlay invariant]; routerLink [`@angular/router` peerDep fork, shared w/ tab-menu #165]), scroll-panel #329, splitter #325 remaining (px min/max, double-click-reset, gutterSize, touch), image-compare #318 remaining (click-track-to-jump, content-projection, i18n aria-valuetext), input-family #305/#312/#315, galleria #287–#289, tree-select #280–#283, carousel #276, tree-table #263–#270, cae-table/grid clusters, chip-set #201/#205/#206, test-infra #240 (M4).

**Open decisions:** none (D-16/D-17 ratified-by-silence; D-18 human-decided 2026-07-07). **98 open issues** = 0 decisions + followups/debt/ideas/slices — a *deferral log*, not a burndown. Two latent forks, filed as `decision` when their slice is picked up: (1) `@angular/router` peerDep for breadcrumb/tab-menu `routerLink`; (2) the data-driven per-item **icon-supply** convention (`string→mat-icon` vs projected-template vs sanitized-SVG), cross-cutting breadcrumb #333 / menubar #150 / tab-menu #165 / split-button #149.

**Resume:** `main` @ `94919c2`, tree clean, CI green · next action = continue the standing menu or pick the next M3 component (see **Next**) · **dev inner loop** now: `ng test caelum --include <spec> --watch` + `npm run build:lib:fast` (the full suite + full `build:lib` stay the DoD + CI gate; #403) · **doc-budget guard now caps line width too** (#402 — journaling can't hide on one long line) · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (1166 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
