...
**As of:** 2026-07-22 · main @ `6592b4b` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — ★ families COMPLETE (media, tree, input, Splitter, drag-drop); **standing menu** is the lane, with early-theme density pulled forward from M4. Recently landed: **D-595 routerLink OPEN** — #333 (PR #654) ships `caelum/breadcrumb-router`, the first optional-peer entry point (directive over the honest `<a href>`, barrel-exempt); **D-596 icons COMPLETE** — #644 (PR #646) + #645 (PR #650). Both reviews paid: #648 (MatMenu typeahead) and **#652 — `caelum/grid`'s "optional" @tanstack peer is unbuildable when absent, measured**. **1487 tests** (1418 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `6592b4b`, tree clean, CI green · last shipped: breadcrumb routerLink #333 (PR #654) · **no in-flight slice** · **decision queue EMPTY** · **next = pick ONE:** **#652 grid optional-peer break** (HIGH, measured, wants a fix-option call), #165/#150 **routerLink** (copy #654), #120 (D-549), #639 (**owner approval**), #648, #655 · **M3 exit needs ~9 unticketed components** → `plan_milestone_tickets` **with the owner** · verify: `bash scripts/preflight.sh` **with node on PATH** (else gates falsely PASS — #538), read the **EXIT CODE**.
