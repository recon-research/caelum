...
**As of:** 2026-07-17 · main @ `2b4e6a0` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **hit-target #456** second pass (PR #540 — file-upload/password/split-button floors + non-gap audit) atop first pass PR #530; #456 open for the drag-handle slice. Prior: galleria `responsiveOptions` #288 (PR #538 — **#288 CLOSED**), `showThumbnailNavigators` #288 (PR #536), breadcrumb a11y #384/#385 (PR #533). **1345 tests** (1276 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Next:** continue the **standing menu** or start the next M3 component. Candidates: tree-select leaf-only #282 (checkbox×propagation pass) / scale #283, chip-set #201 text-entry+listbox/#448, carousel #276 swipe/loop, breadcrumb #333 extras (per-item icons = cross-cutting **icon-supply** fork; routerLink [`@angular/router` peerDep fork]), splitter px #418, image-compare #422, input-family #305/#312/#315, tree-table #264–#270, galleria lightbox zoom #289 (dep #275), hit-target #456 drag-handle hit-slop (distinct slice — invisible `::before` slop; relates #418/#419/#399), test-infra #240 (M4).
...
**Resume:** `main` @ `2b4e6a0`, tree clean, CI green · last shipped: #456 hit-target second pass (PR #540 — file-upload/password/split-button floors + non-gap audit) · **next = standing menu, pick ONE:** tree-select #282/#283 (avoid while #526 window open), chip-set #201, carousel #276 swipe/loop, #456 drag-handle hit-slop slice (relates #418/#419/#399), splitter px #418, breadcrumb #333 extras · verify: `bash scripts/preflight.sh` **with node on PATH** (nvm bin; else format/build/test/lint silently skip & falsely PASS — #538).
