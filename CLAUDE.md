...
**As of:** 2026-07-15 · main @ `8886024` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `12f3608`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **tree-select a11y** #281 (PR #443 — focus+reveal selected node on open; multi-select `LiveAnnouncer`); **scroll-panel overflow-detection** #329 (PR #441 — MutationObserver horizontal-nowrap fallback); **galleria projected item/thumb templates** #287 (PR #439); **chip-set `[emptyFocusTarget]` dev-warn** #206 (PR #437). **1215 tests** (1146 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Open decisions:** **#428** — shipped-validator convention (offered `ValidatorFn` / naming / error-key; `caeMaskComplete` is the first) — **provisional on option A, ratifies by silence 2026-07-21** (reversible; the offered fn is additive). **83 open issues** = 1 decision + followups/debt/ideas/slices — a *deferral log*, not a burndown. Two latent forks, filed as `decision` when their slice is picked up: (1) `@angular/router` peerDep for breadcrumb/tab-menu `routerLink`; (2) the data-driven per-item **icon-supply** convention (`string→mat-icon` vs projected-template vs sanitized-SVG), cross-cutting breadcrumb #333 / menubar #150 / tab-menu #165 / split-button #149.

**Resume:** `main` @ `8886024`, tree clean, CI green · landed: tree-select #281 (#443), scroll-panel #329 (#441), galleria #287 (#439), chip-set #206 (#437) · next = standing menu / next M3 (see **Next**; #276 remainder = vertical/swipe/loop/focus) · **dev inner loop**: `CI=true npx ng test caelum --filter '<test-name>'` (not `--include`; #414) + `npm run build:lib:fast` (full suite + `build:lib` = DoD + CI gate; #403) · verify: `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS, 1215) · `python3 scripts/check_provenance.py` (GREEN).
