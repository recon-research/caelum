...
**As of:** 2026-07-15 · main @ `1c3eaf5` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `5e8dbc0`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **tree-select `selectionMode='checkbox'`** tri-state parent↔child propagation #280 (PR #477 — one `classify()` canonicalize+display walk, `aria-checked` tri-state on the treeitem, 2-lens reviewed); **pyxis template sync** →`5e8dbc0` #474 (PR #475); **tree-select `[filterable]` filter** #282 (PR #472). **1288 tests** (1219 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `1c3eaf5`, tree clean, CI green · landed this session: onboarded (Mode B) + **tree-select `selectionMode='checkbox'` tri-state** #280 (PR #477 — one `classify()` canonicalize+display walk; 2-lens reviewed → writeValue-retain-unresolved-keys + filtered-uncheck-via-findNodeByKey + no-opinion-subtree roll-up + aria-multiselectable-omitted fixes; size 16896→20480) · **next = standing menu, pick ONE full window:** galleria numVisible strip-paging #288 [focus-on-page-shift MAJOR, jsdom→M4], chip-set #201-rest/#448, carousel #276, tree-select #283 (lazy/virtual/mat-form-field) · verify: `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
