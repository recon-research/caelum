...
**As of:** 2026-07-17 · main @ `f5b7df3` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **cae-breadcrumb a11y hardening** #384/#385 (PR #533 — list roles, WCAG 2.5.8 floor, nameless-crumb dev-warn); **hit-target floor sweep #456** first pass (PR #530 — `--cae-target-min` on 9 components; residual on #456, recipe → PATTERNS §10); **tree-select per-node `disabled`** #282 (PR #528, provisional on **#526**). **1327 tests** (1258 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Next:** continue the **standing menu** or start the next M3 component. Candidates: tree-select leaf-only #282 (checkbox×propagation pass) / scale #283, galleria #288 navigator-split/#289, chip-set #201 text-entry+listbox/#448, carousel #276 swipe/loop, breadcrumb #333 extras (per-item icons = cross-cutting **icon-supply** fork; routerLink [`@angular/router` peerDep fork]), splitter px #418, image-compare #422, input-family #305/#312/#315, tree-table #264–#270, hit-target residual #456 (drag-handle hit-slop / space-6 / file-upload / split-button / multi-select chip), test-infra #240 (M4).

**Open decisions:** **#526** — tree-family per-node state (on-node `CaeTreeNode` fields vs accessor-function inputs); recommended **on-node** for `p-tree` parity, **proceeding provisionally** (PR #528 carries `Provisional on #526`; reversible, silence ratifies). Prior: **D-428/D-501** decided 2026-07-16; CoC adopted; Forge Pages authorized (#495). **~98 open issues** = deferral log, not a burndown. Latent forks, filed `decision` when their slice lands: (1) `@angular/router` peerDep for breadcrumb/tab-menu `routerLink`; (2) data-driven per-item **icon-supply** convention (breadcrumb #333 / menubar #150 / tab-menu #165 / split-button #149).

**Resume:** `main` @ `f5b7df3`, tree clean, CI green · this session: onboarded (Mode B) + shipped **hit-target floor #456 first pass** (PR #530, 9 components) then **cae-breadcrumb a11y hardening #384/#385** (PR #533 — role=list/listitem, WCAG 2.5.8 floor, nameless-crumb dev-warn; 2-lens review clean, spawned #532; #405 M4 eyeball noted) · **next = standing menu, pick ONE:** #456 residual (drag-handle hit-slop / space-6 / file-upload / split-button / multi-select chip), tree-select leaf-only #282 / #283, galleria #288 navigator-split, chip-set #201, carousel #276 swipe/loop · verify: `bash scripts/preflight.sh` / `.\scripts\preflight.ps1` (PASS 2026-07-17, 14 stages).
