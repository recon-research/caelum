...
**As of:** 2026-07-29 · main @ `72a348f` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑ · M4 ☑ · M5 ◐** · template @ pyxis `79fdfbe`. Full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M5** (**D-849**) — parity first, *then* publish. **The parity track is CLOSED, 5/5:** ☑ #709 `cae-drawer` (PR #859) · ☑ #710 `cae-alert` (PR #867) · ☑ #711 `cae-panel`/`cae-fieldset` (PR #872) · ☑ #150 `cae-menu` submenus (PR #876) · ☑ **#120 `cae-autocomplete` `[multiple]`+`[freeText]`** (PR #889, **D-549**). #150/#120 stay OPEN for their non-parity bullets. **Exit clause (1) met** — `audit_comparison.py` grades 14 `☐`, all policy (#712/#667 on-demand, #232/#233 D-18 cuts). **Release track 1/3:** ☑ **#514** — the package is `@recon-research/caelum` everywhere (PR #892, **D-501**). Nobody outside this repo can install Caelum until #852.

**Next:** **#851** release engineering (CHANGELOG, the 0.x contract in published docs, tag-triggered `npm publish --provenance`) → **#852** publish `0.1.0` (**owner-run**). Also owed: `autocomplete`'s ledger row (cites PR #121, predates #889) — **needs an independent review, not a self-sign** (the M4 retro finding); ledger otherwise **68/68**. Carried, non-gating: #811/#812/#813/#832/#836/#840/#844/#846/#228 + #854–#858 (drawer) + #863–#866 (alert) + #870/#871 (panel) + #875/#877–#881 (menu; **#879 is a real `#774`-class bug in `cae-menubar`**) + **#886** (CI — now seen twice, and it *blocks* under `full`+`blocking`).

**Open decisions:** **#841** — where focus goes when a popover's trigger is removed while the panel is open. Non-blocking (rare path; an `isConnected` guard was tried and DELETED as provably inert). Standing: **D-849** (parity before publish) · **D-850** (first release is `0.1.0` — so breaking changes stay *minor* bumps until a deliberate 1.0.0; grounded in D-831 breaking a shipped signature during M4's exit week). The **open tracker is the deferral log** (~160; count it with `gh issue list`, don't cache it here).

**Resume:** `main` @ `72a348f`, tree clean · **M5: parity closed, release 1/3, no in-flight slice** · next: take **#851** (release engineering) — then #852 publishes; claim + branch per `ship_pr` step 0 · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**; a Linux preflight cannot see a Windows-only break (#814). **A ledger row ships in its own post-merge PR** (#860). Lessons: [`docs/PATTERNS.md`](docs/PATTERNS.md) §14/§15. **A timeout-shaped windows-latest red is #886** (see the ticket for the diagnosis + the log-reading trap), not your slice.
