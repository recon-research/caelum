...
**As of:** 2026-07-18 · main @ `710a0a7` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **galleria #572 + #573** arrows anchor on the active item and both components wrap under `[circular]` (PR #582 — filed #580/#581/#583); **carousel #560** (PR #575 — filed #571–#574); **status-anchor guard #525** (PR #568). **1393 tests** (1324 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `710a0a7`, tree clean, CI green · last shipped: galleria/carousel #572+#573 (PR #582) · **no in-flight slice** · **next = standing menu, pick ONE:** breadcrumb #333 extras, splitter slop #544/#545, chip-set #552/#553/#556, modifier-chord sweep #581 (Alt+Arrow breaks browser Back, ~8 components), NaN-index tab-order #580 · #559/#574 are **M4-gated** (jsdom can't model `inert` blur or SR output) · ⚠ decision-blocked: tree-select #282/#283 (#526), chip-set listbox (#549), splitter px #418 · verify: `bash scripts/preflight.sh` **with node on PATH** (nvm bin; else format/build/test/lint silently skip & falsely PASS — #538).
