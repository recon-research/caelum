...
**As of:** 2026-07-30 · main @ `65974a1` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑ · M4 ☑ · M5 ◐** · template @ pyxis `79fdfbe`. Full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M5** (**D-849**) — parity first, *then* publish. **Parity track CLOSED 5/5** (#709/#710/#711/#150/#120; #150/#120 stay OPEN for their non-parity bullets) — **exit clause (1) met**, `audit_comparison.py` grades 14 `☐`, all policy. **Release track 2/3:** ☑ **#514** the package is `@recon-research/caelum` (PR #892, **D-501**) · ☑ **#851** a gated release path (PR #894) — `CHANGELOG.md` @ `0.1.0`, the 0.x contract in the *packaged* readme, `release.yml`, `check-package-surface.mjs`; dry run green. **The 5-lens independent review of #889 found a shipped bug** → #897/#898 bugs · #899 a11y · #900 test debt · #901 small.

**Next:** **#897** (`[multiple]` commits stale text as a second chip — mechanism in the ticket) → **#898**. **#852 HELD by owner direction** 2026-07-30 — not blocked by us; npm setup waits. Also owed: `autocomplete`'s ledger row — the independent review is now **done**, and its verdict is **do not sign until #897 is fixed**; ledger otherwise **68/68**. Carried, non-gating: #896 (OIDC publishing) + #811/#812/#813/#832/#836/#840/#844/#846/#228 + #854–#858 (drawer) + #863–#866 (alert) + #870/#871 (panel) + #875/#877–#881 (menu; **#879 is a real `#774`-class bug in `cae-menubar`**) + **#886** (CI — now seen twice, and it *blocks* under `full`+`blocking`).
