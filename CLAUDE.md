...
**As of:** 2026-07-24 · main @ `d2e75bf` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑** (grid gap → editor #232 / charts #233; niche widgets #667 → all on-demand per D-18) · **M4 ◐ Parity hardening & adoption** (density R4 #411 · a11y Layer 1 axe #690+#691 · real-browser harness #240 complete) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M4 active** (parity hardening & adoption). Shipped: density R4 #411 · a11y Layer 1 axe #690+#691 · 07-24 hardening batch (PRs #697–#705) · adoption track (#713, #716) · **real-browser harness #240** (PR #719 — Vitest browser mode + Playwright, dev-tier; `*.browser.spec.ts` split from the jsdom target). **#405 sweep OPEN, 2/12 boxes:** ☑ #718 grid `role=table` off the visual frame · ☑ #724 browser tests load the real token layer · ☑ divider #91 + toolbar #128 layout measured (PR #728). **1781 tests** (caelum 1674 + forge 71 + browser 26 + scripts 10); gates GREEN.

**Next:** **work the #405 sweep** — remaining boxes: tree · accordion · confirm · slider · table(×3) · tree-table · grid(×2) · listbox. Work-list heuristic (PR #728): grep components' own prose for real-browser deferrals — galleria/listbox/table still carry one, and it is richer than the checklist, which had missed listbox entirely. Recipes → `docs/PATTERNS.md` §9/§12. Then: theming polish (density × scheme) · visual-regression suite · capability ledger green. Queued extras: **#709** cae-drawer · #710 cae-alert · #711 cae-panel/fieldset · #648(opt3) · #585.
...
**Resume:** `main` @ `d2e75bf`, tree clean, CI green · last shipped: **#405** divider #91 + toolbar #128 layout (PR #728) — a browser spec needs an in-file arm that neutralizes its own mechanism, or the geometry goes vacuous (`docs/PATTERNS.md` §9); before it **#724** (PR #726), **#718** (PR #722) · **no in-flight slice** · **decision queue EMPTY** · **NEXT: the next #405 box** — galleria/listbox/table carry unverified prose claims · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
