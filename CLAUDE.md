...
**As of:** 2026-07-18 · main @ `13c2470` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **modifier-chord sweep #581** — 10 handlers across 9 components no longer eat Alt+Arrow / Ctrl+Home/End / Ctrl+0 (PR #587 — filed #585/#586); **galleria #572 + #573** (PR #582 — filed #580/#583); **carousel #560** (PR #575). **1403 tests** (1334 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Open decisions:** **#549** — chip-set text-entry shape (`[textEntry]` mode vs a `cae-chips` CVA control); recommended **A** (stepping stone), PR #554 provisional. **#526** — tree per-node state (on-node vs accessors); recommended **on-node**, PR #528 provisional. Both reversible; silence ratifies. Prior: **D-428/D-501** 2026-07-16; CoC adopted; Forge Pages (#495). **~115 open issues** = deferral log, not a burndown. Latent forks, filed when their slice lands: (1) `@angular/router` peerDep (breadcrumb/tab-menu `routerLink`); (2) per-item **icon-supply** convention (#333/#150/#165/#149).

**Resume:** `main` @ `13c2470`, tree clean, CI green · last shipped: modifier-chord sweep #581 (PR #587) · **no in-flight slice** · **next = standing menu, pick ONE:** breadcrumb #333 extras, splitter slop #544/#545, chip-set #552/#553/#556, NaN-index tab-order #580 (zero tab stops, WCAG 2.1.1), tree-select combobox #585, size-budget drift #586 · #559/#574 are **M4-gated** (jsdom can't model `inert` blur or SR output) · ⚠ decision-blocked: tree-select #282/#283 (#526), chip-set listbox (#549), splitter px #418 · verify: `bash scripts/preflight.sh` **with node on PATH** (nvm bin; else format/build/test/lint silently skip & falsely PASS — #538).
