...
**As of:** 2026-07-11 · main @ `766931b` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail**. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 input family + ★ media cluster + Splitter family + navigation set complete. **Book 11 §3.3 drag-drop cluster: OrderList ☑ · PickList ☑ · FileUpload ☐.** Just landed: **cae-pick-list #337** (PR #343) — two connected keyboard-operable `role=listbox` lists (`p-pickList` parity) over `cdkDropList` + `cdkDropListConnectedTo`: a control column between them (move-selected/move-all, both directions) makes transfer fully keyboard-reachable, each list has its own roving tabindex, every transfer announced via `LiveAnnouncer`; content-agnostic `caePickListItem` template, `[(source)]`/`[(target)]` models + `(transfer)`; no foreign drag lib. **v1 transfer-only** — within-list drag reorder disabled (`cdkDropListSortingDisabled`) so §3.5's keyboard-path-for-every-drag holds; reorder-within deferred. Reused order-list's `aria-disabled`-at-bounds pattern (see [[aria-disabled-at-bounds]]). 2-lens review (a11y + API): **no MAJOR**; applied 2 low-sev fixes (distinct-refs doc caveat, mirror drop test). **946 tests** (882 caelum + 64 forge); all gates GREEN. Deferred → **#342**.

**Next:** finish the cluster — **FileUpload #338** (independent; `cdkDropList` dropzone + native `<input type=file>` + `HttpClient` upload with progress, trust-boundary type/size validation, optional CVA). Standing follow-ons: pick-list #342, order-list #341, breadcrumb #333, scroll-panel #329, splitter #325, image-compare #318, input-family #305/#309/#312/#315, demo heading-order #306, galleria #286[#240-gated]/#287/#288/#289, tree-select #280–#283, carousel #276/#290, tree-table #263–#270, cae-table cluster, grid cluster, chip-set #201/#205/#206, test-infra #240 (M4).

**Open decisions:** none (D-16/D-17 ratified-by-silence, D-18 human-decided 2026-07-07). **92 open issues** = 0 decisions + 92 followups/debt/ideas/slices.

**Resume:** `main` @ `766931b`, tree clean, CI green · **cae-pick-list #337 landed** (PR #343 merged; deferrals → #342) · next action = **build FileUpload #338** (dropzone + `HttpClient` upload; validate type/size at the trust boundary *before* upload; keep the drop target keyboard-reachable via the native file input) — or a standing follow-on · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (946 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
