...
**As of:** 2026-07-10 · main @ `20d35a8` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail**. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 input family + the ★ media cluster complete; **the Splitter family (splitter #323 / scroll-panel #328) is now COMPLETE** (Book 11 §3.2 both gaps closed). Just landed: **cae-scroll-panel #328** (PR #330) — a token-styled cross-browser scroll container over native `overflow` + `CdkScrollable`, keyboard-focusable `role=region` only while content overflows (native `ResizeObserver`, no custom-scrollbar engine). 2-lens review fixed one a11y MAJOR (focus-to-`<body>` when overflow cleared on a focused container → `tabindex=-1` while focused). **899 tests** (835 caelum + 64 forge); all gates GREEN. Deferred → **#329**.

**Next:** M3 continues per **D-18** — pick the next slice (natural candidates: **cae-breadcrumb** Build-S, or the Book 11 §3.3 drag-drop cluster **OrderList/PickList/FileUpload** Build-M; both need `plan_work` + ticket). Standing follow-ons: scroll-panel #329, splitter #325, image-compare #318, input-family #305/#309/#312/#315, demo heading-order #306, galleria #286[#240-gated]/#287/#288/#289, tree-select #280–#283, carousel #276/#290, tree-table #263–#270, cae-table cluster, grid cluster, chip-set #201/#205/#206, test-infra #240 (M4).

**Open decisions:** none (D-16/D-17 ratified-by-silence, D-18 human-decided 2026-07-07). **88 open issues** = 0 decisions + 88 followups/debt/ideas/slices.

**Resume:** `main` @ `20d35a8`, tree clean, CI green · **cae-scroll-panel #328 landed** (PR #330 merged; deferred → #329) · next action = **pick the next M3 slice** — `plan_work` on **cae-breadcrumb** (Build-S) or the drag-drop cluster, then build; or take any standing follow-on · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (899 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
