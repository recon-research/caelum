...
**As of:** 2026-07-15 · main @ `705f08d` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `12f3608`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **chip-set per-item `[chipRemovable]`/`[chipDisabled]`** #201 (PR #459 — locked/disabled chips + disabled-only focus-redirect fix); **galleria `[showIndicators]`** #288 (PR #457 — opt-in dot nav + `--cae-target-min` hit-target floor); **image-compare RTL Page keys** #396 (PR #454 — invert horizontal PageUp/PageDown under RTL). **1248 tests** (1179 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Next:** continue the **standing menu** or start the next M3 component. Candidates: breadcrumb #333 extras (per-item icons = cross-cutting **icon-supply** latent fork; overflow/collapse [no-overlay invariant]; routerLink [`@angular/router` peerDep fork, shared w/ tab-menu #165]), splitter px min/max #418, image-compare content-projection #422, input-family #305/#312/#315, galleria #288-rest/#289, tree-select #280/#282/#283, carousel #276, tree-table #264–#270, cae-table/grid clusters, chip-set #201/#448, hit-target sweep #456, test-infra #240 (M4).

**Open decisions:** **#428** — shipped-validator convention (offered `ValidatorFn` / naming / error-key; `caeMaskComplete` is the first) — **provisional on option A, ratifies by silence 2026-07-21** (reversible; the offered fn is additive). **86 open issues** = 1 decision + followups/debt/ideas/slices — a *deferral log*, not a burndown. Two latent forks, filed as `decision` when their slice is picked up: (1) `@angular/router` peerDep for breadcrumb/tab-menu `routerLink`; (2) the data-driven per-item **icon-supply** convention (`string→mat-icon` vs projected-template vs sanitized-SVG), cross-cutting breadcrumb #333 / menubar #150 / tab-menu #165 / split-button #149.

**Resume:** `main` @ `705f08d`, tree clean, CI green · landed: chip-set per-item #201 (#459), galleria `[showIndicators]` #288 (#457), image-compare RTL Page #396 (#454) · next = standing menu / next M3 (see **Next**; galleria #288 rest (caption/numVisible/nav-split/fullScreen/vertical-RTL) + #289 zoom; tree-select #282 filter/per-node-disabled, #280/#283; carousel #276 + #445; chip-set #201-rest (text-entry field + selectable listbox)/#448) · new control → floor hit target w/ `--cae-target-min` (#456) · **dev inner loop**: `CI=true npx ng test caelum --filter '<test-name>'` (#414) + `npm run build:lib:fast` · verify: `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
