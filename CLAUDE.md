...
**As of:** 2026-07-23 · main @ `89f2560` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — ★ families COMPLETE (media, tree, input, Splitter, drag-drop). **M3-exit set ticketed** (07-23): **#662–#666** CI-core-first + niche deferrals **#667**. **#662 display cluster SHIPPED** (PR #670): 4 barrel-riding entry points `caelum/{skeleton,avatar,timeline,tag}` (tag composes MatChip static; D-596 icons); 3-lens review fixed 2 HIGH (tag WCAG contrast, avatar `[hidden]`), deferred **#669**. Naming per COMPARISON: Calendar → **`cae-datepicker`**, OverlayPanel → **`cae-popover`**. Prior: **D-595/D-596/D-652**; **#648** remains. **1526 tests**; gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Next:** continue the **M3-exit set** in order — ✓#662 display cluster → **#663** rating (selection-control CVA, #47 family; star hit-target floors at `--cae-target-min`, NOT `--cae-space-*`) → **#664** popover + confirm-popup (one confirm service, Book 09 §3.3) → **#665** panel-menu (compose CaeAccordion) → **#666** datepicker (full parity; heaviest, lands last). Then the M3 exit-criterion check, with **#667** as the "listed if deferred" evidence. Fallbacks if one blocks: #165/#150 routerLink (copy #654), #120 (D-549), #648, #659, #669, #639, #655, #418 (⚠ px-unit decision), #240 (M4).
...
**Resume:** `main` @ `89f2560`, tree clean, CI green (full matrix) · last shipped: **#662 display cluster** (PR #670, `usd=32.91`) · **no in-flight slice** · **decision queue EMPTY** · **NEXT: start #663 cae-rating** — star rating as a selection-control CVA (#47 family, NG_VALUE_ACCESSOR; mirror cae-radio/select-button, NOT the mat-form-field base); role=radiogroup/radio APG pattern; each star hit-target floors at `--cae-target-min` (24px, WCAG 2.5.8), never `--cae-space-*`; D-596 `[icon]`/`[offIcon]`; value `number|null` with `[allowCancel]`→null · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
