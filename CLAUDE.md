...
**Open decisions:** **#501** npm name (`caelum` TAKEN; rec. scoped `@recon-research/caelum`; ratifies 07-18) · **#428** validator convention (provisional A, ratifies 07-21). **Owner queue:** CoC yes/no (#494 note). **97 open issues** = 1 decision + followups/debt/ideas/slices — a *deferral log*, not a burndown. Two latent forks, filed as `decision` when their slice is picked up: (1) `@angular/router` peerDep for breadcrumb/tab-menu `routerLink`; (2) the data-driven per-item **icon-supply** convention (`string→mat-icon` vs projected-template vs sanitized-SVG), cross-cutting breadcrumb #333 / menubar #150 / tab-menu #165 / split-button #149.
