...
**As of:** 2026-07-18 · main @ `7d87ac7` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **stepper index clamp #592** — a consumer NaN/negative/out-of-range `[selectedIndex]` *threw out of CDK's setter*, crashing change detection (PR #599 — filed #597/#598); **NaN-index guard #580** — zero tab stops on carousel+galleria, incl. the poisoned clamp *bound* (PR #593); **modifier-chord sweep #581** (PR #587). **1420 tests** (1351 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Open decisions:** **#595** router peerDep (blocks breadcrumb #333 / tab-menu #165 `routerLink`) — recommended **A** optional peerDep + secondary entry point; **#596** per-item icon-supply convention (#333/#165/#150/#149) — recommended **A+B** built-in glyph then template. Both **hard-to-reverse ⇒ parked, not provisional**. **#549** chip-set text-entry (rec. **A**, PR #554 provisional); **#526** tree per-node state (rec. **on-node**, PR #528 provisional). All four carry a **7-day objection window**. Prior: **D-428/D-501** 2026-07-16; CoC adopted; Forge Pages (#495). **~120 open issues** = deferral log, not a burndown. The two latent forks are now FILED (#595/#596), so that list is empty.

**Resume:** `main` @ `7d87ac7`, tree clean, CI green · last shipped: stepper index clamp #592 (PR #599) · **no in-flight slice** · **next = standing menu, pick ONE:** splitter slop #544/#545, chip-set #552/#553/#556, stepper residuals #597/#598, size-budget drift #586, commit-keyword guard #591 · #559/#574 **M4-gated** (jsdom can't model `inert` blur or SR output) · ⚠ decision-blocked: breadcrumb #333 **fully parked** (#595/#596), tree-select #282/#283 (#526), chip-set listbox (#549), splitter px #418 · verify: `bash scripts/preflight.sh` **with node on PATH** (nvm bin; else format/build/test/lint silently skip & falsely PASS — #538).
