...
**As of:** 2026-07-24 · main @ `c508d71` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑** (grid gap → editor #232 / charts #233; niche widgets #667 → all on-demand per D-18) · **M4 ◐ Parity hardening & adoption** (density R4 #411 · a11y Layer 1 axe #690+#691 · real-browser harness #240 complete) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M4 active** (parity hardening & adoption). Shipped: density R4 (#411); a11y Layer 1 axe (#690+#691); the 07-24 hardening batch (PRs #697–#705); the adoption track (PRs #713, #716). **Real-browser harness LIVE (07-24, #240, PR #719):** Vitest browser mode + Playwright (dev-tier), Chromium-preferred auto-select, `*.browser.spec.ts` split from the jsdom target — per-slice detail in the PRs + ROADMAP M4. **1760 tests** (caelum 1673 + forge 71 + browser 6 + scripts 10); gates GREEN.

**Next:** **work the #405 real-browser sweep** now the harness (#240) unblocks it — a consolidated per-component checklist (the individual verify followups were rolled into it at the 07-13 triage; 5 stayed independent: #138 #155 #158 #165 #236). **#718** (grid `role=status` in `role=table`, axe critical) is the first concrete find, from the harness itself. Then: theming polish (density × scheme) · visual-regression suite · capability ledger green. Queued extras: **#709** cae-drawer · #710 cae-alert · #711 cae-panel/fieldset · #648(opt3) · #585.
...
**Resume:** `main` @ `c508d71`, tree clean, CI green · last shipped: **#240** real-browser harness (PR #719, `usd=18.61`) — `npm run test:browser`; a pin is **strict** (missing pinned browser FAILS, never falls back), preflight SKIPs loudly with no browser but FAILS a broken pin (#538's shape). Its first spec found 2 jsdom-invisible a11y defects: keyboard-unreachable scroll viewport (**fixed**) and **#718** — whose rule the jsdom carve-out had masked with a wrong reason · **no in-flight slice** · **decision queue EMPTY** · **NEXT: work the #405 real-browser sweep** (now unblocked; #718 first) · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
