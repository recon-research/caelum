...
**As of:** 2026-07-22 · main @ `8c9fd55` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — ★ families COMPLETE (media, tree, input, Splitter, drag-drop); **standing menu** is the lane, early-theme density pulled forward from M4. Recently: **D-595 routerLink OPEN** — #333 (PR #654) ships `caelum/breadcrumb-router`, the first optional-peer entry point (directive over the honest `<a href>`, barrel-exempt); **D-596 icons COMPLETE** (#644/#645). Review debt: **#649** single-homed the iconContext (PR #657); **#648** (MatMenu typeahead) + **#652** (`caelum/grid`'s optional @tanstack unbuildable when absent, measured) remain. **1487 tests**; gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
