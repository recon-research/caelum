...
**As of:** 2026-07-15 · main @ `b4f474d` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `12f3608`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **galleria `[captionPosition]`** #288 (PR #461 — below|overlay caption + WCAG-safe scrim); **chip-set per-item `[chipRemovable]`/`[chipDisabled]`** #201 (PR #459 — locked/disabled + disabled-only focus-redirect); **galleria `[showIndicators]`** #288 (PR #457 — opt-in dot nav + `--cae-target-min` floor). **1251 tests** (1182 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `b4f474d`, tree clean, CI green · landed: galleria `[captionPosition]` #288 (#461), chip-set per-item #201 (#459), galleria `[showIndicators]` #288 (#457) · next = standing menu / next M3 (see **Next**; galleria #288 rest (numVisible/nav-split/fullScreen/vertical-RTL) + #289 zoom; tree-select #282 filter/per-node-disabled, #280/#283; carousel #276 + #445; chip-set #201-rest (text-entry field + selectable listbox)/#448) · new control → floor hit target w/ `--cae-target-min` (#456) · **dev inner loop**: `CI=true npx ng test caelum --filter '<test-name>'` (#414) + `npm run build:lib:fast` · verify: `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
