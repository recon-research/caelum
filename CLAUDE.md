...
**As of:** 2026-07-29 · main @ `7a8e5e7` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑ · M4 ☑ · M5 ◐** · template @ pyxis `79fdfbe`. Full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).
...
**Next:** M5 parity: **#120** (*only* the `p-chips` form/CVA half closes the row) — the last one. Then release: **#514** rename → **#851** release engineering → **#852** publish `0.1.0` (**owner-run**). Carried, non-gating: #811/#812/#813/#832/#836/#840/#844/#846/#228 + #854–#858 (drawer) + #863–#866 (alert) + #870/#871 (panel) + #875/#877–#881 (menu; **#879 is a real `#774`-class bug in `cae-menubar`**).

**Open decisions:** **#841** — where focus goes when a popover's trigger is removed while the panel is open. Non-blocking (rare path; an `isConnected` guard was tried and DELETED as provably inert). Standing: **D-849** (parity before publish) · **D-850** (first release is `0.1.0` — so breaking changes stay *minor* bumps until a deliberate 1.0.0; grounded in D-831 breaking a shipped signature during M4's exit week). **159 open issues** = deferral log.

**Resume:** `main` @ `7a8e5e7`, tree clean · **M5 open, no in-flight slice** · next: take **#120** (`p-chips` form/CVA half) — the last parity slice; claim + branch per `ship_pr` step 0 · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE** (a wrapper's exit can mask it); a Linux preflight cannot see a Windows-only break (#814). **A ledger row ships in its own post-merge PR** (#860). Two #150 lessons now in [`docs/PATTERNS.md`](docs/PATTERNS.md): a content query and a row's DI both key off a template's **declaration** site, and prose in a `template:` literal **ships**.
