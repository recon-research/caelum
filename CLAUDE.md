...
**As of:** 2026-07-14 · main @ `29fc1ab` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `12f3608`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **splitter parity extras** #325 (PR #420 — `[gutterSize]` + double-click reset + touch hit-slop; px min/max split to #418), **density parity (R4)** #411 (PR #415 — runtime `[data-density]` compact arm, target-size floor held), **template sync** pyxis→`12f3608` (#407). **1174 tests** (1105 caelum + 69 forge); all gates GREEN. Per-slice detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Next:** continue the **standing menu** or start the next M3 component. Candidates: breadcrumb #333 extras (per-item icons = cross-cutting **icon-supply** latent fork; overflow/collapse [no-overlay invariant]; routerLink [`@angular/router` peerDep fork, shared w/ tab-menu #165]), scroll-panel #329, splitter px min/max #418, image-compare #318 remaining (click-to-jump, projection, i18n), input-family #305/#312/#315, galleria #287–#289, tree-select #280–#283, carousel #276, tree-table #264–#270, cae-table/grid clusters, chip-set #201/#205/#206, test-infra #240 (M4).
...
**Resume:** `main` @ `29fc1ab`, tree clean, CI green · **splitter extras #325 shipped (PR #420)**; density #411 + D-19 landed · next action = continue the standing menu or pick the next M3 component (see **Next**) · **dev inner loop**: `CI=true npx ng test caelum --filter '<test-name>'` (not `--include`; #414) + `npm run build:lib:fast` (full suite + `build:lib` stay the DoD + CI gate; #403) · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (1174 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
