...
**As of:** 2026-07-08 · main @ `d10eeab` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand tracks per D-18) · **M3 ◐ Build-S/M long tail** · ops template synced from pyxis → `b263b94` (#245). Recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md); full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Where we are:** the grid is a swappable-engine adapter (client default / TanStack / server) behind one neutral `CaeGridAdapter` port, consumer-tree-shake-guarded (#182), extended with loading/busy a11y, pager focus management, a rows-per-page menu, server total-shrink recovery, and first-load live-region announcement (#176/#188/#189/#192/#177/#190/#194). **cae-table's #144 umbrella is closed** — custom body-cell templates (#143), absolute row-index (#213), multi- + single-select (#144), opt-in click/Space-to-deselect (#224), expandable detail rows (#144, PR #243), and sticky header + sticky columns (#144/#252, PR #256; the 2-lens review caught a WCAG 2.4.11 defect — a frozen leading column occluded the still-focusable built-in select/expand controls). Also cae-chip-set (#84/#202). **570 tests** (510 caelum + 60 forge); provenance GREEN.

**Next:** M3 (build-S/M long tail) is active per **D-18**; team-priority slices (★ media Carousel/Galleria, TreeSelect/TreeTable) lead. Immediate independent work: cae-table follow-ups (#145, #251 sticky footer [needs a footer content model], #235, #236, #223, #241 collapse-focus, #242 detail-row scale, #253 p-table frozen-API naming, #254 M4 focus-occlusion, #255 pager-outside-scroller), grid follow-ups (#193/#175/#177/#190 parts 2–3/#174/#178/#179/#216/#228), cae-chip-set (#201/#205/#206), test-infra #240 (M4 browser runner: prefer Chromium, fall back to Firefox), editor #232 / charts #233 on demand.

**Open decisions:** none — #142 → **D-16** (Forge warn-budget) and #168 → **D-17** (grid public-API shape) ratified by silence 2026-07-07; #185 → **D-18** (M2 scope: grid-only, advance to M3) decided by the human 2026-07-07. **62 open issues** = 0 decisions + 62 followups/debt/ideas.

**Resume:** `main` @ `d10eeab`, tree clean · next action (human, 2026-07-08) = **run [`update_from_template`](.claude/skills/update_from_template/SKILL.md) from pyxis** — sync forward from `TEMPLATE_VERSION` sha `b263b94`; **then** pick the next slice — recommended: open the **M3 ★ team-priority track** (TreeSelect / Carousel, ticket-first) now that cae-table #144 is closed; or drain #251 (sticky footer — it forces a footer content model). a11y/API-touching slices → 2-lens review · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (570 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS 0-skipped).
...
- **A permission denial is a decision point, not an obstacle.** When the harness or its auto-mode classifier denies a tool call mid-run: never retry verbatim, never work around it. If the block is the human's to clear, surface it as a **structured question** (AskUserQuestion) with ready-made choices — *approve and retry* · *add this standing rule* (the exact settings snippet) · *here's the command to run yourself* — then continue on work that doesn't depend on it. Denials land in the local metrics ledger (`docs/AUTOMATION.md` §1) and feed the retrospective loop.
