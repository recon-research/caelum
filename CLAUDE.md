...
**As of:** 2026-07-25 · main @ `1a67cbc` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑** (grid gap → editor #232 / charts #233; niche widgets #667 → all on-demand per D-18) · **M4 ◐ Parity hardening & adoption** (density R4 #411 · a11y Layer 1 axe #690+#691 · real-browser harness #240 · VR suite #732) · template @ pyxis `79fdfbe`. Full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M4 active**. Shipped: density R4 #411 · a11y Layer 1 axe #690+#691 · 07-24 hardening batch (PRs #697–#705) · adoption track (#713, #716) · **real-browser harness #240** (PR #719) · **#405 sweep 3/12** (#718, #724, PRs #728/#730) · **VR suite #732** (PR #737 — `caelum:test-vr` target, 4 arms light|dark × comfortable|compact, goldens in `__vr__/`). **Onboard reconcile found 3 of 4 remaining M4 exit slices UNTRACKED** — now #732 (◐) · #733 capability ledger · #734 theming polish. **1798 tests** (caelum 1674 + forge 71 + browser 34 + VR 4 + scripts 15); gates GREEN.
...
**Resume:** `main` @ `1a67cbc`, tree clean, CI green · last shipped: **#732** VR suite (PR #737 — goldens portable ONLY with the render pinned: `sans-serif` = Noto Sans here vs WenQuanYi Zen Hei in the Playwright image) · **no in-flight slice** · **decision queue: #735** (provisional, measured) · **NEXT: either the next #405 box or extending #732's VR arms** · new: **#736** `loadCaelumTheme()` loads half the bridge, so existing browser-spec contrast checks are vacuous · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
