...
**As of:** 2026-07-27 · main @ `ebd6531` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑** (grid gap → editor #232 / charts #233; niche widgets #667 → all on-demand per D-18) · **M4 ◐ — all exit criteria met, exit awaits the milestone `adversarial_review`** (#411 · #690+#691 · #240 · VR ☑ #732 · ledger ☑ #733 · theming ☑ #734) · template @ pyxis `79fdfbe`. Full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M4 active**. Shipped: density R4 #411 · a11y axe #690+#691 · hardening (PRs #697–#705) · adoption #713/#716 · harness #240 · VR ☑ #732 · ledger ☑ #733/#774/#779/#773 (**64/64 + 1 proven exempt**) · **☑ #785** error-state axe · **☑ #765** modals trap focus at attach (prov. #791) · **☑ #759** accordion arrow-key roving (D-623 reach; upstream ask #796) · **☑ #741** pre-touched controls surface their error (shared base → 9 subclasses) · **☑ #734** theming, closed by **☑ #413** `caelum.theme()` mixin (D-757) · **#405 11/12**. **1952 tests** (caelum 1700 + forge 71 + browser 142 + VR 24 + scripts 15); gates GREEN.

**Next:** **the M4 exit `adversarial_review`** — every exit criterion is now met, but the review cadence (`PROJECT_CONVENTIONS.md` › Operating posture) requires one at a milestone boundary and `definition_of_done` blocks the exit without linked findings; it needs reviewer subagents, so it is the owner's call to start. Then the README milestone-exit update. Recipes → `docs/PATTERNS.md` §9–§9l/§12–§13. Queued extras: **#709** cae-drawer · #710 cae-alert · #711 cae-panel/fieldset · #648(opt3) · #585.

**Open decisions:** **none** — the queue cleared 2026-07-27 (owner survey): **D-735** pin the VR render · **D-744** darken the failing light arms (shipped #746) · **D-757** ship the `caelum.theme($density)` mixin, mixin-only — *overruled* the recommended "close #413", so #413 is now a build slice · **D-771** retire #405 at 11/12, #228 spun out as a standing manual pass · **D-791** `delayFocusTrap: false` default (shipped #793). All five rows live in ARCHITECTURE Appendix A (the log, not here); no PR carries an unsettled `Provisional on`. **~130 open issues** = deferral log, not a burndown.

**Resume:** `main` @ `ebd6531`, tree clean, CI green · **no in-flight slice** · last shipped: **#413** (PR #806) the `caelum.theme()` mixin — mixin-only, both arms parameterized, default output byte-identical; `loadCaelumTheme()` re-routed via `testing/_theme-probe.scss` · **decisions: all 5 settled** (D-735/744/757/771/791) · **NEXT: the M4 exit `adversarial_review`** (owner's call — needs subagents), then README · queued **#709**/#710/#711 · findings: **#804**, **#805**, #796, #792, #787, #769, #766, #762, #758 · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE** (a pipe to `tail` returns the pipe's), and re-run it **after** doc edits.
