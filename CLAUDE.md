...
**As of:** 2026-07-23 · main @ `906f151` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — ★ families COMPLETE (media, tree, input, Splitter, drag-drop). **M3-exit set** (07-23): **#662–#666** CI-core-first + niche deferrals **#667**. **#662 display cluster SHIPPED** (PR #670): `caelum/{skeleton,avatar,timeline,tag}`; deferred **#669**. **#663 cae-rating SHIPPED** (PR #674): `caelum/rating` — selection-control CVA (#47), roving-tabindex radiogroup, D-596 icons; parity extras → **#675**. Naming per COMPARISON: Calendar → **`cae-datepicker`**, OverlayPanel → **`cae-popover`**. Prior: **D-595/D-596/D-652**; **#648** remains. **1549 tests**; gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Next:** continue the **M3-exit set** in order — ✓#662 display cluster → ✓#663 rating → **#664** popover + confirm-popup (one confirm service, Book 09 §3.3) → **#665** panel-menu (compose CaeAccordion) → **#666** datepicker (full parity; heaviest, lands last). Then the M3 exit-criterion check, with **#667** as the "listed if deferred" evidence. Fallbacks if one blocks: #165/#150 routerLink (copy #654), #120 (D-549), #648, #659, #669, #675, #639, #655, #418 (⚠ px-unit decision), #240 (M4).
...
**Resume:** `main` @ `906f151`, tree clean, CI green (full matrix) · last shipped: **#663 cae-rating** (PR #674, `usd=10.08`) · **no in-flight slice** · **decision queue EMPTY** · **NEXT: start #664 cae-popover + cae-confirm-popup** — the command-overlay pair (p-popover / p-confirmpopup parity). Anchored, trigger-relative overlay → the `cdkConnectedOverlay + cdkTrapFocus` family (NOT the centered-modal CaeDialog substrate); ONE shared confirm service for confirm-popup (Book 09 §3.3). Read the #664 body in full before scoping. · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
