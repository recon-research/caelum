...
**As of:** 2026-07-22 · main @ `58b9244` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — ★ families COMPLETE (media, tree, input, Splitter, drag-drop); **standing menu** is the lane, early-theme density pulled forward from M4. Recently: **D-595 routerLink OPEN** — #333 (PR #654) ships `caelum/breadcrumb-router`, the first optional-peer entry point (directive over the honest `<a href>`, barrel-exempt); **D-596 icons COMPLETE** (#644/#645). Review debt: **#649** iconContext single-homed; **#652 FIXED** — `caelum/grid-tanstack` split (D-652) so the barrel is @tanstack-free, exports guard now derived; **#648** (MatMenu typeahead) remains. **1487 tests**; gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `58b9244`, tree clean, CI green · last shipped: **grid-tanstack split #652** (PR #660, D-652) · **no in-flight slice** · **decision queue EMPTY** · **NEXT (owner-chosen): START M3 EXIT** — run `plan_milestone_tickets` **with the owner** to ticket the ~9 remaining (Calendar/Rating/Skeleton · OverlayPanel/ConfirmPopup · PanelMenu · Avatar/Timeline/Tag), then build · independent fallbacks: #165/#150 **routerLink** (copy #654), #120 (D-549), #648, #659 (guard-harden), #639, #655 · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
