...
**As of:** 2026-07-15 · main @ `585d787` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `12f3608`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **tree-select `[showClear]`** #282 (PR #452 — the library's first `showClear`; ×-clear + focus-restore-on-unmount + `--cae-target-min` density-invariant hit-target floor + dup-key dev-warn); **chip-set async empty-focus** #205 (PR #449); **carousel vertical** #276 (PR #446). **1232 tests** (1163 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `585d787`, tree clean, CI green · landed: tree-select `[showClear]` #282 (#452), chip-set async empty-focus #205 (#449), carousel vertical #276 (#446) · next = standing menu / next M3 (see **Next**; tree-select #282 remainder = in-panel filter + per-node disabled/selectable, #280/#283; #276 remainder = swipe/seamless-loop/focus + #445; chip-set #201/#448) · **dev inner loop**: `CI=true npx ng test caelum --filter '<test-name>'` (not `--include`; #414) + `npm run build:lib:fast` (full suite + `build:lib` = DoD + CI gate; #403) · verify: `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS, 1220) · `python3 scripts/check_provenance.py` (GREEN).
