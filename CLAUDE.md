...
**As of:** 2026-07-16 · main @ `7f717f0` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `5e8dbc0`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **galleria `[numVisible]`** thumbnail-strip windowing #481 (PR #482 — native-scroll window, full role=tablist stays in the a11y tree, RTL/axis-correct, min()-clamped vertical cap); **galleria `[thumbnailsPosition]`** #288 (PR #479); **tree-select `selectionMode='checkbox'`** #280 (PR #477). **1304 tests** (1235 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Next:** continue the **standing menu** or start the next M3 component. Candidates: breadcrumb #333 extras (per-item icons = cross-cutting **icon-supply** latent fork; overflow/collapse [no-overlay invariant]; routerLink [`@angular/router` peerDep fork, shared w/ tab-menu #165]), splitter px min/max #418, image-compare content-projection #422, input-family #305/#312/#315, galleria #288-rest/#289, tree-select #283, carousel #276, tree-table #264–#270, cae-table/grid clusters, chip-set #201/#448, hit-target sweep #456, test-infra #240 (M4).
...
**Resume:** `main` @ `7f717f0`, tree clean, CI green · landed this session: onboarded (Mode B) + **galleria `[numVisible]`** thumbnail-strip windowing #481 (PR #482 → 7f717f0, cost usd=9.78 — native-scroll window, `--cae-galleria-num-visible` cap var × shared thumb-extent vars, all tabs stay in the a11y tree, 2-lens reviewed no MAJOR, size 14584/15360 95%) · **next = standing menu, pick ONE full window:** galleria showThumbnailNavigators/fullScreen #288-rest, chip-set #201-rest/#448, carousel #276 (swipe/RTL-arrows/seamless-loop), tree-select #283 (lazy/virtual/mat-form-field) · verify: `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
