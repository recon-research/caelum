...
**As of:** 2026-07-14 · main @ `0ca8177` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `12f3608`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **density parity (R4)** #411 (PR #415 — runtime `[data-density]` compact arm: `--cae-space-*` re-scale + Material `density:-2` under one selector; target-size floor held; Forge toggle), **template sync** pyxis→`12f3608` (#407). **1170 tests** (1101 caelum + 69 forge); all gates GREEN. Per-slice detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Next:** #412 window → 2026-07-21 (density-API, provisional on A → `D-NN`); density follow-ons #413. Else **standing menu** / next M3 component. Candidates: breadcrumb #333 extras (per-item icons = cross-cutting **icon-supply** latent fork; overflow/collapse [no-overlay invariant]; routerLink [`@angular/router` peerDep fork, shared w/ tab-menu #165]), scroll-panel #329, splitter #325 remaining (px min/max, double-click-reset, gutterSize, touch), image-compare #318 remaining (click-to-jump, projection, i18n), input-family #305/#312/#315, galleria #287–#289, tree-select #280–#283, carousel #276, tree-table #264–#270, cae-table/grid clusters, chip-set #201/#205/#206, test-infra #240 (M4).

**Open decisions:** **#412** — density-selection API shape (global runtime `[data-density]` vs compile-time vs per-component); **proceeding provisionally on Option A** (recommended; reversible — Caelum unpublished), window → 2026-07-21, silence ratifies → `D-NN`. **85 open issues** = 1 decision + followups/debt/ideas/slices — a *deferral log*, not a burndown. Two latent forks, filed as `decision` when their slice is picked up: (1) `@angular/router` peerDep for breadcrumb/tab-menu `routerLink`; (2) the data-driven per-item **icon-supply** convention (`string→mat-icon` vs projected-template vs sanitized-SVG), cross-cutting breadcrumb #333 / menubar #150 / tab-menu #165 / split-button #149.

**Resume:** `main` @ `0ca8177`, tree clean, CI green · **density parity #411 shipped (PR #415)**, provisional on #412 · next action = continue the standing menu or pick the next M3 component (see **Next**); handle #412 if the human rules · **dev inner loop**: `CI=true npx ng test caelum --filter '<test-name>'` (not `--include`; #414) + `npm run build:lib:fast` (full suite + `build:lib` stay the DoD + CI gate; #403) · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (1170 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
