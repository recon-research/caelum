...
**As of:** 2026-07-24 · main @ `1f7fe11` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑** (grid gap → editor #232 / charts #233; niche widgets #667 → all on-demand per D-18) · **M4 ◐ Parity hardening & adoption** (density R4 #411 · a11y Layer 1 axe #690+#691 complete) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).
...
**Next:** **`p-*`→`cae-*` migration guide** — owner-chosen M4 priority, **gate now CLEAR** (#706+#707 closed by PR #713: COMPARISON gained Import + Status columns and is the single home of the map, so the guide must *link* it, never restate it). Research done (57 components, 8 API deltas, 6 divergences); draft at `~/caelum-migration-research-2026-07-24.md`. **#133 corrected** — the `ariaLabelledby` casing change is NOT library-wide (`CaeDialogConfig` keeps capital-B).
...
**Resume:** `main` @ `1f7fe11`, tree clean, CI green · last shipped: **#707+#706 map shape** (PR #713, `usd=2.44`) — COMPARISON gained Import + Status columns (swept against the 65 real entry points) and is now the single home of the map; #706's 13 untracked triaged → **#709/#710/#711** real gaps · **#712** on-demand · 2 reclassified; M3's cutoff claim corrected, not deleted · **no in-flight slice** · **decision queue EMPTY** · **NEXT: write `docs/MIGRATION.md`** (process + API deltas + divergences; link COMPARISON for the map) · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
