...
**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **image-compare RTL Page keys** #396 (PR #454 — horizontal PageUp/PageDown now invert under RTL, splitter parity); **tree-select `[showClear]`** #282 (PR #452 — first `showClear`; ×-clear + focus-restore + `--cae-target-min` hit-target floor + dup-key warn); **chip-set async empty-focus** #205 (PR #449). **1233 tests** (1164 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `7265a98`, tree clean, CI green · landed: image-compare RTL Page #396 (#454), tree-select `[showClear]` #282 (#452), chip-set async empty-focus #205 (#449) · next = standing menu / next M3 (see **Next**; tree-select #282 remainder = filter + per-node disabled, #280/#283; galleria #288 indicators; carousel #276 swipe/loop/focus + #445; chip-set #201/#448) · **dev inner loop**: `CI=true npx ng test caelum --filter '<test-name>'` (not `--include`; #414) + `npm run build:lib:fast` (full suite + `build:lib` = DoD + CI gate; #403) · verify: `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS, 1220) · `python3 scripts/check_provenance.py` (GREEN).
