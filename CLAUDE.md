...
**As of:** 2026-07-25 · main @ `720c4f2` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑** (grid gap → editor #232 / charts #233; niche widgets #667 → all on-demand per D-18) · **M4 ◐ Parity hardening & adoption** (density R4 #411 · a11y Layer 1 axe #690+#691 · real-browser harness #240 · VR suite ☑ #732) · template @ pyxis `79fdfbe`. Full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M4 active**. Shipped: density R4 #411 · a11y Layer 1 axe #690+#691 · 07-24 hardening batch (PRs #697–#705) · adoption track (#713, #716) · **harness #240** (PR #719) · **#405 sweep 3/12** (PRs #722/#726/#728/#730) · **VR suite ☑ #732** (PRs #737/#742) — 5 components × 4 arms; extending past one component exposed 3 mechanism defects, worst being a font pin that lost the cascade after test 1, so **3 of every 4 goldens rasterized unpinned**. Goldens now agree across dev box, container and CI runner. **1818 tests** (caelum 1674 + forge 71 + browser 38 + VR 20 + scripts 15); gates GREEN.

**Next:** **#405 sweep** is the open track — remaining: tree · accordion · confirm · table(×3) · tree-table · grid(×2) · listbox; work-list heuristics = grep component prose for real-browser deferrals, and where a jsdom spec's header admits it asserts *component state* not the DOM. Then **#734** theming polish (**now unblocked** by #732; likely absorbs #425 warn-colour WCAG 1.4.3, #510, #413) · **#733** capability ledger (M4 exit criterion; carries a derived-vs-curated fork that may need a `D-NN`). Recipes → `docs/PATTERNS.md` §9/§9a/§9b/§12. Queued extras: **#709** cae-drawer · #710 cae-alert · #711 cae-panel/fieldset · #741 · #648(opt3) · #585.

**Open decisions:** **#735** — VR golden portability; proceeding **provisionally on Option 1** (pin the render), **measured across 3 environments** and posted to the issue; 72h objection window, silence ratifies → becomes a `D-NN`. Recently decided: **D-595** router = optional peerDep · **D-596** per-item icons · **D-623** guarded 3p-private reach · **D-526** tree per-node state · **D-549** tag-entry → #120 · **D-614** no commit-subject gate · **D-652** grid optional-peer split. Prior: **D-428/D-501**. **~130 open issues** = deferral log, not a burndown.

**Resume:** `main` @ `720c4f2`, tree clean, CI green · last shipped: **#732 VR suite complete** (PR #742 — extending past one component is what exposed all three mechanism defects; every golden was inspected as an image, byte counts showed nothing) · **no in-flight slice** · **decision queue: #735** (provisional Option 1, now measured across dev/container/CI; window ends 2026-07-28T01:05Z, silence ratifies → record as `D-NN`) · **NEXT: the next #405 box, or #734 theming polish now that #732 unblocks it** · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
