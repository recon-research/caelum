...
**As of:** 2026-07-11 · main @ `67a5158` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail**. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 input family + ★ media cluster + Splitter family + navigation set complete. **Book 11 §3.3 drag-drop cluster COMPLETE** (OrderList ☑ · PickList ☑ · FileUpload ☑); now **closing its deferred parity extras**. The **multi-select pair is done**: **order-list #341** (PR #351 — `[(selection)]`) + **pick-list #342** (PR #353 — per-list `[(sourceSelection)]`/`[(targetSelection)]`), both ARIA listbox multiselect (focus≠selection, pointer + keyboard, block move/transfer, per-option describedby); plus **file-upload request-config #345** (PR #348, Content-Type-guarded). Each ran a 2-lens adversarial review — #341 fixed 2 MAJOR, #342 no MAJOR (emit-time selection coherence + coverage/doc fixes). #341/#342/#345 stay open for remaining extras. **997 tests** (933 caelum + 64 forge); all gates GREEN.

**Next:** continue **closing drag-drop parity extras** — remaining **pick-list #342** (within-list reorder + per-side move buttons shipped together per §3.5, header slots, in-list filter, RTL), remaining **order-list #341** (in-list filter, per-item disabled, dataKey, RTL), remaining **file-upload #345** (thumbnails, chunked, concurrent, paste/folder, row-template, method). Standing followups: focus-restore #350, forge initial-bundle budget #354. Then the standing menu (parity-extras/debt on shipped components): breadcrumb #333, scroll-panel #329, splitter #325, image-compare #318, input-family #305/#309/#312/#315, demo heading-order #306, galleria #287/#288/#289 [#286 #240-gated], tree-select #280–#283, carousel #276/#290, tree-table #263–#270, cae-table cluster, grid cluster, chip-set #201/#205/#206, test-infra #240 (M4).
...
**Resume:** `main` @ `67a5158`, tree clean, CI green · **pick-list multi-select #342 landed** (PR #353 merged) · next action = **continue closing drag-drop extras** — recommended **remaining pick-list #342** (within-list reorder + per-side move buttons, shipped together per §3.5) or pick from the standing menu · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (997 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
