...
**As of:** 2026-07-28 · main @ `936f15d` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑ · M4 ☑** (closed 2026-07-28 — all three exit clauses met; ledger **64/64**, no row revoked) · template @ pyxis `79fdfbe`. Full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M4 closed.** Its hard clause was the capability ledger: **#809** raised the bar from "a pointer resolves" to "a review happened", which revoked 3 sign-offs that were inline *self*-reviews. **☑ #825** (D-831) · **☑ #823** · **☑ #824** (D-826) each fixed its defect and earned the row back through an independent multi-lens review — those reviews found **13 HIGH**, mostly in the *fixes*. **2021 tests**; gates GREEN.

**Next:** **M5 is unplanned** — no milestone is active, so the next move is a planning call, not a slice. Carried-forward tickets, none of which gated M4: **#811** (axe scans one theme arm) · **#812** · **#813** · **#832** (anchored `aria-modal` vs `reposition()`) · **#836** · **#840** (popover RTL) · **#844**/**#846** (retro guards) · **#514** (the pre-publish `@recon-research/caelum` rename) · **#228** (standing manual SR pass, D-771).

**Open decisions:** **#841** — where focus should go when a popover's trigger is removed while the panel is open. Non-blocking (rare path, everything else in #824 shipped); an `isConnected` guard was tried and DELETED as provably inert. Decided this session: **D-831** (`confirmAt` takes a required `destroyRef`), D-826 widened to `cae-confirm-popup` with its grounding corrected, and #823's echo scope (**shape only, never range**) + numeric-string coercion, both on the ticket. **142 open issues** = deferral log.

**Resume:** `main` @ `936f15d`, tree clean, CI green · **no in-flight slice, no active milestone** · last shipped: **M4 exit** (PRs #842/#843) + its retrospective (#847 → guards #844/#845/#846) · next: answer **#841**, or plan M5 · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE** (a wrapper's exit can mask it); a Linux preflight cannot see a Windows-only break (#814).
