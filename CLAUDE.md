...
**As of:** 2026-07-24 · main @ `d0b3268` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑** (grid gap → editor #232 / charts #233; niche widgets #667 → all on-demand per D-18) · **M4 ◐ Parity hardening & adoption** (density R4 #411 · a11y Layer 1 axe #690+#691 · real-browser harness #240 complete) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M4 active** (parity hardening & adoption). Shipped: density R4 #411 · a11y Layer 1 axe #690+#691 · 07-24 hardening batch (PRs #697–#705) · adoption track (#713, #716) · **real-browser harness #240** (PR #719 — Vitest browser mode + Playwright, dev-tier; `*.browser.spec.ts` split from the jsdom target). **#405 sweep OPEN:** ☑ #718 grid `role=table` off the visual frame · ☑ #724 browser tests now load the real token layer (`color-contrast` had been vacuous). **1766 tests** (caelum 1674 + forge 71 + browser 11 + scripts 10); gates GREEN.

**Next:** **work the #405 real-browser sweep** now the harness (#240) unblocks it — a consolidated per-component checklist (the individual verify followups were rolled into it at the 07-13 triage; 5 stayed independent: #138 #155 #158 #165 #236). Harness blockers cleared (#718, #724); recipes → `docs/PATTERNS.md` §9/§12. Then: theming polish (density × scheme) · visual-regression suite · capability ledger green. Queued extras: **#709** cae-drawer · #710 cae-alert · #711 cae-panel/fieldset · #648(opt3) · #585.
...
**Resume:** `main` @ `d0b3268`, tree clean, CI green · last shipped: **#724** browser theme layer (PR #726) — a `*.browser.spec.ts` must call `loadCaelumTheme()` or its colour checks are vacuous (`docs/PATTERNS.md` §9); before it **#718** (PR #722) · **no in-flight slice** · **decision queue EMPTY** · **NEXT: work the #405 checklist** — a `*.browser.spec.ts` per component (tree/accordion/confirm/slider/toolbar/table/tree-table); both harness blockers are gone · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
