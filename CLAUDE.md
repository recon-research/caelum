...
**As of:** 2026-07-23 · main @ `bfbb3bc` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — ★ families COMPLETE (media, tree, input, Splitter, drag-drop). **M3-exit set** (07-23): **#662–#666** CI-core-first + niche deferrals **#667**. SHIPPED: **#662** display cluster (PR #670; → #669) · **#663** cae-rating (PR #674; CVA #47, D-596; → #675) · **#664** cae-popover + cae-confirm-popup (PR #678): `caelum/popover` (CDK Overlay + cdkTrapFocus, `[caePopoverTriggerFor]`) + `confirmAt()` on the ONE confirm service; → #677. Naming: Calendar → **`cae-datepicker`** (#666). **#648** remains. **1579 tests**; gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Next:** continue the **M3-exit set** in order — ✓#662 → ✓#663 → ✓#664 → **#665** panel-menu (compose CaeAccordion; p-panelmenu) → **#666** datepicker (full parity; heaviest, lands last). Then the M3 exit-criterion check, with **#667** as the "listed if deferred" evidence. Fallbacks if one blocks: #165/#150 routerLink (copy #654), #120 (D-549), #648, #659, #669, #675, #677, #639, #655, #418 (⚠ px-unit decision), #240 (M4).
...
**Resume:** `main` @ `bfbb3bc`, tree clean, CI green (full matrix) · last shipped: **#664 cae-popover + cae-confirm-popup** (PR #678, `usd=13.51`) · **no in-flight slice** · **decision queue EMPTY** · **NEXT: start #665 cae-panel-menu** — accordion-composed nested navigation menu (p-panelmenu parity); **compose `CaeAccordion`** rather than re-implementing expand/collapse. Read the #665 body in full before scoping. · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
