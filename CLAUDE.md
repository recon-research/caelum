...
**As of:** 2026-07-23 · main @ `a730b5a` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — ★ families COMPLETE (media, tree, input, Splitter, drag-drop). **M3-exit set** (07-23): **#662–#666** CI-core-first + niche deferrals **#667**. SHIPPED: **#662** display (PR #670) · **#663** cae-rating (#674) · **#664** cae-popover+confirm-popup (#678) · **#665** cae-panel-menu (#681) · **#666** cae-datepicker **stage 1** (#684): `caelum/datepicker` single/range/inline over MatDatepicker (imperative outer-control Validator for keyboard min/max/filter). #666 **stays OPEN** for stage 2 (multiple/time/datetime/month-year/showToday-clear = full parity, its DoD). **#648** remains. **1627 tests**; gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Next:** **#666 stage 2** (closes #666) — `multiple` (custom `MatDateSelectionModel<Date[],Date>` + `[dateClass]`/`(_userSelection)`; re-widen the narrowed `CaeDateSelectionMode`/`CaeDatepickerValue`), time/datetime (`MatTimepicker`), month/year (`startView`+monthSelected/yearSelected), showToday/showClear, + COMPARISON `cae-time-picker` note reconcile. Then the **M3 exit-criterion check** (**#667** = "listed if deferred" evidence). Fallbacks: #648, #659, #669, #675, #677, #680, #683, #639, #655, #418 (⚠ px-unit), #240 (M4).
...
**Resume:** `main` @ `a730b5a`, tree clean, CI green (full matrix) · last shipped: **#666 cae-datepicker stage 1** (PR #684, `usd=41.46`) · **no in-flight slice** · **decision queue EMPTY** · **NEXT: #666 stage 2** — multiple/time/datetime/month-year/showToday-clear (closes #666; read the #666 stage-2 checkpoint comment before scoping) · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
