...
**As of:** 2026-07-09 · main @ `2355e53` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand tracks per D-18) · **M3 ◐ Build-S/M long tail** · ops template synced from pyxis → `dfd6b47` (#259). Recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md); full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Where we are:** the grid is a swappable-engine adapter behind one neutral `CaeGridAdapter` port; **cae-table #144** and **cae-tree-table #262** landed. Recent ★ M3 slices: **cae-carousel #273**, **cae-tree-select #279** (value = node key(s); the library's first `cdkConnectedOverlay`), **cae-galleria #274** (thumbnail tablist + fullscreen lightbox over `CaeDialog`), and newest **cae-image #275** (PR #295) — the ★ media family's smallest member (`p-image` parity): a token-styled image with an **opt-in** fullscreen **zoom / rotate / pan** preview opened through **`CaeDialog` (D-15, MatDialog Direct)** — the **same centered-modal substrate as the galleria lightbox** (deliberately not tree-select's `cdkConnectedOverlay`, Book 09 §3.3). Internal `CaeImagePreview` (not exported): a labeled control group + `scale`/`rotation`/`pan` signals composed into one CSS transform; `aria-disabled` (not `disabled`) at zoom limits keeps focus; **every pointer gesture has a keyboard path** (`+`/`-` zoom, `0` reset, arrow-key pan — Book 11 §3.5 #1) + a polite zoom-% live region. New `caelum/image` entry point + @defer'd Forge demo (defer count 18→19). The zoom/pan transform is the primitive the galleria lightbox will later share (**#289**). Its 2-lens review (a11y + API) found **no MAJOR**; applied: preview default-**off** (p-image parity), `<figure>`/`<figcaption>` caption association (WCAG 1.3.1), "Reset view" rename (it clears rotation+pan), invalid-zoom-bounds dev-warn, rotation `mod 360`, focus-retained + observable-DOM tests. **695 tests** (631 caelum + 64 forge); provenance GREEN.

**Next:** M3 active per **D-18**; ★ media family complete (Carousel/Galleria/Image) — remaining ★ leads landed (TreeTable/Carousel/TreeSelect/Galleria/Image). Pick the next M3 ★/Build-S slice (e.g. InputNumber/InputMask/InputOtp directives, Rating, Skeleton, Splitter). Independent work: **cae-image-compare #293** (the before/after slider — its own slice) + `afterClosed`/DestroyRef family tidy **#294**; galleria follow-ups (#286 virtualization [#240-gated] / #287 item-templates / #288 parity-extras / #289 lightbox-zoom), tree-select (#280–#283), carousel (#276/#290), cae-tree-table (#263–#270), cae-table (#145/#251/#235/#236/#223/#241/#242/#253/#254/#255), grid (#193/#175/#177/#190/#174/#178/#179/#216/#228), cae-chip-set (#201/#205/#206), test-infra #240 (M4 browser runner), editor #232 / charts #233 on demand.

**Open decisions:** none — #142 → **D-16** (Forge warn-budget) and #168 → **D-17** (grid public-API shape) ratified by silence 2026-07-07; #185 → **D-18** (M2 scope: grid-only, advance to M3) decided by the human 2026-07-07. **81 open issues** = 0 decisions + 81 followups/debt/ideas.

**Resume:** `main` @ `2355e53`, tree clean · **cae-image #275 landed** (PR #295; `caelum/image` entry point, @defer'd Forge demo, 695 tests; follow-ups #293 image-compare / #294 DestroyRef filed) · next action = **pick the next M3 slice** (★ media family now complete) — e.g. an InputNumber/InputMask/InputOtp directive set, Rating, Skeleton, or Splitter, ticket-first. a11y/API-touching slices → 2-lens review · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (695 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS 0-skipped).
