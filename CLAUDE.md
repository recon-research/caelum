...
**Open decisions:** **#501** npm name (`caelum` TAKEN; rec. scoped `@recon-research/caelum`; ratifies 07-18) · **#428** validator convention (provisional A, ratifies 07-21). **Owner queue:** #504 allowlist snippet · CoC (#494). **97 open issues** = 1 decision + followups/debt/ideas/slices — a *deferral log*, not a burndown. Two latent forks, filed as `decision` when their slice is picked up: (1) `@angular/router` peerDep for breadcrumb/tab-menu `routerLink`; (2) the data-driven per-item **icon-supply** convention (`string→mat-icon` vs projected-template vs sanitized-SVG), cross-cutting breadcrumb #333 / menubar #150 / tab-menu #165 / split-button #149.

**Resume:** `main` @ `a7823dd`, tree clean, CI green · **dev box = Windows 10 laptop**: both preflights PASS natively (python3-shim gotcha → conventions › Toolchain) · two writers today (galleria + review lanes, protocol held) · review leftovers: **file-upload D-04 #496** (risky — review cadence), guard #498, nits #497, cae-tree #491, Pages #495, notes #499 · **next = pick ONE:** #496+#498, or standing menu (galleria #288-rest, chip-set #201/#448, carousel #276, tree-select #283) · verify: `bash scripts/preflight.sh` / `.\scripts\preflight.ps1` (PASS 2026-07-16).
