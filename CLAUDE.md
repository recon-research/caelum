...
**As of:** 2026-07-11 · main @ `cd77e58` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail**. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 input family + ★ media cluster + Splitter family + navigation set complete. **Book 11 §3.3 drag-drop cluster opened** (planned + ticketed: OrderList #336, PickList #337, FileUpload #338). Just landed: **cae-order-list #336** (PR #339) — a keyboard-operable, drag-reorderable `role=listbox` (`p-orderList` parity) over `cdkDropList` + `LiveAnnouncer`: rows are `role=option` with a roving tabindex (signal `activeIndex`, `cae-input-otp` no-CDK-key-manager precedent), a control column of 4 move buttons, content-agnostic `caeOrderListItem` template, `[(value)]` model + `(reorder)`; no foreign drag lib. 2-lens review fixed a converged MAJOR — move buttons use `aria-disabled` not native `[disabled]` (a bound-reaching disabled button blurred focus to `<body>`) — plus a doc MAJOR (signal-two-way example) and 3 MINORs (aria-describedby, activeIndex-clamp effect, non-interactive-row caveat). **929 tests** (865 caelum + 64 forge); all gates GREEN. Deferred → **#336** follow-ons.

**Next:** finish the drag-drop cluster — **PickList #337** (two connected `cdkDropList`s; reuses OrderList's drag/button/announce patterns) then **FileUpload #338** (independent; dropzone + `HttpClient` upload, trust-boundary validation). Standing follow-ons: order-list #336, breadcrumb #333, scroll-panel #329, splitter #325, image-compare #318, input-family #305/#309/#312/#315, demo heading-order #306, galleria #286[#240-gated]/#287/#288/#289, tree-select #280–#283, carousel #276/#290, tree-table #263–#270, cae-table cluster, grid cluster, chip-set #201/#205/#206, test-infra #240 (M4).

**Open decisions:** none (D-16/D-17 ratified-by-silence, D-18 human-decided 2026-07-07). **91 open issues** = 0 decisions + 91 followups/debt/ideas/slices.

**Resume:** `main` @ `cd77e58`, tree clean, CI green · **cae-order-list #336 landed** (PR #339 merged; cluster tickets #337/#338 open) · next action = **build PickList #337** (two connected `cdkDropList`s + move-selected/move-all buttons, reusing OrderList's `LiveAnnouncer`/roving patterns) — or FileUpload #338 / a standing follow-on · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (929 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
