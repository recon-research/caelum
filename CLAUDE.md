...
**As of:** 2026-07-12 · main @ `0b2c069` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail**. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 input family + ★ media cluster + Splitter family + navigation set complete. **Book 11 §3.3 drag-drop cluster COMPLETE** (OrderList ☑ · PickList ☑ · FileUpload ☑); now **closing its deferred parity extras**. Done: multi-select pair (order-list #341 / pick-list #342) + file-upload request-config #345 + **pick-list within-list reorder #342** (PR #356) + **pick-list header slots #342** (PR #358) + **order-list in-list filter #341** (PR #360) + **pick-list per-side in-list filter #342** (PR #362 — mirror per pane; shared `[filterMatch]`; **transfer stays live while filtering**, only within-list reorder disabled; filtered view is model-by-ref when blank; 2-lens no MAJOR + family fixes: focus item-stability across a filter change, transfer-enable split, side-named instructions — also touched order-list). In-list filter recipe → memory `filtered-list-index-decoupling`. #341/#342/#345 stay open for remaining extras. **1041 tests** (977 caelum + 64 forge); all gates GREEN.

**Next:** continue **closing drag-drop parity extras** — remaining **pick-list #342** (RTL of the transfer axis: flip arrow glyphs + pane order via `Directionality`), remaining **order-list #341** (per-item disabled + drag handle, dataKey/trackBy, controlsPosition, RTL), remaining **file-upload #345** (thumbnails, chunked, concurrent, paste/folder, row-template, method) — an RTL pass could cover order-list + pick-list together. Standing followups: focus-restore #350, forge initial-bundle budget #354. Then the standing menu (parity-extras/debt on shipped components): breadcrumb #333, scroll-panel #329, splitter #325, image-compare #318, input-family #305/#309/#312/#315, demo heading-order #306, galleria #287/#288/#289 [#286 #240-gated], tree-select #280–#283, carousel #276/#290, tree-table #263–#270, cae-table cluster, grid cluster, chip-set #201/#205/#206, test-infra #240 (M4).

**Open decisions:** none (D-16/D-17 ratified-by-silence, D-18 human-decided 2026-07-07). **94 open issues** = 0 decisions + 94 followups/debt/ideas/slices.

**Resume:** `main` @ `0b2c069`, tree clean, CI green · **pick-list per-side in-list filter #342 landed** (PR #362 merged) · next action = **continue closing drag-drop extras** — recommended an **RTL pass** for the family (order-list #341 + pick-list #342 transfer axis) or remaining **order-list #341** (per-item disabled, dataKey) / **file-upload #345**, or the standing menu · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (1041 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
