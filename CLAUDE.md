...
**Resume:** `main` @ `fb9d67d`, tree clean, CI green · **no in-flight slice** · last shipped: **#759** (PR #797) accordion regains APG arrow-key roving — Material's own key manager, fed across the view boundary (D-623 reach, guard 3 = #796); mutation-tested 4 ways · **decisions: #735, #744, #757** (ratify 07-28), **#771, #791** (ratify 07-29) · **NEXT: #734** (#413 parked on #757) or an M4 exit pass once #771 lands · open findings: #792, #787, #769, #766, #762, #758 · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE** (a pipe to `tail` returns the pipe's), and re-run it **after** doc edits — a stale PASS reddened #753.
