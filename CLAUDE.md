...
**As of:** 2026-07-24 · main @ `ac1cc16` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑** (grid gap → editor #232 / charts #233; niche widgets #667 → all on-demand per D-18) · **M4 ◐ Parity hardening & adoption** (density R4 #411 · a11y Layer 1 axe #690+#691 complete) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M4 active** (parity hardening & adoption). Shipped: density R4 (#411); a11y Layer 1 axe (#690+#691 — 60 components, 0 real violations; real-browser gaps → #240). **Hardening batch (07-24):** #563+#419 splitter focus-ring tokens + stale jsdom comments (PR #697) · #642 receipt-alarm denominator, 17/40→17/17 (#698) · #532 breadcrumb 2.4.8 nameless-current dev-warn (#699) · #669 tag colour-only dev-warn (#700) · #571 carousel focus re-sync under a consumer page write (#701) · #491 one CDK node type in cae-tree/cae-tree-select (#703). **1744 tests** (caelum 1673 + forge 71); gates GREEN.

**Next:** continue M4 — a fresh priority call among **theming polish** (density × scheme), **visual-regression #240** (also carries keyboard/SR + real-browser axe), **capability ledger**, or the **`p-*`→`cae-*` migration guide**. Queued parity-extras: **#585** (tree-select Alt+ArrowDown; APG option 1 = the reversible default) · **#648** (scope to opt-3 dev-warn; opt-2 private reach needs a decision) · #659, #675, #677, #680, #683, #686, #639, #655, #418 (⚠ px-unit), #509 (item 1 = em-vs-rem convention pick).
...
**Resume:** `main` @ `ac1cc16`, tree clean, CI green (full matrix) · last shipped: **#491 CDK node-type fix** (PR #703, `usd=0.64`) — the ticket's suspected cause (`[isExpandable]`) was wrong; it was a flat leaf in a nested tree · also closed this batch: #563/#419/#642/#532/#669/#571; **#570 closed as already-fixed** by #603, **#509 item 2** already done by #456 (item 1 still open) · **no in-flight slice** · **decision queue EMPTY** · **NEXT: fresh M4 priority call** (theming polish / #240 / capability ledger / migration guide), else the queued extras above · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
