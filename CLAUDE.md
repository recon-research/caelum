...
**As of:** 2026-07-15 · main @ `c8fd8e4` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `12f3608`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **scroll-panel overflow-detection** #329 (PR #441 — MutationObserver horizontal-nowrap fallback); **galleria projected item/thumb templates** #287 (PR #439); **chip-set `[emptyFocusTarget]` dev-warn** #206 (PR #437); **carousel #276** `responsiveOptions` + inline-axis RTL (PR #433/#435 — vertical/swipe/loop deferred). **1209 tests** (1140 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Next:** continue the **standing menu** or start the next M3 component. Candidates: breadcrumb #333 extras (per-item icons = cross-cutting **icon-supply** latent fork; overflow/collapse [no-overlay invariant]; routerLink [`@angular/router` peerDep fork, shared w/ tab-menu #165]), splitter px min/max #418, image-compare content-projection #422, input-family #305/#312/#315, galleria #288–#289, tree-select #280–#283, carousel #276, tree-table #264–#270, cae-table/grid clusters, chip-set #201/#205, test-infra #240 (M4).
...
**Resume:** `main` @ `c8fd8e4`, tree clean, CI green · landed: scroll-panel #329 (#441), galleria #287 (#439), chip-set #206 (#437), carousel #276 (#433/#435) · next = standing menu / next M3 (see **Next**; #276 remainder = vertical/swipe/loop/focus) · **dev inner loop**: `CI=true npx ng test caelum --filter '<test-name>'` (not `--include`; #414) + `npm run build:lib:fast` (full suite + `build:lib` = DoD + CI gate; #403) · verify: `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS, 1209) · `python3 scripts/check_provenance.py` (GREEN).
