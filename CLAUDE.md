...
**As of:** 2026-07-03 · main @ `ede85a7` · **repo LIVE + protected** — [github.com/recon-research/caelum](https://github.com/recon-research/caelum) · **Phase:** 0 — Foundation · **Milestone:** M0 ◐ — the Direct-component parity family (active). **CI:** `full` + blocking, main green (required checks: `static gates` + `heavy matrix gate`). Prerequisites DONE: the 20-book knowledge library (`textbooks/`, five audits green) + REPO-1 public go-live. Full M0 slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md); the reusable Direct-component recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Done — M0 Direct-component family** (latest first; full write-up in each issue/PR, not re-narrated here): cae-checkbox/switch ariaLabel/ariaLabelledby naming seam #70 · cae-toolbar/badge #126 · cae-autocomplete #119 · cae-listbox #114 · cae-slider #109 · **D-15 dialog/toast slice COMPLETE** (cae-confirm #101 · cae-dialog #100 · cae-toast #96) · Forge @defer budget #85 · display primitives #88 · cae-chip #83 · accordion #77 · toggle/select-button #73 · cae-switch #68 · cae-button a11y forwarding (tooltip #36 / menu #57 / disabledInteractive #58) · stepper linear #40 · `CaeFormFieldControlBase` #46 · validation-error forwarding #29/#47 · secondary entry points #28 · Direct batches 1–3 #5/#26/#27 · foundation #1–#6. Housekeeping: python→python3 #7, doc compaction #123.

**Next:** the M0 Direct family + its a11y seams are now essentially complete (naming #70 + describedby #47 done); remaining candidates lean **Compose (M1-flavored)** — the richer **`cae-multi-select`** (Book 09 — MatSelect[multiple] + filter + chip summary), **#84** cae-chip-set (managed removable-chip list, focus redirect + roving), or a **data-display / panel-layout row** (avatar/skeleton = Build-S bespoke; tag = Compose over MatChip). Consider whether to formally open **M1 Composed components** vs. squeeze remaining M0 items. **#97** (split Forge's monolithic app.scss) must land before the next EAGER styled Forge demo (recent demos are `@defer`'d with lean styles, so the 8 kB anyComponentStyle error hasn't tripped).

**Decisions:** all resolved → `D-01..D-15` (ARCHITECTURE Appendix A); **no open `decision` issues**. Open followups/debt/ideas: `gh issue list` (#8/#12/#15/#33/#41/#48/#51/#54/#61/#72/#75/#78/#79/#80/#84/#90/#91/#97/#102/#105/#107/#110/#111/#116/#117/#120/#127/#128/#129/#133).

**Resume:** `main` @ `ede85a7` · next = pick the next M0/M1 slice (candidates in **Next**); docs current, truth = tracker. **Flow** (details: [`PROJECT_CONVENTIONS.md`](PROJECT_CONVENTIONS.md) › Merge policy / Tracker): ticket-first → `slice/<n>-<slug>` branch → PR → green CI → merge → merge-time checkpoint; every public-API/a11y slice gets a **4-lens ultracode adversarial review** before merge. **Verify:** `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (292 pass — 255 lib + 37 forge; `ng test` alone runs Forge only → also run `ng test caelum`) · `python3 scripts/check_provenance.py` (GREEN) · `bash scripts/preflight.sh` (PASS 0-skipped).
