...
**As of:** 2026-07-19 · main @ `027bbc5` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **chip-set sibling-grab focus steal #556** (PR #619 — filed #617/#618; budget row bumped per #603); **commit-msg semantics guard #591** (PR #615 — #583 length half → decision #614); **stepper shrink-focus #604 partial** (PR #612 — filed #611). **1440 tests** (1371 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `027bbc5`, tree clean, CI green · last shipped: chip-set focus steal #556 (PR #619) · **no in-flight slice** · **next = standing menu, pick ONE:** chip-set #553, splitter slop #544/#545 (⚠ jsdom-invisible — weigh M4), stepper key-manager #611 (⚠ privates or upstream), size-budget #603 · #559/#574 **M4-gated** (jsdom can't model `inert` blur or SR output) · ⚠ decision-blocked: breadcrumb #333 **fully parked** (#595/#596), tree-select #282/#283 (#526), chip-set listbox (#549), splitter px #418 · verify: `bash scripts/preflight.sh` **with node on PATH** (nvm bin; else format/build/test/lint silently skip & falsely PASS — #538).
