...
**As of:** 2026-07-18 · main @ `c6d8b48` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **carousel #560** indicator arrows anchor on the active page (PR #575 — filed #571–#574); **status-anchor guard #525** (PR #568 — the banner cried STALE after every checkpoint); **carousel #276 touch swipe** (PR #561). **1388 tests** (1319 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `c6d8b48`, tree clean, CI green · last shipped: carousel #560 (PR #575) · **no in-flight slice** · **next = standing menu, pick ONE:** galleria #572 (the #560 bug, unfixed sibling), breadcrumb #333 extras, splitter slop #544/#545, chip-set #552/#553/#556, carousel #573 circular-arrow parity · #559/#574 are **M4-gated** (jsdom can't model `inert` blur or SR output) · ⚠ decision-blocked: tree-select #282/#283 (#526), chip-set listbox (#549), splitter px #418 · verify: `bash scripts/preflight.sh` **with node on PATH** (nvm bin; else format/build/test/lint silently skip & falsely PASS — #538).
