...
**As of:** 2026-07-11 · main @ `2cbe473` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail**. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 input family + ★ media cluster + Splitter family + navigation set complete. **Book 11 §3.3 drag-drop cluster COMPLETE** (OrderList ☑ · PickList ☑ · FileUpload ☑). Now **closing the cluster's deferred parity extras** — two landed this session: **file-upload request-config #345** (PR #348 — `[withCredentials]`/`[headers]`/`[params]`, Content-Type-guarded) and **order-list multi-select #341** (PR #351 — `[(selection)]` ARIA listbox multiselect: focus≠selection, pointer + keyboard, block move, `movedIndices`). Both ran a 2-lens adversarial review; #341's caught **2 MAJOR** (describedby forwarded per-option à la listbox.ts; range anchor stored as item not stale index) + 4 MINOR, all fixed. #341/#345 stay open for remaining extras. **987 tests** (923 caelum + 64 forge); all gates GREEN.

**Next:** continue **closing drag-drop parity extras** → **pick-list #342** (mirror order-list's multi-select + per-option describedby; within-list reorder, in-list filter, header slots), remaining **order-list #341** (in-list filter, per-item disabled, dataKey, RTL), remaining **file-upload #345**. New followups from #341 review: focus-restore #350. Then the standing menu (parity-extras/debt on shipped components): breadcrumb #333, scroll-panel #329, splitter #325, image-compare #318, input-family #305/#309/#312/#315, demo heading-order #306, galleria #287/#288/#289 [#286 #240-gated], tree-select #280–#283, carousel #276/#290, tree-table #263–#270, cae-table cluster, grid cluster, chip-set #201/#205/#206, test-infra #240 (M4).
...
**Resume:** `main` @ `2cbe473`, tree clean, CI green · **order-list multi-select #341 landed** (PR #351 merged; also file-upload #345 via #348) · next action = **continue closing drag-drop extras** — recommended **pick-list #342 multi-select** (mirror order-list: `[(selection)]` + per-option describedby) · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (987 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
