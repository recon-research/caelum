...
**As of:** 2026-07-09 · main @ `da99702` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand tracks per D-18) · **M3 ◐ Build-S/M long tail** · ops template synced from pyxis → `dfd6b47` (#259). Recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md); full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Where we are:** the grid is a swappable-engine adapter behind one neutral `CaeGridAdapter` port; **cae-table #144** and **cae-tree-table #262** landed. Recent ★ M3 slices: **cae-carousel #273**, **cae-tree-select #279** (value = node key(s); the library's first `cdkConnectedOverlay`), and newest **cae-galleria #274** (PR #291) — the ★ media family's image gallery: a main image view + prev/next navigators + a **thumbnail strip** (WAI-ARIA `tablist`/`tab`/`tabpanel`, roving tabindex, selection-follows-focus) + a polite position live region + a **fullscreen lightbox** opened through **`CaeDialog` (D-15)** — Material's centered modal supplies the focus-trap/`Escape`/backdrop/focus-restore for free (deliberately **not** tree-select's connected overlay — wrong shape for a centered modal, Book 09 §3.3); typed `CaeGalleriaItem`, two-way `[(activeIndex)]` shared with the lightbox; new `caelum/galleria` entry point + @defer'd Forge demo. Its 2-lens review fixed a **no-orphan-tabpanel** MAJOR (gate the tab semantics on the tablist actually rendering — single-image / `showThumbnails=false`), a `role=group` + conditional roledescription (mirror carousel, no region-landmark spam), an already-open + async-load-clobber guard, and lightbox Home/End parity. **cdk-virtual-scroll strip virtualization deferred to #286 (gated on #240)** — jsdom can't verify virtualized roving focus. **676 tests** (612 caelum + 64 forge); provenance GREEN.

**Next:** M3 active per **D-18**; ★ leads landed (TreeTable/Carousel/TreeSelect/Galleria) — remaining media ★ = **cae-image #275** (lightbox preview + before/after compare, `p-image` parity; reuses the galleria lightbox shell, zoom/pan shared via #289). Independent work: galleria follow-ups (#286 virtualization [#240-gated] / #287 item-templates / #288 parity-extras / #289 lightbox-zoom), tree-select follow-ups (#280–#283), carousel follow-ups (#276/#290), cae-tree-table (#263–#270), cae-table (#145/#251/#235/#236/#223/#241/#242/#253/#254/#255), grid (#193/#175/#177/#190/#174/#178/#179/#216/#228), cae-chip-set (#201/#205/#206), test-infra #240 (M4 browser runner), editor #232 / charts #233 on demand.

**Open decisions:** none — #142 → **D-16** (Forge warn-budget) and #168 → **D-17** (grid public-API shape) ratified by silence 2026-07-07; #185 → **D-18** (M2 scope: grid-only, advance to M3) decided by the human 2026-07-07. **80 open issues** = 0 decisions + 80 followups/debt/ideas.

**Resume:** `main` @ `da99702`, tree clean · **cae-galleria #274 landed** (PR #291; `caelum/galleria` entry point, @defer'd Forge demo, 676 tests; follow-ups #286–#290 filed) · next action = **pick the next M3 slice** — recommended: **cae-image #275** (★ media, lightbox preview / before-after, reuses the galleria lightbox shell) or another M3 ★/Build-S slice, ticket-first. a11y/API-touching slices → 2-lens review · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (676 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS 0-skipped).
