...
**As of:** 2026-07-14 · main @ `ed042ac` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** · template @ pyxis `12f3608`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; now working the **standing menu** (parity-extras/debt on shipped components). Recently landed: **template sync** pyxis→`12f3608` (#407, PR #409 — heavy-gate #206 classify-died guard, staleness audit #222, harvest_downstream, audit_docs converged to upstream width-cap), splitter collapse/expand #325 (PR #400), image-compare vertical #318 (PR #397), input-otp mask+readonly #309 (PR #392). **1166 tests** (1098 caelum + 68 forge); all gates GREEN. Per-slice detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Open decisions:** none (D-16/D-17 ratified-by-silence; D-18 human-decided 2026-07-07). **82 open issues** = 0 decisions + followups/debt/ideas/slices — a *deferral log*, not a burndown. Two latent forks, filed as `decision` when their slice is picked up: (1) `@angular/router` peerDep for breadcrumb/tab-menu `routerLink`; (2) the data-driven per-item **icon-supply** convention (`string→mat-icon` vs projected-template vs sanitized-SVG), cross-cutting breadcrumb #333 / menubar #150 / tab-menu #165 / split-button #149.

**Resume:** `main` @ `ed042ac`, tree clean, CI green · **template synced to pyxis `12f3608` (#407)** — machinery current · next action = continue the standing menu or pick the next M3 component (see **Next**) · **dev inner loop**: `ng test caelum --include <spec> --watch` + `npm run build:lib:fast` (the full suite + full `build:lib` stay the DoD + CI gate; #403) · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (1166 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
