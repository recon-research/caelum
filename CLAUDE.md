...
**Where we are:** **M4 active** (parity hardening & adoption). Shipped: density R4 (#411); a11y Layer 1 axe (#690+#691 — 60 components, 0 real violations; real-browser gaps → #240). **Hardening batch (07-24):** 8 tickets over PRs #697–#705 (#563/#419 · #642 · #532 · #669 · #571 · #491 · #509) — per-slice detail in the PRs + ROADMAP M4. **Adoption track DONE (07-24):** COMPARISON gained Import + Status columns (#707+#706, PR #713) and **`docs/MIGRATION.md` shipped** (#715, PR #716), linked from README. **1744 tests** (caelum 1673 + forge 71); gates GREEN.

**Next:** **fresh M4 priority call** — the adoption track is DONE (guide shipped #715). Remaining M4 exit items: **#240** keyboard/SR + real-browser audit + visual-regression suite (the largest, and the only one gating a real a11y claim) · theming polish (density × scheme) · capability ledger green. Queued extras: **#709** cae-drawer (Direct, highest-impact gap) · #710 cae-alert · #711 cae-panel/fieldset · #648(opt3) · #585.
...
**Resume:** `main` @ `160247d`, tree clean, CI green · last shipped: **#715** `docs/MIGRATION.md` (PR #716, `usd=2.02`) — it **links** COMPARISON, never restates the map; closed **#133 by correction** (its "library-wide" casing claim was wrong — a blanket codemod would unname every dialog) · prior **#707+#706** map shape (PR #713, `usd=2.44`) · **no in-flight slice** · **decision queue EMPTY** · **NEXT: fresh M4 priority call** (see Next) · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
