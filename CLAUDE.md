...
**As of:** 2026-07-20 · main @ `ee7d1dc` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — ★ families COMPLETE (media, tree, input, Splitter, drag-drop); **standing menu** is the lane, with early-theme density pulled forward from M4. Recently landed: **stepper keyboard trap + refused repair #611/#605** (PR #640 — first D-623 private reach; guard-3 upstream report drafted on #639, needs approval), closing the stepper cluster; **vacuous assertions #632** (PR #635). **1451 tests** (1382 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `ee7d1dc`, tree clean, CI green · last shipped: #611/#605 (PR #640) · **no in-flight slice** · **decision queue EMPTY** · **next = pick ONE:** #333/#165/#150/#149 router + icons (D-595/D-596, the big unlock), #120 tag-entry CVA (D-549), #639 upstream post (**owner approval**), chip-set #553, splitter #544/#545, carousel #276 · #602 SSR-gated (D-11) · #559/#574 **M4-gated** · verify: `bash scripts/preflight.sh` **with node on PATH** (else gates falsely PASS — #538) **and read the EXIT CODE, not the test count**.
