...
**As of:** 2026-07-24 · main @ `6b376e7` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑** (grid gap → editor #232 / charts #233; niche widgets #667 → all on-demand per D-18) · **M4 ◐ Parity hardening & adoption** (density R4 #411 · a11y Layer 1 axe #690+#691 complete) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M4 active** (parity hardening & adoption). Shipped: density R4 (#411); a11y Layer 1 axe (#690+#691 — 60 components, 0 real violations; real-browser gaps → #240). **Hardening batch (07-24):** #563+#419 splitter focus-ring tokens + stale jsdom comments (PR #697) · #642 receipt-alarm denominator, 17/40→17/17 (#698) · #532 breadcrumb 2.4.8 nameless-current dev-warn (#699) · #669 tag colour-only dev-warn (#700) · #571 carousel focus re-sync under a consumer page write (#701) · #491 one CDK node type in cae-tree/cae-tree-select (#703) · #509 em-vs-rem type-sizing convention, PATTERNS §11 (#705). **1744 tests** (caelum 1673 + forge 71); gates GREEN.

**Next:** **`p-*`→`cae-*` migration guide** — owner-chosen M4 priority. Research done (57 components mapped, 8 API deltas, 6 divergences); source draft at `~/caelum-migration-research-2026-07-24.md`. Gated on **#706** (13 untracked no-equivalent components) + **#707** (COMPARISON lacks import-path / shipped-vs-planned columns, which the guide inherits); **#133 corrected** — the casing change is NOT library-wide.
...
**Resume:** `main` @ `6b376e7`, tree clean, CI green (full matrix) · last shipped: **#509 type-sizing convention** (PR #705, `usd=1.23`) — audit showed 5 of 6 `em` sites were already correct, so the ticket's proposed sweep would have broken them · batch also closed: #563/#419/#642/#532/#669/#571/#491; **#570 closed as already-fixed** by #603, **#509 item 2** already done by #456 (item 1 still open) · **no in-flight slice** · **decision queue EMPTY** · **NEXT: fresh M4 priority call** (theming polish / #240 / capability ledger / migration guide), else the queued extras above · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
