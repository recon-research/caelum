...
**As of:** 2026-07-07 · main @ `79b3941` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand tracks per D-18) · **M3 ◐ Build-S/M long tail**. Recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md); full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Next:** M3 (build-S/M long tail) is active per **D-18**; team-priority slices (★ media Carousel/Galleria, TreeSelect/TreeTable) lead. Immediate independent work: cae-table follow-ups (#224 next, then #144 sticky/expandable, #145, #223), grid follow-ups (#193/#175/#177/#190 parts 2–3/#174/#178/#179/#216/#228), cae-chip-set (#201/#205/#206), editor #232 / charts #233 on demand.

**Open decisions:** none — #142 → **D-16** (Forge warn-budget) and #168 → **D-17** (grid public-API shape) ratified by silence 2026-07-07; #185 → **D-18** (M2 scope: grid-only, advance to M3) decided by the human 2026-07-07. **55 open issues** = 0 decisions + 55 followups/debt/ideas (editor #232, charts #233 filed).

**Resume:** `main` @ `79b3941`, tree clean · next action = **slice #224** — cae-table single-select click-to-deselect (p-table parity): either an opt-in `[allowDeselect]` that clears the selection when the checked radio is re-activated (native radios swallow the re-click → needs a click handler), or document a consumer "Clear selection" control + a `textbooks/reference/COMPARISON.md` divergence note (the issue lists both); a11y/API-touching → 2-lens review · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (535 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS 0-skipped).
