...
**As of:** 2026-07-14 · main @ `e61c756` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `12f3608`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **image-compare parity extras** #318 (PR #423 — click-the-track-to-jump + i18n `[valueTextFormatter]`; content-projection split to #422), **splitter parity extras** #325 (PR #420 — `[gutterSize]` + double-click reset + touch hit-slop; px min/max → #418), **density parity (R4)** #411 (PR #415). **1178 tests** (1109 caelum + 69 forge); all gates GREEN. Per-slice detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Open decisions:** none — density-API ratified as **D-19** (#412, human decided in-session 2026-07-14). **85 open issues** = 0 decisions + followups/debt/ideas/slices — a *deferral log*, not a burndown. Two latent forks, filed as `decision` when their slice is picked up: (1) `@angular/router` peerDep for breadcrumb/tab-menu `routerLink`; (2) the data-driven per-item **icon-supply** convention (`string→mat-icon` vs projected-template vs sanitized-SVG), cross-cutting breadcrumb #333 / menubar #150 / tab-menu #165 / split-button #149.

**Resume:** `main` @ `e61c756`, tree clean, CI green · **image-compare extras #318 shipped (PR #423)**; splitter #325 + density #411/D-19 landed · next action = continue the standing menu or pick the next M3 component (see **Next**) · **dev inner loop**: `CI=true npx ng test caelum --filter '<test-name>'` (not `--include`; #414) + `npm run build:lib:fast` (full suite + `build:lib` stay the DoD + CI gate; #403) · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (1178 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
