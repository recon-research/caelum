...
**As of:** 2026-07-09 · main @ `6ec7aa8` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand tracks per D-18) · **M3 ◐ Build-S/M long tail** · ops template synced from pyxis → `dfd6b47` (#259). Recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md); full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Where we are:** the grid is a swappable-engine adapter behind one neutral `CaeGridAdapter` port; **cae-table #144** and **cae-tree-table #262** landed. Recent ★ M3 slices: **cae-carousel #273** (PR #277 — content-agnostic rotating carousel, WCAG 2.2.2 autoplay, APG a11y) and newest **cae-tree-select #279** (PR #284) — the value-picker family's tree control and TreeTable's form cousin: hierarchical single/multi node-select, **value = node key(s)** (never label/expansion — Book 09 §3.5), rendered in the library's **first `cdkConnectedOverlay`** panel (MatTree + selection Set + `cdkTrapFocus` focus-in/restore), following the **#47 consumer-owned-error** selection-control pattern (not the mat-form-field base); new `caelum/tree-select` entry point + @defer'd Forge demo. Its 2-lens review shipped: `aria-controls`→the `role=tree` popup, single-mode `aria-selected` only on the selected node, an empty-tree focus-strand + `setDisabledState`-touched guards, a `selectionMode` set-once dev-warn, and a keyboard-`(activation)` test (API lens confirmed value-fidelity + no double-fire, CDK-traced). **649 tests** (585 caelum + 64 forge); provenance GREEN.

**Next:** M3 active per **D-18**; the ★ leads have landed (TreeTable/Carousel/TreeSelect) — next ★ = **Galleria** (#274, media; thumbnail strip + overlay lightbox, builds on carousel's index model + the overlay shell). Independent work: tree-select follow-ups (#280 checkbox tri-state / #281 focus-selected + live-region / #282 showClear/filter/leaf-only / #283 lazy/virtual/mat-form-field), carousel follow-ups (#275/#276), cae-tree-table follow-ups (#263–#270), cae-table follow-ups (#145/#251/#235/#236/#223/#241/#242/#253/#254/#255), grid follow-ups (#193/#175/#177/#190/#174/#178/#179/#216/#228), cae-chip-set (#201/#205/#206), test-infra #240 (M4 browser runner), editor #232 / charts #233 on demand.

**Open decisions:** none — #142 → **D-16** (Forge warn-budget) and #168 → **D-17** (grid public-API shape) ratified by silence 2026-07-07; #185 → **D-18** (M2 scope: grid-only, advance to M3) decided by the human 2026-07-07. **76 open issues** = 0 decisions + 76 followups/debt/ideas.

**Resume:** `main` @ `6ec7aa8`, tree clean · **cae-tree-select #279 landed** (PR #284; `caelum/tree-select` entry point, @defer'd Forge demo, 649 tests; follow-ups #280–#283 filed) · next action = **pick the next M3 slice** — recommended: **Galleria** (#274, ★ media, thumbnail strip + overlay-shell lightbox, builds on carousel's index model) or another M3 ★/Build-S slice, ticket-first. a11y/API-touching slices → 2-lens review · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (649 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS 0-skipped).
