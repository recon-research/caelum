...
**As of:** 2026-07-24 · main @ `0a71780` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑** (grid gap → editor #232 / charts #233; niche widgets #667 → all on-demand per D-18) · **M4 ◐ Parity hardening & adoption** (density R4 #411 · a11y Layer 1 axe #690+#691 · real-browser harness #240 complete) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M4 active** (parity hardening & adoption). Shipped: density R4 (#411); a11y Layer 1 axe (#690+#691); the 07-24 hardening batch (PRs #697–#705); the adoption track (PRs #713, #716). **Real-browser harness LIVE (07-24, #240, PR #719):** Vitest browser mode + Playwright (dev-tier), Chromium-preferred auto-select, `*.browser.spec.ts` split from the jsdom target — per-slice detail in the PRs + ROADMAP M4. **#405 sweep OPEN (07-24):** first find #718 fixed (PR #722) — the grid's `role=table` now sits on an inner element, not the visual frame, so both axe carve-outs came off. **1764 tests** (caelum 1674 + forge 71 + browser 9 + scripts 10); gates GREEN.

**Next:** **work the #405 real-browser sweep** now the harness (#240) unblocks it — a consolidated per-component checklist (the individual verify followups were rolled into it at the 07-13 triage; 5 stayed independent: #138 #155 #158 #165 #236). First find **#718** done (PR #722); sweep heuristic → `docs/PATTERNS.md` §12. Then: theming polish (density × scheme) · visual-regression suite · capability ledger green. Queued extras: **#709** cae-drawer · #710 cae-alert · #711 cae-panel/fieldset · #648(opt3) · #585.

**Open decisions:** **none** — queue empty. Recently decided (in-session): **D-595** router = optional peerDep + 2nd entry point · **D-596** per-item icons = `icon` glyph **and** `iconTemplate` · **D-623** guarded 3p-private reach + upstream · **D-526** tree per-node state = on-node fields · **D-549** tag-entry → `cae-autocomplete` #120, not `cae-chips` · **D-614** no commit-subject gate · **D-652** (07-22) grid optional-peer → split `caelum/grid-tanstack`, generalizing D-595. Prior: **D-428/D-501**. **~127 open issues** = deferral log, not a burndown.

**Resume:** `main` @ `0a71780`, tree clean, CI green · last shipped: **#718** grid table-role split (PR #722, `usd=5.72`) — frame ≠ role holder; axe runs with **no rules disabled** in both arms. Harness **#240**: `npm run test:browser` (a pin is **strict** — missing pinned browser FAILS) · **no in-flight slice** · **decision queue EMPTY** · **NEXT: #724** — browser tests load no theme, so `--cae-*` tokens are undefined and `color-contrast` is vacuous (the harness's whole point); then the rest of the #405 sweep. Carve-out heuristic is **spent** (#718 was the only one) · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
