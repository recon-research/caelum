...
**As of:** 2026-07-15 · main @ `f6f7282` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `b637016`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **tree-select `[filterable]` in-panel filter** #282 (PR #472 — pruned dataSource, keyboard/SR-reachable, live result count, view-only); **galleria RTL inline nav** #288 (PR #467; lightbox → #466); **galleria `[captionPosition]`** #288 (PR #461); **pyxis template sync** →`b637016` (PR #464). **1274 tests** (1205 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `f6f7282`, tree clean, CI green · landed this session: Status reconcile (#471), tree-select `[filterable]` in-panel filter #282 (#472 — full 2-lens review, snapshot/restore simplified away, teeth mutation-proven) · **next = standing menu, pick ONE full window:** galleria numVisible strip-paging #288 [focus-on-page-shift MAJOR, jsdom→M4], tree-select multiple checkbox tri-state #280, chip-set #201-rest/#448, carousel #276 · **process (worked this session):** hand-claim at branch-cut + `receipt` at merge, or `ship_pr` (#255/#469) · verify: `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
