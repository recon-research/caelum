...
**As of:** 2026-07-15 · main @ `927a797` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `5e8dbc0`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **pyxis template sync** →`5e8dbc0` #474 (PR #475 — skill-adherence JIT routing #266, bare-`python` audit #265, skill-count guard #264); **tree-select `[filterable]` filter** #282 (PR #472 — pruned dataSource, view-only); **galleria RTL inline nav** #288 (PR #467; lightbox → #466). **1274 tests** (1205 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `927a797`, tree clean, CI green · landed this session: onboarded (Mode B) + **pyxis template sync b637016→5e8dbc0** #474 (#475 — 3 commits; both new guards mutation-tested + both hook rules fire-tested; upstream report pyxis#268 flags the receipts-tripwire's doc-checkpoint false-positive) · **next = standing menu, pick ONE full window:** galleria numVisible strip-paging #288 [focus-on-page-shift MAJOR, jsdom→M4], tree-select multiple checkbox tri-state #280, chip-set #201-rest/#448, carousel #276 · verify: `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
