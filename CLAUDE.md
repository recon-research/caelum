...
**Resume:** `main` @ `0a71780`, tree clean, CI green · last shipped: **#718** grid table-role split (PR #722, `usd=5.72`) — frame ≠ role holder; axe now runs with **no rules disabled** in both arms, layout-inertness measured not assumed. Harness **#240** (PR #719) stays the entry point: `npm run test:browser`, a pin is **strict** (missing pinned browser FAILS, never falls back) · **no in-flight slice** · **decision queue EMPTY** · **NEXT: continue the #405 real-browser sweep**, starting from the repo's remaining `disableRules` carve-outs · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
