...
**As of:** 2026-07-20 · main @ `9f5ceb2` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **stepper unmet-request re-apply #606** (PR #629 — #608's identity supplies the never-satisfied vs moved-on discriminator); **stepper identity reconcile #608** (PR #626 — filed #624/#627; CONFIRMED #605's repro); **size gate minifies before gzip #603** (PR #621 — 55 rows re-baselined; #586 part 2 refuted). **1447 tests** (1378 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Open decisions:** **#595** router peerDep (blocks #333/#165 `routerLink`) — rec. **A** optional peerDep + 2nd entry point; **#596** per-item icon supply (#333/#165/#150/#149) — rec. **A+B** glyph then template. Both **hard-to-reverse ⇒ parked**. **#549** chip-set text-entry (rec. **A**, PR #554 provisional); **#526** tree per-node state (rec. **on-node**, PR #528 provisional); **#614** commit-subject length (rec. **A retire the `≤72` fiction**). **#623** reach into a 3p private (CDK `_keyManager`)? — rec. **A+B**; blocks #611+#605. All carry a **7-day window**. Prior: **D-428/D-501**. **~128 open issues** = deferral log, not a burndown.

**Resume:** `main` @ `9f5ceb2`, tree clean, CI green · last shipped: stepper unmet-request #606 (PR #629) · **no in-flight slice** · **next = pick ONE:** #627 guard false-positive (rec. B+C), #607 refused-step re-report, chip-set #553 (⚠ item 1 API fork, item 2 browser), splitter #544/#545 (⚠ jsdom-invisible) · ⚠ #611+#605 blocked on **#623**; #602 SSR-gated per D-11; #586 part 2 refuted · #559/#574 **M4-gated** · decision-blocked: #333 (#595/#596), #282/#283 (#526), #549, #418 · verify: `bash scripts/preflight.sh` **with node on PATH** (nvm bin; else format/build/test/lint silently skip & falsely PASS — #538).
