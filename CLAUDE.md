...
**Resume:** `main` @ `ced4774`, tree clean, CI green · **no in-flight slice** · last shipped: **#405** roving trio (PR #760, +25 tests) · **decisions: #735, #744, #757** (provisional, ratify by silence 07-28 → `D-NN`) · **NEXT: the next #405 box** (confirm · table · tree-table · grid) — #413 is parked on #757 · new: **#758** (ship_pr identity doc gap), **#759** (accordion has no arrow roving — parity gap vs Material + PrimeNG) · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**, and re-run it **after** doc edits — a stale PASS reddened #753.
