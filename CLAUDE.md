...
**As of:** 2026-07-15 · main @ `b1421b1` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `12f3608`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **chip-set async empty-focus** #205 (PR #449 — `[emptyFocusTarget]` redirect keyed off `items()` emptying via `afterRenderEffect`; async last-chip drop + move-time ownership re-check); **carousel vertical** #276 (PR #446 — block-axis `translateY` window, Up/Down nav, RTL-independent). **1220 tests** (1151 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `b1421b1`, tree clean, CI green · landed: chip-set async empty-focus #205 (#449), carousel vertical #276 (#446), tree-select #281 (#443) · next = standing menu / next M3 (see **Next**; #276 remainder = swipe/seamless-loop/focus + #445; chip-set #201/#448) · **dev inner loop**: `CI=true npx ng test caelum --filter '<test-name>'` (not `--include`; #414) + `npm run build:lib:fast` (full suite + `build:lib` = DoD + CI gate; #403) · verify: `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS, 1220) · `python3 scripts/check_provenance.py` (GREEN).
