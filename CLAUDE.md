...
**As of:** 2026-07-17 · main @ `69be099` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **template sync** #518 → pyxis `79fdfbe` (PR #520 — +`understand_intent`, receipts tripwire live, #485 closed); **Forge demo LIVE** (#495, PR #516) + **D-428/D-501 + CoC** (PR #515); **file-upload D-04 + theming guard** (PR #512). **1315 tests** (1246 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Open decisions:** none — **D-428** (validator convention, option A) + **D-501** (npm name → scoped `@recon-research/caelum`; rename #514) decided in-session 2026-07-16; CoC adopted (Covenant v2.1); Forge Pages deploy authorized (#495). **~95 open issues** = followups/debt/ideas/slices — a *deferral log*, not a burndown. Two latent forks, filed as `decision` when their slice is picked up: (1) `@angular/router` peerDep for breadcrumb/tab-menu `routerLink`; (2) the data-driven per-item **icon-supply** convention (`string→mat-icon` vs projected-template vs sanitized-SVG), cross-cutting breadcrumb #333 / menubar #150 / tab-menu #165 / split-button #149.

**Resume:** `main` @ `69be099`, tree clean, CI green · this session: onboarded (Mode B, local main was 8 commits behind → ff'd) + **template sync #518** (PR #520; `understand_intent` added 31→32, #485 closed, #519 filed for the not-imported grounding note) + slice_telemetry bare-`python` grant applied #522 (PR #523); owner OK'd self-merge going forward · **next = standing menu, pick ONE:** galleria #288-rest, chip-set #201/#448, carousel #276, tree-select #283 · verify: `bash scripts/preflight.sh` / `.\scripts\preflight.ps1` (PASS 2026-07-17, 14 stages).
