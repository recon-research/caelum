...
**Resume:** `slice/405-grid-recycle-announce` · last shipped: **#405** grid recycling + status-region mechanism (+5 tests) · **decisions: #735, #744, #757** (provisional, ratify by silence 07-28 → `D-NN`) · **NEXT: the next #405 box** (confirm · table · tree-table · grid) — #413 is parked on #757 · new: **#762** (grid #194 born-empty guard defers a CD pass, not a frame — needs real-SR call), #758, #759 · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**, and re-run it **after** doc edits — a stale PASS reddened #753.
