...
**As of:** 2026-07-10 · main @ `323efe6` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail**. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 input family + the ★ media cluster + the Splitter family complete. Just landed: **cae-breadcrumb #332** (PR #334) — the navigation-trail component (`p-breadcrumb` parity, Book 09 §3.4): a semantic `<nav>`+`<ol>` with `aria-current="page"` on the current page (non-link) and silent `aria-hidden` token separators, **no overlay/CDK** (Angular core only); data-driven `[items]`/`[home]` unified into one trail. 2-lens review fixed 2 MAJORs — CSS `::before` separators ARE announced by Chromium/WebKit → silent `aria-hidden` spans (also removed a `JSON.stringify`-into-CSS surface); demo SPA-teardown on real `href` → fragment urls. **913 tests** (849 caelum + 64 forge); all gates GREEN. Deferred → **#333**.

**Next:** M3 continues per **D-18** — pick the next slice (natural candidate: the Book 11 §3.3 drag-drop cluster **OrderList/PickList/FileUpload** Build-M; needs `plan_work` + ticket). Standing follow-ons: breadcrumb #333, scroll-panel #329, splitter #325, image-compare #318, input-family #305/#309/#312/#315, demo heading-order #306, galleria #286[#240-gated]/#287/#288/#289, tree-select #280–#283, carousel #276/#290, tree-table #263–#270, cae-table cluster, grid cluster, chip-set #201/#205/#206, test-infra #240 (M4).

**Open decisions:** none (D-16/D-17 ratified-by-silence, D-18 human-decided 2026-07-07). **89 open issues** = 0 decisions + 89 followups/debt/ideas/slices.

**Resume:** `main` @ `323efe6`, tree clean, CI green · **cae-breadcrumb #332 landed** (PR #334 merged; deferred → #333) · next action = **pick the next M3 slice** — `plan_work` on the Book 11 §3.3 drag-drop cluster (OrderList/PickList/FileUpload), then build; or take any standing follow-on · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (913 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
