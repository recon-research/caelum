...
**As of:** 2026-07-25 · main @ `3f1efd1` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑** (grid gap → editor #232 / charts #233; niche widgets #667 → all on-demand per D-18) · **M4 ◐ Parity hardening & adoption** (density R4 #411 · a11y Layer 1 axe #690+#691 · real-browser harness #240 · VR suite ☑ #732) · template @ pyxis `79fdfbe`. Full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M4 active**. Shipped: density R4 #411 · a11y Layer 1 axe #690+#691 · 07-24 hardening batch (PRs #697–#705) · adoption track (#713, #716) · **harness #240** (PR #719) · **#405 sweep 10/12** (latest PR #767: confirm #107 + table #223/#254) · **VR suite ☑ #732** (PRs #737/#742; goldens agree across dev box, container and CI runner) · **#734 ◐** — #425 semantic text contrast fixed + pinned (PR #746), #745 status palette under VR (PR #748), **#510** last 12 `--mat-sys-*` → `--cae-*` + bridge map pinned (PR #753). **1909 tests** (caelum 1674 + forge 71 + browser 125 + VR 24 + scripts 15); gates GREEN.

**Next:** **#405 sweep** is nearly closed — only **tree-table #263** is workable; **#228** stays open by design (needs a real screen reader). Work-list heuristic = grep component prose for real-browser deferrals, and where a jsdom spec's header admits it asserts *component state* not the DOM. Then **#734** theming polish (◐ — #425 + #510 done; **#413** remains, and it needs a `decision`) · **#733** capability ledger (M4 exit criterion; carries a derived-vs-curated fork that may need a `D-NN`). Recipes → `docs/PATTERNS.md` §9–§9g/§12. Queued extras: **#709** cae-drawer · #710 cae-alert · #711 cae-panel/fieldset · #741 · #648(opt3) · #585.

**Open decisions:** **#735** VR golden portability (prov. Opt 1, pin the render) · **#744** semantic status colours (prov. Opt 2, darken light arms; shipped PR #746) · **#757** compile-time density baseline for #413 (prov. Opt 1, close it — measured) — all three ratify by silence **2026-07-28** → `D-NN`; each ticket carries the fork + grounding. Recently decided: **D-595** router = optional peerDep · **D-596** per-item icons · **D-623** guarded 3p-private reach · **D-526** tree per-node state · **D-549** tag-entry → #120 · **D-614** no commit-subject gate · **D-652** grid optional-peer split. Prior: **D-428/D-501**. **~130 open issues** = deferral log, not a burndown.

**Resume:** `main` @ `3f1efd1`, tree clean, CI green · **no in-flight slice** · last shipped: **#405** confirm focus + sticky occlusion (PR #767) · **decisions: #735, #744, #757** (provisional, ratify by silence 07-28 → `D-NN`) · **NEXT: #405 tree-table #263**, the last workable box — then #734 / #733; #413 is parked on #757 · new: **#765** (modal doesn't trap focus until its open animation ends), **#766** (no dev-warn for a missing `scroll-padding-block-start`), #762, #758, #759 · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE** (a pipe to `tail` returns the pipe's), and re-run it **after** doc edits — a stale PASS reddened #753.
