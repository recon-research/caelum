...
**As of:** 2026-07-20 · main @ `d4021f3` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — ★ families COMPLETE (media, tree, input, Splitter, drag-drop); **standing menu** is the lane, with early-theme density pulled forward from M4. Recently landed: **D-596 icons #644** (PR #646 — `caelum/icon` glyph registry + per-item `icon`/`iconTemplate` on breadcrumb/tab-menu/menu/split-button + primary icon; completion → #645); before that the stepper cluster #611/#605 (PR #640; #639 upstream report still needs approval). **1465 tests** (1396 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
