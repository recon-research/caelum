...
**As of:** 2026-07-16 · main @ `38e83da` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `d04440a`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **galleria `[fullScreen]` + `[(visible)]`** overlay mode #488 (PR #502 — overlay-only mode over the shared lightbox, two-way open-state model, empty-async guard, 2-lens SHIP; edges → #500); **template sync** `5e8dbc0→d04440a` #484 (PR #486); **galleria `[numVisible]`** windowing #481 (PR #482). **1315 tests** (1246 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `38e83da`, tree clean, CI green · landed this session: onboarded (Mode B) + **template sync** #484 (PR #486) + **galleria `[fullScreen]`+`[(visible)]`** overlay mode #488 (PR #502, usd=19.31 — overlay-only over the shared lightbox, two-way open-state model, empty-async guard, 2-lens SHIP no MAJOR; a11y group-suppression + docs folded; close→reopen + destroy-visible edges → #500) · **next = standing menu, pick ONE:** galleria #288-rest (nav-split, native-scroll↔paging fork), chip-set #201-rest/#448, carousel #276, tree-select #283 · verify: `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
