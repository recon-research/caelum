...
**As of:** 2026-07-16 · main @ `dfd22e3` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `d04440a`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **file-upload D-04 fix + theming-scan guard** #496/#498/#504 (PR #512 — 3-lens adversarial, gate live in preflight+CI); **2026-07-16 review** (→ #489–#511): front door #492–#494 (PR #506) + Windows hardening #489/#490/#503 (PR #507); galleria fullScreen #488 (PR #502). **1315 tests** (1246 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Open decisions:** none — **D-428** (validator convention, option A) + **D-501** (npm name → scoped `@recon-research/caelum`; rename #514) decided in-session 2026-07-16; CoC adopted (Covenant v2.1); Forge Pages deploy authorized (#495). **~97 open issues** = 1 decision + followups/debt/ideas/slices — a *deferral log*, not a burndown. Two latent forks, filed as `decision` when their slice is picked up: (1) `@angular/router` peerDep for breadcrumb/tab-menu `routerLink`; (2) the data-driven per-item **icon-supply** convention (`string→mat-icon` vs projected-template vs sanitized-SVG), cross-cutting breadcrumb #333 / menubar #150 / tab-menu #165 / split-button #149.

**Resume:** `main` @ `ea32907`, tree clean, CI green · **dev box = Windows 10 laptop**: both preflights PASS natively (python3-shim gotcha → conventions › Toolchain) · two writers today (galleria + review lanes, protocol held) · review leftovers: cae-tree #491, Pages #495, nits #497, notes #499, #509–#511 · pyxis reflux reports filed: pyxis#287–#289 · **next = standing menu, pick ONE:** galleria #288-rest, chip-set #201/#448, carousel #276, tree-select #283 · verify: `bash scripts/preflight.sh` / `.\scripts\preflight.ps1` (PASS 2026-07-16, 14 stages incl. theming scan).
