...
**As of:** 2026-07-18 · main @ `b3cd80b` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **chip-set textEntry a11y fixes** (PR #557 — keyboard trap #550 partial + focus grab **#551 CLOSED**; **Provisional on #549**); chip-set #201 `[textEntry]` (PR #554; #201 now listbox-only). **1370 tests** (1301 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `b3cd80b`, tree clean, CI green · last shipped: chip-set #550/#551 focus fixes (PR #557; #551 closed, #550 browser half open) · **next = standing menu, pick ONE:** carousel #276 swipe (RTL/responsive/vertical already shipped), breadcrumb #333 extras, splitter slop #544/#545, chip-set #552/#553/#556 · ⚠ decision-blocked: tree-select #282/#283 (#526), chip-set listbox (#549), splitter px #418 · verify: `bash scripts/preflight.sh` **with node on PATH** (nvm bin; else format/build/test/lint silently skip & falsely PASS — #538).
