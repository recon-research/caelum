...
**As of:** 2026-07-12 · main @ `a37f329` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail**. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 input family + ★ media cluster + Splitter family + navigation set complete. **Book 11 §3.3 drag-drop cluster COMPLETE** (OrderList ☑ · PickList ☑ · FileUpload ☑); now **closing its deferred parity extras**. Done: multi-select pair (order-list #341 / pick-list #342) + file-upload request-config #345 + **pick-list within-list reorder #342** (PR #356) + **pick-list header slots #342** (PR #358) + **order-list in-list filter #341** (PR #360 — `[filter]` + type-safe `[filterMatch]` predicate, labelled `type=search` box, empty-state + announced count, reorder disabled while filtering; filtered view is `value()` by-ref when blank → zero-cost; 2-lens no MAJOR). #341/#342/#345 stay open for remaining extras. **1026 tests** (962 caelum + 64 forge); all gates GREEN.

**Next:** continue **closing drag-drop parity extras** — recommended **pick-list #342 in-list filter** (mirror the order-list filter into each pane; then remaining #342 RTL), remaining **order-list #341** (per-item disabled, dataKey, RTL), remaining **file-upload #345** (thumbnails, chunked, concurrent, paste/folder, row-template, method). Standing followups: focus-restore #350, forge initial-bundle budget #354. Then the standing menu (parity-extras/debt on shipped components): breadcrumb #333, scroll-panel #329, splitter #325, image-compare #318, input-family #305/#309/#312/#315, demo heading-order #306, galleria #287/#288/#289 [#286 #240-gated], tree-select #280–#283, carousel #276/#290, tree-table #263–#270, cae-table cluster, grid cluster, chip-set #201/#205/#206, test-infra #240 (M4).
...
**Resume:** `main` @ `a37f329`, tree clean, CI green · **order-list in-list filter #341 landed** (PR #360 merged) · next action = **continue closing drag-drop extras** — recommended **pick-list #342 in-list filter** (mirror the order-list filter — per-side query signal + filtered view, transfer acts on the selection) or pick from the standing menu · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (1026 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
