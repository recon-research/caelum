...
**As of:** 2026-07-25 · main @ `919afed` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑** (grid gap → editor #232 / charts #233; niche widgets #667 → all on-demand per D-18) · **M4 ◐ Parity hardening & adoption** (density R4 #411 · a11y Layer 1 axe #690+#691 · real-browser harness #240 · VR suite ☑ #732) · template @ pyxis `79fdfbe`. Full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M4 active**. Shipped: density R4 #411 · a11y Layer 1 axe #690+#691 · 07-24 hardening batch (PRs #697–#705) · adoption track (#713, #716) · **harness #240** (PR #719) · **#405 sweep 6/12** (PRs #722/#726/#728/#730; roving-tabindex trio = tree #41 · listbox · accordion #79) · **VR suite ☑ #732** (PRs #737/#742; goldens agree across dev box, container and CI runner) · **#734 ◐** — #425 semantic text contrast fixed + pinned (PR #746), #745 status palette under VR (PR #748), **#510** last 12 `--mat-sys-*` → `--cae-*` + bridge map pinned (PR #753). **1886 tests** (caelum 1674 + forge 71 + browser 102 + VR 24 + scripts 15); gates GREEN.

**Next:** **#405 sweep** is the open track — remaining: confirm · table(×3) · tree-table · grid(×2); work-list heuristics = grep component prose for real-browser deferrals, and where a jsdom spec's header admits it asserts *component state* not the DOM. Then **#734** theming polish (◐ — #425 + #510 done; **#413** remains, and it needs a `decision`) · **#733** capability ledger (M4 exit criterion; carries a derived-vs-curated fork that may need a `D-NN`). Recipes → `docs/PATTERNS.md` §9–§9e/§12. Queued extras: **#709** cae-drawer · #710 cae-alert · #711 cae-panel/fieldset · #741 · #648(opt3) · #585.
...
**Resume:** `slice/405-roving-tabindex` · last shipped: **#405** roving-tabindex trio — tree/listbox/accordion browser specs (+25 tests) · **decisions: #735, #744, #757** (provisional, ratify by silence 07-28 → `D-NN`) · **NEXT: the next #405 box** (confirm · table · tree-table · grid) — #413 is parked on #757 · new: **#758** (ship_pr identity doc gap), **#759** (accordion has no arrow roving — parity gap vs Material + PrimeNG) · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**, and re-run it **after** doc edits — a stale PASS reddened #753.
