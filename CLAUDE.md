...
**As of:** 2026-07-08 · main @ `5e28d1f` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand tracks per D-18) · **M3 ◐ Build-S/M long tail** · ops template synced from pyxis → `b263b94` (#245). Recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md); full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `5e28d1f`, tree clean · next action = **pick the next slice** — recommended: open the **M3 ★ team-priority track** (TreeSelect / Carousel, ticket-first) now that cae-table #144 is closed; or drain cae-table follow-ups (#251 sticky footer is the natural next one — it unblocks a footer content model). a11y/API-touching slices → 2-lens review · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (570 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS 0-skipped).
