...
**As of:** 2026-07-09 · main @ `cdea70d` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand tracks per D-18) · **M3 ◐ Build-S/M long tail** · ops template synced from pyxis → `dfd6b47` (#259). Recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md); full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Where we are:** the grid is a swappable-engine adapter (client / TanStack / server) behind one neutral `CaeGridAdapter` port (#176/#188/#189/#192/#194). **cae-table's #144 umbrella is closed** (templates #143, row-index #213, multi/single-select #144/#224, expandable rows #243, sticky header+columns #256). Newest: **cae-tree-table #262** (PR #271) — a hierarchical `role=treegrid` over MatTable (flattened-tree, expansion-as-view-state, row-level roving keyboard, tree-aware `caeTreeCellDef`, new `caelum/tree-table` entry point + @defer'd Forge demo); its 2-lens review caught a keydown-hijack on interactive cell content (WCAG 2.1.1) and a treegrid caption-naming gap (→ `aria-labelledby`). Also cae-chip-set (#84/#202). **601 tests** (537 caelum + 64 forge); provenance GREEN.

**Next:** M3 active per **D-18**; ★ team-priority leads = media Carousel/Galleria and **TreeSelect** (`cae-tree-select`, the treegrid's form cousin — reuses the tree node model). Independent work: cae-tree-table follow-ups (#264 selection, #265 sort/filter/paginate, #266 lazy+dataKey, #267 sticky, #268 2-D cell roving, #269 virtual-scroll, #270 malformed-input, #263 M4 verify), cae-table follow-ups (#145/#251/#235/#236/#223/#241/#242/#253/#254/#255), grid follow-ups (#193/#175/#177/#190/#174/#178/#179/#216/#228), cae-chip-set (#201/#205/#206), test-infra #240 (M4 browser runner), editor #232 / charts #233 on demand.

**Open decisions:** none — #142 → **D-16** (Forge warn-budget) and #168 → **D-17** (grid public-API shape) ratified by silence 2026-07-07; #185 → **D-18** (M2 scope: grid-only, advance to M3) decided by the human 2026-07-07. **70 open issues** = 0 decisions + 70 followups/debt/ideas.

**Resume:** `main` @ `cdea70d`, tree clean · **cae-tree-table #262 landed** (PR #271; `caelum/tree-table` entry point, @defer'd Forge demo, 601 tests; follow-ups #263–#270 filed) · next action = **pick the next M3 slice** — recommended: **TreeSelect** (`cae-tree-select`, reuses the tree node model + overlay patterns) or **Carousel/Galleria** (★ media), ticket-first. a11y/API-touching slices → 2-lens review · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (601 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS 0-skipped).
