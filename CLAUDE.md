...
**Where we are:** **M4 active**. Shipped: density R4 #411 · a11y Layer 1 axe #690+#691 · 07-24 hardening batch (PRs #697–#705) · adoption track (#713, #716) · **harness #240** (PR #719) · **#405 sweep 3/12** (PRs #722/#726/#728/#730) · **VR suite ☑ #732** (PRs #737/#742; goldens agree across dev box, container and CI runner) · **#734 ◐** — #425 semantic text contrast fixed + pinned (PR #746), #745 status palette under VR (PR #748), **#510** last 12 `--mat-sys-*` → `--cae-*` + bridge map pinned (PR #753). **1861 tests** (caelum 1674 + forge 71 + browser 77 + VR 24 + scripts 15); gates GREEN.

**Next:** **#405 sweep** is the open track — remaining: tree · accordion · confirm · table(×3) · tree-table · grid(×2) · listbox; work-list heuristics = grep component prose for real-browser deferrals, and where a jsdom spec's header admits it asserts *component state* not the DOM. Then **#734** theming polish (◐ — #425 + #510 done; **#413** remains, and it needs a `decision`) · **#733** capability ledger (M4 exit criterion; carries a derived-vs-curated fork that may need a `D-NN`). Recipes → `docs/PATTERNS.md` §9–§9d/§12. Queued extras: **#709** cae-drawer · #710 cae-alert · #711 cae-panel/fieldset · #741 · #648(opt3) · #585.
...
**Resume:** `main` @ `919afed`, tree clean, CI green · last shipped: **#510 `--mat-sys-*` migration + bridge pinned** (PR #753) · **no in-flight slice** · **decision queue: #735, #744** (both provisional, ratify by silence 07-28 → record as `D-NN`) · **NEXT: #413** (compile-time density baseline — semi-public Sass API, so **file a `decision` first, don't guess**), or the next **#405** box · new: **#752** VR blind spot (overlay panels sit outside `renderArm`'s capture box) · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**, and re-run it **after** doc edits — a stale PASS cost a red `static gates` on PR #753.
