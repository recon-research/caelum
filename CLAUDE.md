...
**Next:** continue **M1 Composed** — remaining canonical slices: **Menubar** (MatToolbar+MatMenu), **ContextMenu** (CDK Menu `cdkContextMenuTriggerFor`), **TabMenu** (`mat-tab-nav-bar`); plus **#84** cae-chip-set (managed removable-chip list, roving keyboard — a filed follow-up, not on the M1 canonical list). Follow-ups — cae-split-button (**#149** button-side extras / **#150** rich menu items), cae-table (**#143** custom cell templates / **#144** sticky-expandable-selection / **#145** server-side-filter-reorder), cae-multi-select (#137 removable trigger chips, **#138** accessible filter → flip `filterable` on, #139 parity extras). **#97** (split Forge's monolithic app.scss) must land before the next EAGER styled Forge demo (recent demos are `@defer`'d with lean styles, so the 8 kB anyComponentStyle error hasn't tripped).

**Decisions:** `D-01..D-15` resolved (ARCHITECTURE Appendix A); **1 open `decision` #142** — Forge initial-bundle warn-budget policy (proceeding **provisionally** on the 850→875 kB raise; reversible; silence ratifies). Open followups/debt/ideas: **39 open** — `gh issue list` (incl. #84/#97/#149/#150/#143/#144/#145/#137/#138/#139 + the M4 real-browser verify family #41/#79/#91/#107/#110/#128 + #8/#12/#15/#33/#48/#51/#54/#61/#72/#75/#78/#80/#90/#102/#105/#111/#116/#117/#120/#127/#129/#133).

**Resume:** `main` @ `1bc5bb8` · next = pick the next **M1 Composed** slice (candidates in **Next**); docs current, truth = tracker. **Flow** (details: [`PROJECT_CONVENTIONS.md`](PROJECT_CONVENTIONS.md) › Merge policy / Tracker): ticket-first → `slice/<n>-<slug>` branch → PR → green CI → merge → merge-time checkpoint; every public-API/a11y slice gets a **4-lens ultracode adversarial review** before merge. **Verify:** `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (**339 pass — 299 lib + 40 forge**; `ng test` alone runs Forge only → also run `ng test caelum`) · `python3 scripts/check_provenance.py` (GREEN) · `bash scripts/preflight.sh` (PASS 0-skipped).
