...
**As of:** 2026-07-24 · main @ `2416bfe` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑** (grid gap → editor #232 / charts #233; niche widgets #667 → all on-demand per D-18) · **M4 ◐ Parity hardening & adoption** (density R4 #411 · a11y Layer 1 axe #690+#691 · real-browser harness #240 complete) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M4 active** (parity hardening & adoption). Shipped: density R4 #411 · a11y Layer 1 axe #690+#691 · 07-24 hardening batch (PRs #697–#705) · adoption track (#713, #716) · **real-browser harness #240** (PR #719 — Vitest browser mode + Playwright, dev-tier; `*.browser.spec.ts` split from the jsdom target). **#405 sweep OPEN, 3/12 boxes:** ☑ #718 grid `role=table` off the visual frame · ☑ #724 real token layer · ☑ divider #91 + toolbar #128 layout (PR #728) · ☑ slider #110 (PR #730 — severing its `[value]` binding leaves all 1674 jsdom tests green). **1789 tests** (caelum 1674 + forge 71 + browser 34 + scripts 10); gates GREEN.

**Next:** **work the #405 sweep** — remaining boxes: tree · accordion · confirm · table(×3) · tree-table · grid(×2) · listbox. Two work-list heuristics: grep components' prose for real-browser deferrals (galleria/listbox/table still carry one — richer than the checklist, which had missed listbox); and where a jsdom spec's header says it asserts *component state* not the DOM, that delta is the browser spec's job. Recipes → `docs/PATTERNS.md` §9/§9a/§12. Then: theming polish (density × scheme) · visual-regression suite · capability ledger green. Queued extras: **#709** cae-drawer · #710 cae-alert · #711 cae-panel/fieldset · #648(opt3) · #585.
...
**Resume:** `main` @ `2416bfe`, tree clean, CI green · last shipped: **#405** slider #110 (PR #730 — real input via `userEvent` from `vitest/browser`); before it divider+toolbar (PR #728), **#724** (PR #726), **#718** (PR #722) · **no in-flight slice** · **decision queue EMPTY** · **NEXT: the next #405 box** — tree/accordion/confirm/table/tree-table/listbox; galleria/listbox/table carry unverified prose claims · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**.
