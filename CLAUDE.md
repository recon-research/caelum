...
**As of:** 2026-07-14 · main @ `1762819` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `12f3608`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed on **carousel #276**: **`responsiveOptions`** (PR #433 — window by breakpoint) + **inline-axis RTL** (PR #435 — mirrored track+arrows; vertical/swipe/loop deferred); plus **splitter persist-collapse** #399 (PR #431 — reload-safe), **`caeMaskComplete`** validator #315 (PR #429 — first `ValidatorFn`; #428 provisional). **1200 tests** (1131 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `1762819`, tree clean, CI green · landed: carousel #276 (PR #433 responsive + #435 RTL), splitter #399, `caeMaskComplete` #315 (#428) · next = standing menu / next M3 (see **Next**; #276 remainder = vertical/swipe/loop/focus) · **dev inner loop**: `CI=true npx ng test caelum --filter '<test-name>'` (not `--include`; #414) + `npm run build:lib:fast` (full suite + `build:lib` = DoD + CI gate; #403) · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (1200 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
