...
**As of:** 2026-07-28 · main @ `9bfe34e` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑** (grid gap → editor #232 / charts #233; niche widgets #667 → all on-demand per D-18) · **M4 ◐ — exit clauses 1 ☑ · 2 ☐ · 3 ☑**; clause 2's blocker #809 is fixed, but the re-reviews it commissioned FAILED 3 components (#823/#824/#825) · template @ pyxis `79fdfbe`. Full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M4 active**. Shipped: density R4 #411 · a11y axe #690+#691 · hardening (PRs #697–#705) · adoption #713/#716 · harness #240 · VR ☑ #732 · ledger ☑ #733/#773 · #785 · #765 · #759 · #741 · theming ☑ #734/#413 (D-757) · **☑ #808+#810** clause 1 mechanized · **☑ #809** the ledger now verifies each quoted review against its commit and re-keys 13 rows off a packaging slice; `paths` is shape-constrained, `revoked` records a sign-off a later review took back. **1967 tests**; gates GREEN.

**Next:** the #809 re-reviews found **6 HIGH** across three entry points, so clause 2 is now **61/64** and honest: **#823** cae-rating (keyboard follows value not focus; a value above `[stars]` strands every tab stop) · **#825** cae-confirm (`confirmAt()` leaks overlay + backdrop on destroy/navigation) — both decision-free, take either first · **#824** cae-popover (3 HIGH) is **blocked on #826**. Each fix needs its OWN two-party review to regain sign-off. Then #811/#812/#813.

**Open decisions:** **none** — #826 answered 2026-07-28 and closed as **D-826: `cae-popover` goes modal** (add `aria-modal`, keep `role="dialog"` + trap + backdrop), which unblocks #824 and makes its fix *hold the trap* (`tabindex="0"`) rather than remove it. Two component-scoped forks in #823 were decided in the same survey — `writeValue` **echoes** the normalized value back to the form (deferred), and `[icon]` off-state gets a **non-colour cue** — both recorded on the ticket, both overruling the recommended default. D-735/744/757/771/791/826 live in ARCHITECTURE Appendix A. **~139 open issues** = deferral log, not a burndown.

**Resume:** `main` @ `9bfe34e`, tree clean, CI green · **no in-flight slice** · last shipped: **#809** (PR #827) — ledger **61/64**, three sign-offs `revoked` (pointer + quote kept; "reviewed and failed" ≠ "never reviewed") · next: **#825** cae-confirm (app-breaking overlay leak), then **#823** cae-rating, then **#824** cae-popover per D-826 — each fix owes its OWN two-party review to regain sign-off · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**; a Linux preflight cannot see a Windows-only break (#814).
