...
**As of:** 2026-07-18 · main @ `b0d1c7a` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **NaN-index guard #580** — a bound `NaN`/fraction left carousel+galleria with zero tab stops (WCAG 2.1.1); grew to carousel's `numVisible`/`numScroll`, which poison the clamp *bound* (PR #593 — filed #591/#592); **modifier-chord sweep #581** (PR #587); **galleria #572 + #573** (PR #582). **1413 tests** (1344 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Next:** continue the **standing menu** or start the next M3 component. Candidates: tree-select leaf-only #282 (checkbox×propagation pass) / scale #283, chip-set #201 text-entry+listbox / #448 concurrent-async (departure-diff), carousel #276 loop / #559 / #571 / #574, breadcrumb #333 extras (per-item icons = cross-cutting **icon-supply** fork; routerLink [`@angular/router` peerDep fork]), splitter px #418 (⚠ px-unit API decision — file when picked), image-compare #422, input-family #305/#312/#315, tree-table #264–#270, galleria lightbox zoom #289 (dep #275), splitter slop #544/#545, chip-set residuals #550 (browser half) / #552/#553/#556, test-infra #240 (M4).

**Open decisions:** **#549** — chip-set text-entry shape (`[textEntry]` mode vs a `cae-chips` CVA control); recommended **A** (stepping stone), PR #554 provisional. **#526** — tree per-node state (on-node vs accessors); recommended **on-node**, PR #528 provisional. Both reversible; **7-day objection window stated on each 2026-07-18** (both were filed without one, so "silence ratifies" could never fire). Prior: **D-428/D-501** 2026-07-16; CoC adopted; Forge Pages (#495). **~117 open issues** = deferral log, not a burndown. Latent forks, filed when their slice lands: (1) `@angular/router` peerDep (breadcrumb/tab-menu `routerLink`); (2) per-item **icon-supply** convention (#333/#150/#165/#149).

**Resume:** `main` @ `b0d1c7a`, tree clean, CI green · last shipped: NaN-index guard #580 (PR #593) · **no in-flight slice** · **next = standing menu, pick ONE:** breadcrumb #333 extras, splitter slop #544/#545, chip-set #552/#553/#556, tree-select combobox #585, tabs/stepper NaN passthrough #592 (verify-or-close), size-budget drift #586, commit-keyword guard #591 · #559/#574 are **M4-gated** (jsdom can't model `inert` blur or SR output) · ⚠ decision-blocked: tree-select #282/#283 (#526), chip-set listbox (#549), splitter px #418 · verify: `bash scripts/preflight.sh` **with node on PATH** (nvm bin; else format/build/test/lint silently skip & falsely PASS — #538).
