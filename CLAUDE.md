...
**As of:** 2026-07-28 · main @ `a7589e3` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑** (grid gap → editor #232 / charts #233; niche widgets #667 → all on-demand per D-18) · **M4 ◐ — exit clauses 1 ☑ · 2 ☐ · 3 ☑**; clause 2 is **63/64** — #825 and #823 won their rows back; only #824 owes one · template @ pyxis `79fdfbe`. Full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M4 active**. Shipped: density R4 #411 · a11y axe #690+#691 · hardening (PRs #697–#705) · adoption #713/#716 · harness #240 · VR ☑ #732 · ledger ☑ #733/#773 · #785 · #765 · #759 · #741 · theming ☑ #734/#413 (D-757) · **☑ #808+#810** clause 1 mechanized · **☑ #809** the ledger verifies each quoted review against its commit; `revoked` records a withdrawn sign-off · **☑ #825** (D-831) · **☑ #823** — each fix's own review found HIGHs in the fix itself. **2007 tests**; gates GREEN.

**Next:** clause 2 is **63/64** — only **#824** cae-popover (3 HIGH) still owes a sign-off, and it is unblocked: per **D-826** the fix must *hold* the focus trap (`tabindex="0"`, so CDK's `_getFirstTabbableElement` returns the panel and both anchors wrap), not remove it; its spec's `panel()` oracle also cannot see a leaked pane. Then #811/#812/#813, plus #832 (anchored `aria-modal` vs `reposition()`) and #836 (nameless radiogroup).

**Open decisions:** **none** — **D-831** (#831): `confirmAt` takes a **required** third `destroyRef`. D-826 widened to `cae-confirm-popup`, grounding **corrected** (`MatDialog` ships `aria-modal="false"`). #823's contracts shipped; its review forced two more owner calls, on the ticket: the `writeValue` echo is **shape only, never range**, and numeric strings **coerce**. D-735/744/757/771/791/826/831 live in ARCHITECTURE Appendix A. **~140 open issues** = deferral log.

**Resume:** `main` @ `a7589e3`, tree clean, CI green · **no in-flight slice** · last shipped: **#823** (PRs #837/#838) — ledger **63/64**; two revoked rows have now been *earned back*, so the recovery path is demonstrated · next: **#824** per D-826, which owes its OWN two-party review · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE** (a wrapper's exit can mask it); a Linux preflight cannot see a Windows-only break (#814).
