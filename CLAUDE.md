...
**Open decisions:** **#841** — where focus goes when a popover's trigger is removed while the panel is open. Non-blocking (rare path; an `isConnected` guard was tried and DELETED as provably inert). Standing: **D-849** (parity before publish) · **D-850** (first release is `0.1.0` — so breaking changes stay *minor* bumps until a deliberate 1.0.0; grounded in D-831 breaking a shipped signature during M4's exit week). **153 open issues** = deferral log.
