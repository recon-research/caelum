...
**As of:** 2026-07-11 · main @ `3f2dc0f` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail**. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 input family + ★ media cluster + Splitter family + navigation set complete. **Book 11 §3.3 drag-drop cluster COMPLETE: OrderList ☑ · PickList ☑ · FileUpload ☑.** Just landed: **cae-file-upload #338** (PR #346) — keyboard-reachable native `<input type=file>` + native `DataTransfer` dropzone (**not** `cdkDropList` — OS file drops aren't `cdkDrag` items, so an inert directive would fail liveness; native drop still satisfies every §3.5 leg), **trust-boundary type/size validation on every intake path — input, drop, and `writeValue` — rejecting before upload**, `HttpClient` `reportProgress` per-file progress/cancel/retry, controlled CVA (value = accepted `File[]`), #47 ariaDescribedby, `LiveAnnouncer`, and per-row focus management so Cancel↔Retry swap + row removal never strand focus to `<body>` (WCAG 2.4.3). 2-lens review (a11y + security/validation): **2 MAJOR fixed** (writeValue validation hole; per-row focus stranding) + 3 MINOR (subs-set-after-subscribe, negative maxFileSize → no-limit, not-configured announce). **970 tests** (906 caelum + 64 forge); all gates GREEN. Deferred → **#345**.

**Next:** drag-drop cluster done, no in-flight slice → **pick a standing follow-on** (all parity-extras/debt on shipped components): file-upload #345, pick-list #342, order-list #341, breadcrumb #333, scroll-panel #329, splitter #325, image-compare #318, input-family #305/#309/#312/#315, demo heading-order #306, galleria #286[#240-gated]/#287/#288/#289, tree-select #280–#283, carousel #276/#290, tree-table #263–#270, cae-table cluster, grid cluster, chip-set #201/#205/#206, test-infra #240 (M4).
...
**Resume:** `main` @ `3f2dc0f`, tree clean, CI green · **cae-file-upload #338 landed** (PR #346 merged; **Book 11 §3.3 drag-drop cluster COMPLETE**; deferrals → #345) · next action = **pick a standing follow-on** (no in-flight slice) · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (970 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
