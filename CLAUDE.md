...
**Open decisions:** **#549** chip-set text-entry (rec. **A**, PR #554 provisional); **#526** tree per-node state (rec. **on-node**, PR #528 provisional) — both ratify by silence **2026-07-25**; **#614** commit-subject length (rec. **A retire the `≤72` fiction**). Just decided (2026-07-20, in-session survey): **D-595** router = optional peerDep + 2nd entry point · **D-596** per-item icons = `icon` glyph **and** `iconTemplate` (template wins) · **D-623** guarded 3p-private reach + upstream, the general rule — unblocks #611/#605. #624 answered **B** (status quo). Prior: **D-428/D-501**. **~126 open issues** = deferral log, not a burndown.
