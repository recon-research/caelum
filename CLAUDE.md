...
**As of:** 2026-07-22 · main @ `692dd03` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — ★ families COMPLETE (media, tree, input, Splitter, drag-drop); **standing menu** is the lane, with early-theme density pulled forward from M4. Recently landed: **D-596 icons COMPLETE** — #644 (PR #646, the four-component rollout + `caelum/icon` registry) then #645 (PR #650, `cae-context-menu` slot + `cae-menubar` forward; review caught a real CDK typeahead poisoning → pinned `cdkMenuitemTypeaheadLabel`, MatMenu half filed #648); before that the stepper cluster #611/#605 (PR #640; #639 upstream report still needs approval). **1471 tests** (1402 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Open decisions:** **none** — the queue is empty for the first time since go-live. Decided 2026-07-20 (in-session surveys): **D-595** router = optional peerDep + 2nd entry point · **D-596** per-item icons = `icon` glyph **and** `iconTemplate` (template wins) · **D-623** guarded 3p-private reach + upstream, as the general rule · **D-526** tree per-node state = on-node fields (ratified as shipped) · **D-549** tag-entry form case → `cae-autocomplete` #120, **not** a new `cae-chips` (`p-chips` was REMOVED in PrimeNG v20-rc) · **D-614** no commit-subject length gate. #624 answered **B** (status quo). Prior: **D-428/D-501**. **~120 open issues** = deferral log, not a burndown.

**Resume:** `main` @ `692dd03`, tree clean, CI green · last shipped: icon completion #645 (PR #650) · **no in-flight slice** · **decision queue EMPTY** · **next = pick ONE:** #333/#165/#150 **routerLink entry points** (D-595, the remaining big unlock), #120 tag-entry CVA (D-549), #639 upstream post (**owner approval**), #648 MatMenu typeahead (may want a D-623-shaped `decision`), chip-set #553, splitter #544/#545, carousel #276 · #602 SSR-gated (D-11) · #559/#574 **M4-gated** · verify: `bash scripts/preflight.sh` **with node on PATH** (else gates falsely PASS — #538) **and read the EXIT CODE, not the test count**.
