...
**Resume:** `main` @ `0d532fb`, tree clean, CI green · last shipped: #456 splitter touch-slop derivation (PR #547; **#456 CLOSED** — sweep complete) · **next = standing menu, pick ONE:** tree-select #282/#283 (avoid while #526 window open), chip-set #201 text-entry, carousel #276 swipe/loop, splitter px #418 (⚠ px-unit API decision — file when picked), breadcrumb #333 extras, splitter slop follow-ons #544/#545 · verify: `bash scripts/preflight.sh` **with node on PATH** (nvm bin; else format/build/test/lint silently skip & falsely PASS — #538).
