...
**As of:** 2026-07-29 · main @ `bcce718` · CI green (full matrix) · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑ · M3 ☑ · M4 ☑ · M5 ◐** · template @ pyxis `79fdfbe`. Full slice index → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** **M5** (**D-849**) — close the five parity gaps that were never a decision to defer, *then* publish. The other 14 `☐` COMPARISON rows are policy, not debt (#712/#667 on-demand lists, #232/#233 D-18 adapter cuts). **4 of 5 done:** ☑ #709 `cae-drawer` (PR #859) · ☑ #710 `cae-alert` (PR #867) · ☑ #711 `cae-panel`/`cae-fieldset` (PR #872) · ☑ **#150 `cae-menu` submenus** (PR #876; #150 stays OPEN for its non-parity `command`/`routerLink` bullets). Ledger **68/68**. Nobody outside this repo can install Caelum yet — that is what M5 ends.

**Next:** M5 parity: **#120** (*only* the `p-chips` form/CVA half closes the row) — the last one. Then release: **#514** rename → **#851** release engineering → **#852** publish `0.1.0` (**owner-run**). Carried, non-gating: #811/#812/#813/#832/#836/#840/#844/#846/#228 + #854–#858 (drawer) + #863–#866 (alert) + #870/#871 (panel) + #875/#877–#881 (menu; **#879 is a real `#774`-class bug in `cae-menubar`**) + **#886** (CI).
...
**Resume:** `main` @ `bcce718`, tree clean · **M5 open, no in-flight slice** · next: take **#120** (`p-chips` form/CVA half) — the last parity slice; claim + branch per `ship_pr` step 0 · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE** (a wrapper's exit can mask it); a Linux preflight cannot see a Windows-only break (#814). **A ledger row ships in its own post-merge PR** (#860). Two #150 lessons are in [`docs/PATTERNS.md`](docs/PATTERNS.md) §14/§15. **A timeout-shaped red on windows-latest is #886** (81 axe tests at vitest's 5s default), not your slice — diagnose, don't assume.
