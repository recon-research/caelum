...
**As of:** 2026-07-20 · main @ `82ace61` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `79fdfbe`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **stepper reconciler cluster CLOSED** — #608 identity (PR #626), #606 unmet-request (PR #629), #607 plain-property boundary (PR #633); plus **commit-msg guard #627** (PR #631) and **vacuous stepper assertions #632** (PR #635 — one test could not fail). **1448 tests** (1379 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `82ace61`, tree clean, CI green · last shipped: vacuous stepper assertions #632 (PR #635) · **no in-flight slice** · **next:** #611+#605 stepper keyboard trap (UNBLOCKED by D-623), then #333/#165 routerLink + icons (D-595/D-596); also chip-set #553 (⚠ item 1 API fork, item 2 browser), splitter #544/#545 (⚠ jsdom-invisible), carousel #276 loop · stepper cluster CLOSED · #602 SSR-gated per D-11; #586 part 2 refuted · #559/#574 **M4-gated** · decision-blocked: #333 (#595/#596), #282/#283 (#526), #549, #418 · verify: `bash scripts/preflight.sh` **with node on PATH** (nvm bin; else format/build/test/lint silently skip & falsely PASS — #538).
