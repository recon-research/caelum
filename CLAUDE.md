...
**As of:** 2026-07-15 · main @ `caf95c2` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `12f3608`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **galleria `[showIndicators]`** #288 (PR #457 — opt-in dot nav; role=group roving buttons + `--cae-target-min` hit-target floor); **image-compare RTL Page keys** #396 (PR #454 — horizontal PageUp/PageDown invert under RTL); **tree-select `[showClear]`** #282 (PR #452 — ×-clear + focus-restore + hit-target floor + dup-key warn). **1242 tests** (1173 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).
...
**Resume:** `main` @ `caf95c2`, tree clean, CI green · landed: galleria `[showIndicators]` #288 (#457), image-compare RTL Page #396 (#454), tree-select `[showClear]` #282 (#452) · next = standing menu / next M3 (see **Next**; galleria #288 rest (caption/numVisible/nav-split/fullScreen/vertical-RTL) + #289 zoom; tree-select #282 filter/per-node-disabled, #280/#283; carousel #276 + #445; chip-set #201/#448) · new control → floor hit target w/ `--cae-target-min` (#456) · **dev inner loop**: `CI=true npx ng test caelum --filter '<test-name>'` (#414) + `npm run build:lib:fast` · verify: `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
