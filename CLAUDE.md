...
**Resume:** `main` @ `919afed`, tree clean, CI green · last shipped: **#510** `--mat-sys-*` migration + bridge pinned (PR #753) · **no in-flight slice** · **decisions: #735, #744** (provisional, ratify by silence 07-28 → `D-NN`) · **NEXT: #413** density baseline — semi-public Sass API, so **file a `decision` first** — or the next **#405** box · new: **#752** (VR overlay blind spot), **#755** (bridge binds 11/146 roles; needs a `decision`) · verify: `bash scripts/preflight.sh` **node on PATH** (#538), read the **EXIT CODE**, and re-run it **after** doc edits — a stale PASS reddened #753.
