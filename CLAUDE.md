...
**As of:** 2026-07-15 · main @ `6bd4022` · CI green · **repo LIVE + protected** ([github.com/recon-research/caelum](https://github.com/recon-research/caelum)) · **Phase 2** · **M0 ☑ · M1 ☑ · M2 ☑** (grid gap; editor #232 / charts #233 → on-demand per D-18) · **M3 ◐ Build-S/M long tail** (+ early-theme density pulled forward from M4) · template @ pyxis `12f3608`. Full slice index + per-slice detail → [`docs/ROADMAP.md`](docs/ROADMAP.md); recipe + gotchas → [`docs/PATTERNS.md`](docs/PATTERNS.md).

**Where we are:** M3 active — the ★ families are COMPLETE (media, tree, input, Splitter, drag-drop) and breadcrumb #332 shipped; **standing menu** (parity-extras/debt) is the lane, with **early-theme density pulled forward from M4**. Recently landed: **chip-set `[emptyFocusTarget]` dev-warn** #206 (PR #437); **carousel #276** `responsiveOptions` (PR #433) + inline-axis RTL (PR #435 — vertical/swipe/loop deferred); **splitter persist-collapse** #399 (PR #431); **`caeMaskComplete`** validator #315 (PR #429 — first `ValidatorFn`; #428 provisional). **1202 tests** (1133 caelum + 69 forge); gates GREEN. Detail → the PRs + [`docs/ROADMAP.md`](docs/ROADMAP.md).

**Next:** continue the **standing menu** or start the next M3 component. Candidates: breadcrumb #333 extras (per-item icons = cross-cutting **icon-supply** latent fork; overflow/collapse [no-overlay invariant]; routerLink [`@angular/router` peerDep fork, shared w/ tab-menu #165]), scroll-panel #329, splitter px min/max #418, image-compare content-projection #422, input-family #305/#312/#315, galleria #287–#289, tree-select #280–#283, carousel #276, tree-table #264–#270, cae-table/grid clusters, chip-set #201/#205, test-infra #240 (M4).
...
**Resume:** `main` @ `6bd4022`, tree clean, CI green · landed: chip-set #206 (#437), carousel #276 (#433/#435), splitter #399, `caeMaskComplete` #315 (#428) · next = standing menu / next M3 (see **Next**; #276 remainder = vertical/swipe/loop/focus) · **dev inner loop**: `CI=true npx ng test caelum --filter '<test-name>'` (not `--include`; #414) + `npm run build:lib:fast` (full suite + `build:lib` = DoD + CI gate; #403) · verify: `PATH="$HOME/nodejs/bin:$PATH" npm ci && npm run build:lib && npx ng build forge && CI=true npx ng test caelum && CI=true npx ng test` (1202 pass) · `python3 scripts/check_provenance.py` (GREEN) · `PATH="$HOME/nodejs/bin:$PATH" bash scripts/preflight.sh` (PASS).
