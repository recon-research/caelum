...
| `p-menubar` | `cae-menubar` | `@recon-research/caelum/menubar` | ☑ | `MatToolbar` + `MatMenu` (via `cae-menu`; v1 #153 = one-level dropdowns + CDK `FocusKeyManager` roving + Down/Up-opens + empty-items guard; **rove-onto-disabled + disabled-interactive shipped at D-859/#976** (traversal half provisional on #994); rich items/responsive collapse/RTL → #155; **nested submenus inside a group dropdown ship at #150**, inherited via the embedded `cae-menu`) | Compose | 09 |
...
- **Real gaps with their own tickets.** All three of the original list **shipped in M5**: `p-drawer`→`cae-drawer` (#709), whose one remaining edge — a top/bottom drawer — is **#854**; `p-message`→`cae-alert` (#710), which owns its live region (`role=alert`/`status` by severity); and `p-panel`/`p-fieldset`→`cae-panel`/`cae-fieldset` (#711), which splits the row into two components so the fieldset can be a real `<legend>`-named group. Each carries stated edges recorded in [`docs/MIGRATION.md`](../../docs/MIGRATION.md) §6.
